...
[Home](./index.md) &gt; [givety-lib-base](./lib-base.md) &gt; [TroveAdjustmentParams](./lib-base.troveadjustmentparams.md)

## TroveAdjustmentParams type

Parameters of an [adjustTrove()](./lib-base.transactablegivety.adjusttrove.md) transaction.
...
Defining both `depositCollateral` and `withdrawCollateral`<!-- -->, or both `borrowGUSD` and `repayGUSD` at the same time is disallowed, and will result in a type-checking error.
...
<tr> <td> borrowGUSD? </td> <td> T </td> <td> <i>(Optional)</i> The amount of GUSD that's borrowed. </td> </tr>

<tr> <td> repayGUSD? </td> <td> T </td> <td> <i>(Optional)</i> The amount of GUSD that's repaid. </td> </tr>
