...
const { Decimal, UserTrove, GUSD_LIQUIDATION_RESERVE } = require("givety-lib-base");
const { EthersGivety, EthersGivetyWithStore } = require("@givety/lib-ethers");
...
async function main() {
  ...
  const givety = await EthersGivety.connect(wallet, { useStore: "blockPolled" });

  givety.store.onLoaded = () => {
    info("Waiting for price drops...");
    tryToLiquidate(givety);
  };

  givety.store.subscribe(({ newState, oldState }) => {
...
/**
 * @param {EthersGivetyWithStore} [givety]
 */
async function tryToLiquidate(givety) {
  const { store } = givety;

  const [gasPrice, riskiestTroves] = await Promise.all([
    givety.connection.provider
      .getGasPrice()
  ...
  try {
    const liquidation = await givety.populate.liquidate(addresses, { gasPrice: gasPrice.hex });
    ...
    const expectedCompensation = total.collateral
      ...
      .add(GUSD_LIQUIDATION_RESERVE.mul(troves.length));
    ...
    const { collateralGasCompensation, gusdGasCompensation, liquidatedAddresses } = receipt.details;
    ...
    const totalCompensation = collateralGasCompensation
      .mul(store.state.price)
      .add(gusdGasCompensation);

    success(
      `Received ${bold(`${collateralGasCompensation.toString(4)} GIVE`)} + ` +
        `${bold(`${gusdGasCompensation.toString(2)} GUSD`)} compensation (` +
