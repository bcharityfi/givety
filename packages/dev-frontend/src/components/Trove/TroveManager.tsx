...
import { GivetyStoreState, Decimal, Trove, Decimalish, GUSD_MINIMUM_DEBT } from "givety-lib-base";
...
const init = ({ trove }: GivetyStoreState) => ({
  original: trove,
...
const reduce = (state: TroveManagerState, action: TroveManagerAction): TroveManagerState => {
  ...
  switch (action.type) {
    ...
    case "addMinimumDebt":
      return {
        ...state,
        edited: edited.setDebt(GUSD_MINIMUM_DEBT),
...
const select = (state: GivetyStoreState) => ({
  fees: state.fees,
...
export const TroveManager: React.FC<TroveManagerProps> = ({ collateral, debt }) => {
  const [{ original, edited, changePending }, dispatch] = useGivetyReducer(reduce, init);
  const { fees, validationContext } = useGivetySelector(select);
