...
const TroveChangeDescription: React.FC<TroveAdjustmentDescriptionParams> = ({ params }) => (
  <ActionDescription>
    {params.depositCollateral && params.borrowGUSD ? (
      <>
        You will deposit <Amount>{params.depositCollateral.prettify()} GIVE</Amount> and receive{" "}
        <Amount>
          {params.borrowGUSD.prettify()} {COIN}
        </Amount>
      </>
    ) : params.repayGUSD && params.withdrawCollateral ? (
      <>
        You will pay{" "}
        <Amount>
          {params.repayGUSD.prettify()} {COIN}
        </Amount>{" "}
        and receive <Amount>{params.withdrawCollateral.prettify()} GIVE</Amount>
      </>
    ) : params.depositCollateral && params.repayGUSD ? (
      <>
        You will deposit <Amount>{params.depositCollateral.prettify()} GIVE</Amount> and pay{" "}
        <Amount>
          {params.repayGUSD.prettify()} {COIN}
        </Amount>
      </>
    ) : params.borrowGUSD && params.withdrawCollateral ? (
      <>
        You will receive <Amount>{params.withdrawCollateral.prettify()} GIVE</Amount> and{" "}
        <Amount>
          {params.borrowGUSD.prettify()} {COIN}
        </Amount>
      </>
      ...
      <>
        You will deposit <Amount>{params.depositCollateral.prettify()} GIVE</Amount>
      </>
    ) : params.withdrawCollateral ? (
      <>
        You will receive <Amount>{params.withdrawCollateral.prettify()} GIVE</Amount>
      </>
    ) : params.borrowGUSD ? (
      <>
        You will receive{" "}
...
export const selectForTroveChangeValidation = ({
  ...
  gusdBalance,
  numberOfTroves
}: GivetyStoreState) => ({ price, total, accountBalance, gusdBalance, numberOfTroves });
  ...
  if (change.type === "invalidCreation") {
    ...
    return [
      ...
      <ErrorDescription>
        ...
        <Amount>
          {GUSD_MINIMUM_DEBT.toString()} {COIN}
        </Amount>
        .
      </ErrorDescription>
    ];
...
const validateTroveCreation = (
  { depositCollateral, borrowGUSD }: TroveCreationParams<Decimal>,
  {
    resultingTrove,
    recoveryMode,
    wouldTriggerRecoveryMode,
  ...
  if (borrowGUSD.lt(GUSD_MINIMUM_NET_DEBT)) {
    return (
      <ErrorDescription>
        You must borrow at least{" "}
        <Amount>
          {GUSD_MINIMUM_NET_DEBT.toString()} {COIN}
        </Amount>
        .
      </ErrorDescription>
    );
  ...
  if (depositCollateral.gt(accountBalance)) {
    return (
      <ErrorDescription>
        ...
        <Amount>{depositCollateral.sub(accountBalance).prettify()} GIVE</Amount>.
      </ErrorDescription>
    );
  }
...
const validateTroveAdjustment = (
  { depositCollateral, withdrawCollateral, borrowGUSD, repayGUSD }: TroveAdjustmentParams<Decimal>,
  {
    originalTrove,
    resultingTrove,
    recoveryMode,
  ...
  if (repayGUSD) {
    if (resultingTrove.debt.lt(GUSD_MINIMUM_DEBT)) {
      return (
        <ErrorDescription>
          Total debt must be at least{" "}
          <Amount>
    ...
    if (repayGUSD.gt(gusdBalance)) {
      return (
        <ErrorDescription>
          The amount you're trying to repay exceeds your balance by{" "}
          <Amount>
            {repayGUSD.sub(gusdBalance).prettify()} {COIN}
          </Amount>
          .
        </ErrorDescription>
      );
...
const validateTroveClosure = (
  { repayGUSD }: TroveClosureParams<Decimal>,
  {
    recoveryMode,
    wouldTriggerRecoveryMode,
    numberOfTroves,
