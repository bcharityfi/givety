...
const selector = (state: GivetyStoreState) => {
...
const GAS_ROOM_GIVE = Decimal.from(0.1);

const feeFrom = (original: Trove, edited: Trove, borrowingRate: Decimal): Decimal => {
  const change = original.whatChanged(edited, borrowingRate);

  if (change && change.type !== "invalidCreation" && change.params.borrowGUSD) {
    return change.params.borrowGUSD.mul(borrowingRate);
  } else {
    return Decimal.ZERO;
  }
};
...
export const Adjusting: React.FC = () => {
  ...
  const { trove, fees, price, accountBalance, validationContext } = useGivetySelector(selector);
  ...
  const fee = isDebtIncrease
    ...
    : Decimal.ZERO;
  const totalDebt = netDebt.add(GUSD_LIQUIDATION_RESERVE).add(fee);
  const maxBorrowingRate = borrowingRate.add(0.005);
  const updatedTrove = isDirty ? new Trove(collateral, totalDebt) : trove;
  const feePct = new Percent(borrowingRate);
  const availableEth = accountBalance.gt(GAS_ROOM_GIVE)
    ? accountBalance.sub(GAS_ROOM_GIVE)
    : Decimal.ZERO;
    ...
    <Card>
      ...
      <Box sx={{ p: [2, 3] }}>
        ...
        <StaticRow
          ...
          amount={`${GUSD_LIQUIDATION_RESERVE}`}
          unit={COIN}
          infoIcon={
            <InfoIcon
              tooltip={
        ...
        <StaticRow
          ...
          infoIcon={
            <InfoIcon
              tooltip={
                <Card variant="tooltip" sx={{ width: "240px" }}>
                  The total amount of GUSD your Trove will hold.{" "}
                  {isDirty && (
                    <>
                      You will need to repay {totalDebt.sub(GUSD_LIQUIDATION_RESERVE).prettify(2)}{" "}
                      GUSD to reclaim your collateral ({GUSD_LIQUIDATION_RESERVE.toString()} GUSD
