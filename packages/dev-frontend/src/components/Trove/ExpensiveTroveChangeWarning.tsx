...
import { PopulatedEthersGivetyTransaction } from "@givety/lib-ethers";
...
export type GasEstimationState =
  ...
  | { type: "complete"; populatedTx: PopulatedEthersGivetyTransaction };
    ...
    if (troveChange && troveChange.type !== "closure") {
      ...
      const timeoutId = setTimeout(async () => {
        const populatedTx = await (troveChange.type === "creation"
          ? givety.populate.openTrove(troveChange.params, {
              maxBorrowingRate,
              borrowingFeeDecayToleranceMinutes
            })
          : givety.populate.adjustTrove(troveChange.params, {
              maxBorrowingRate,
              borrowingFeeDecayToleranceMinutes
            }));
