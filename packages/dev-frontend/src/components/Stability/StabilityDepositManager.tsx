...
import { Decimal, Decimalish, GivetyStoreState } from "givety-lib-base";
...
const init = ({ stabilityDeposit }: GivetyStoreState) => ({
  originalDeposit: stabilityDeposit,
  editedGUSD: stabilityDeposit.currentGUSD,
  changePending: false
});
...
const reduce = (
  ...
  const { originalDeposit, editedGUSD, changePending } = state;
  ...
  switch (action.type) {
    ...
    case "setDeposit":
      return { ...state, editedGUSD: Decimal.from(action.newValue) };

    case "revert":
      return { ...state, editedGUSD: originalDeposit.currentGUSD };
    ...
    case "updateStore": {
      ...
      const changeCommitted =
        !updatedDeposit.initialGUSD.eq(originalDeposit.initialGUSD) ||
        updatedDeposit.currentGUSD.gt(originalDeposit.currentGUSD) ||
        updatedDeposit.collateralGain.lt(originalDeposit.collateralGain) ||
        updatedDeposit.gvtyReward.lt(originalDeposit.gvtyReward);
      ...
      return {
        ...newState,
        editedGUSD: updatedDeposit.apply(originalDeposit.whatChanged(editedGUSD))
      };
    }
  }
};
...
export const StabilityDepositManager: React.FC = () => {
  const [{ originalDeposit, editedGUSD, changePending }, dispatch] = useGivetyReducer(reduce, init);
  const validationContext = useGivetySelector(selectForStabilityDepositChangeValidation);
