...
const selector = ({ stabilityDeposit, trove, gusdInStabilityPool }: GivetyStoreState) => ({
  stabilityDeposit,
  trove,
  gusdInStabilityPool
});
...
export const ActiveDeposit: React.FC = () => {
  ...
  const { stabilityDeposit, trove, gusdInStabilityPool } = useGivetySelector(selector);

  const poolShare = stabilityDeposit.currentGUSD.mulDiv(100, gusdInStabilityPool);

  const handleAdjustDeposit = useCallback(() => {
    dispatchEvent("ADJUST_DEPOSIT_PRESSED");
  }, [dispatchEvent]);

  const hasReward = !stabilityDeposit.gvtyReward.isZero;
    ...
    <Card>
      ...
      <Box sx={{ p: [2, 3] }}>
        <Box>
          <DisabledEditableRow
            label="Deposit"
            inputId="deposit-gusd"
            amount={stabilityDeposit.currentGUSD.prettify()}
            unit={COIN}
          />
          ...
          <Flex sx={{ alignItems: "center" }}>
            <StaticRow
              ...
              amount={stabilityDeposit.gvtyReward.prettify()}
              color={stabilityDeposit.gvtyReward.nonZero && "success"}
              unit={GT}
              infoIcon={
                <InfoIcon
                  tooltip={
        ...
        <Flex variant="layout.actions">
          ...
          <ClaimRewards disabled={!hasGain && !hasReward}>Claim GIVE and GVTY</ClaimRewards>
        </Flex>

        {hasTrove && (
          <ClaimAndMove disabled={!hasGain}>Claim GVTY and move GIVE to Trove</ClaimAndMove>
        )}
      </Box>
