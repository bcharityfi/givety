...
export type GivetyFrontendConfig = {
  frontendTag: string;
  infuraApiKey?: string;
  testnetOnly?: boolean;
};

const defaultConfig: GivetyFrontendConfig = {
...
const parseConfig = (json: unknown): GivetyFrontendConfig => {
...
let configPromise: Promise<GivetyFrontendConfig> | undefined = undefined;
...
export const getConfig = (): Promise<GivetyFrontendConfig> => {
  if (!configPromise) {
