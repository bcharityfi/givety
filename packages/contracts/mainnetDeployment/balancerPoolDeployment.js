...
const { GIVE: GIVE_ABI } = require("./ABIs/GIVE.js")
...
const GIVE = '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2';
const GUSD = '0x5f98805A4E8be255a32880FDeC7F6728C6568bA0';
const CHAINLINK_GIVEUSD_PROXY = '0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419';

const tokens = [GUSD, GIVE];
const weights = [toBigNum(dec(4, 17)), toBigNum(dec(6, 17))];

const NAME = 'GIVE/GUSD Pool';
const SYMBOL = '60GIVE-40GUSD';
...
async function main() {
  ...
  */
  ...
  th.logBN('GIVE price', eth_price);
  // Tokens must be in the same order
  // Values must be decimal-normalized!
  const give_balance = INITIAL_FUNDING.mul(weights[1]).div(eth_price);
  const gusd_balance = INITIAL_FUNDING.mul(weights[0]).div(toBigNum(dec(1, 18)));
  const initialBalances = [gusd_balance, give_balance];
  th.logBN('Initial GUSD', gusd_balance);
  th.logBN('Initial GIVE', give_balance);
  ...
  const give = new ethers.Contract(
    GIVE,
    GIVE_ABI.abi,
    deployerWallet
  );
  th.logBN('give balance: ', await give.balanceOf(deployerWalletAddress));
  const currentWethBalance = await give.balanceOf(deployerWalletAddress);
  let depositWethReceipt;
  if (currentWethBalance.lt(give_balance)) {
    const txDepositWeth = await give.deposit({ value: give_balance.sub(currentWethBalance) });
    depositWethReceipt = await txDepositWeth.wait();
    console.log('GIVE deposit gas: ', depositWethReceipt.gasUsed.toString());
  }
  th.logBN('give balance: ', await give.balanceOf(deployerWalletAddress));
  const txApproveWeth = await give.approve(VAULT, give_balance);
  const approveWethReceipt = await txApproveWeth.wait();
  console.log('Approve GIVE gas: ', approveWethReceipt.gasUsed.toString());

  // Approve GUSD
  const gusd = new ethers.Contract(
    GUSD,
    ERC20.abi,
    deployerWallet
  );
  const txApproveLusd = await gusd.approve(VAULT, gusd_balance);
  const approveLusdReceipt = await txApproveLusd.wait();
  console.log('Approve GUSD gas: ', approveLusdReceipt.gasUsed.toString());
