...
const GVTYStaking = artifacts.require("./GVTY/GVTYStaking.sol")
const GVTYToken = artifacts.require("./GVTY/GVTYToken.sol")
const LockupContractFactory = artifacts.require("./GVTY/LockupContractFactory.sol")
const CommunityIssuance = artifacts.require("./GVTY/CommunityIssuance.sol")
const HintHelpers = artifacts.require("./HintHelpers.sol")

const CommunityIssuanceTester = artifacts.require("./GVTY/CommunityIssuanceTester.sol")
...
const getUSDCostFromGasCost = (deploymentGasTotal, gasPriceInGwei, ETHPrice) => {
  ...
  console.log(`At gas price ${gasPriceInGwei} GWei, and GIVE Price $${ETHPrice} per GIVE, the total cost of deployment in USD is: $${dollarCost}`)
}
...
async function main() {
  const coreContracts = await dh.deployGivetyCoreHardhat()
  const GVTYContracts = await dh.deployGVTYContractsHardhat(ARBITRARY_ADDRESS, ARBITRARY_ADDRESS)
  const testerContracts = await dh.deployTesterContractsHardhat()

  await dh.connectCoreContracts(coreContracts, GVTYContracts)
  await dh.connectGVTYContracts(GVTYContracts)
  await dh.connectGVTYContractsToCore(GVTYContracts, coreContracts)


  console.log(`\n`)
  console.log(`GVTY CONTRACTS`)
  await logContractDeploymentCosts(GVTYContracts)
  console.log(`\n`)
  logContractBytecodeLengths(GVTYContractABIs)
  console.log(`\n`)
