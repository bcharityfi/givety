...
const MoneyValues = {
  ...
  negative_1e18: "-" + web3.utils.toWei('1', 'give'),
  negative_10e18: "-" + web3.utils.toWei('10', 'give'),
  negative_50e18: "-" + web3.utils.toWei('50', 'give'),
  negative_100e18: "-" + web3.utils.toWei('100', 'give'),
  negative_101e18: "-" + web3.utils.toWei('101', 'give'),
  negative_eth: (amount) => "-" + web3.utils.toWei(amount, 'give'),
...
class TestHelper {
  ...
  static randDecayFactor(min, max) {
    ...
    const amountInWei = web3.utils.toWei(amount.toFixed(18), 'give')
    return amountInWei
  }

  static randAmountInWei(min, max) {
    const amount = Math.random() * (max - min) + min;
    const amountInWei = web3.utils.toWei(amount.toString(), 'give')
    return amountInWei
  }
  ...
  static makeWei(num) {
    return web3.utils.toWei(num.toString(), 'give')
  }
  ...
  // To checkRecoveryMode / getTCR from the Givety mainnet contracts, pass a price value - this can be the lastGoodPrice
  // stored in Givety, or the current Chainlink GIVEUSD price, etc.
  ...
  // Adds the gas compensation (50 GUSD)
  ...
  /*
   * given the requested GUSD amomunt in openTrove, returns the total debt
   * So, it adds the gas compensation and the borrowing fee
   */
  static async getOpenTroveTotalDebt(contracts, gusdAmount) {
    const fee = await contracts.troveManager.getBorrowingFee(gusdAmount)
    const compositeDebt = await this.getCompositeDebt(contracts, gusdAmount)
    return compositeDebt.add(fee)
  }

  /*
   * given the desired total debt, returns the GUSD amount that needs to be requested in openTrove
   * So, it subtracts the gas compensation and then the borrowing fee
   */
  static async getOpenTroveGUSDAmount(contracts, totalDebt) {
  ...
  static async getAmountWithBorrowingFee(contracts, gusdAmount) {
    const fee = await contracts.troveManager.getBorrowingFee(gusdAmount)
    return gusdAmount.add(fee)
  }
  ...
  static getEmittedRedemptionValues(redemptionTx) {
    for (let i = 0; i < redemptionTx.logs.length; i++) {
      if (redemptionTx.logs[i].event === "Redemption") {
        ...
        const GUSDAmount = redemptionTx.logs[i].args[0]
        const totalGUSDRedeemed = redemptionTx.logs[i].args[1]
        const totalETHDrawn = redemptionTx.logs[i].args[2]
        const ETHFee = redemptionTx.logs[i].args[3]

        return [GUSDAmount, totalGUSDRedeemed, totalETHDrawn, ETHFee]
      }
    }
  ...
  static getEmittedLiquidationValues(liquidationTx) {
    for (let i = 0; i < liquidationTx.logs.length; i++) {
      if (liquidationTx.logs[i].event === "Liquidation") {
        ...
        const gusdGasComp = liquidationTx.logs[i].args[3]

        return [liquidatedDebt, liquidatedColl, collGasComp, gusdGasComp]
      }
    }
  ...
  static getGUSDFeeFromGUSDBorrowingEvent(tx) {
    for (let i = 0; i < tx.logs.length; i++) {
      if (tx.logs[i].event === "GUSDBorrowingFeePaid") {
        return (tx.logs[i].args[1]).toString()
      }
    }
    throw ("The transaction logs do not contain an GUSDBorrowingFeePaid event")
  }
  ...
  static async getEntireCollAndDebt(contracts, account) {
    ...
    const pendingGUSDDebtReward = await contracts.troveManager.getPendingGUSDDebtReward(account)
    const entireColl = rawColl.add(pendingETHReward)
    const entireDebt = rawDebt.add(pendingGUSDDebtReward)
  ...
  static async getCollAndDebtFromWithdrawGUSD(contracts, account, amount) {
  ...
  static async getCollAndDebtFromRepayGUSD(contracts, account, amount) {
    const { entireColl, entireDebt } = await this.getEntireCollAndDebt(contracts, account)
  ...
  static async getCollAndDebtFromAdjustment(contracts, account, ETHChange, GUSDChange) {
    const { entireColl, entireDebt } = await this.getEntireCollAndDebt(contracts, account)

    // const coll = (await contracts.troveManager.Troves(account))[1]
    // const debt = (await contracts.troveManager.Troves(account))[0]

    const fee = GUSDChange.gt(this.toBN('0')) ? await contracts.troveManager.getBorrowingFee(GUSDChange) : this.toBN('0')
    const newColl = entireColl.add(ETHChange)
    const newDebt = entireDebt.add(GUSDChange).add(fee)
  ...
  static async openTrove_allAccounts(accounts, contracts, ETHAmount, GUSDAmount) {
    const gasCostList = []
    const totalDebt = await this.getOpenTroveTotalDebt(contracts, GUSDAmount)

    for (const account of accounts) {
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, ETHAmount, totalDebt)

      const tx = await contracts.borrowerOperations.openTrove(this._100pct, GUSDAmount, upperHint, lowerHint, { from: account, value: ETHAmount })
  ...
  static async openTrove_allAccounts_randomETH(minETH, maxETH, accounts, contracts, GUSDAmount) {
    const gasCostList = []
    const totalDebt = await this.getOpenTroveTotalDebt(contracts, GUSDAmount)
    ...
    for (const account of accounts) {
      ...
      const tx = await contracts.borrowerOperations.openTrove(this._100pct, GUSDAmount, upperHint, lowerHint, { from: account, value: randCollAmount })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async openTrove_allAccounts_randomGIVE_ProportionalGUSD(minETH, maxETH, accounts, contracts, proportion) {
    const gasCostList = []
  
    for (const account of accounts) {
      const randCollAmount = this.randAmountInWei(minETH, maxETH)
      const proportionalGUSD = (web3.utils.toBN(proportion)).mul(web3.utils.toBN(randCollAmount))
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, proportionalGUSD)

      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, randCollAmount, totalDebt)

      const tx = await contracts.borrowerOperations.openTrove(this._100pct, proportionalGUSD, upperHint, lowerHint, { from: account, value: randCollAmount })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async openTrove_allAccounts_randomGIVE_randomGUSD(minETH, maxETH, accounts, contracts, minGUSDProportion, maxGUSDProportion, logging = false) {
    const gasCostList = []
    const price = await contracts.priceFeedTestnet.getPrice()
    ...
    for (const account of accounts) {
      ...
      const randGUSDProportion = this.randAmountInWei(minGUSDProportion, maxGUSDProportion)
      const proportionalGUSD = (web3.utils.toBN(randGUSDProportion)).mul(web3.utils.toBN(randCollAmount).div(_1e18))
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, proportionalGUSD)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, randCollAmount, totalDebt)
      ...
      if (logging && tx.receipt.status) {
        i++
        const ICR = await contracts.troveManager.getCurrentICR(account, price)
        // console.log(`${i}. Trove opened. addr: ${this.squeezeAddr(account)} coll: ${randCollAmount} debt: ${proportionalGUSD} ICR: ${ICR}`)
      }
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
  ...
  static async openTrove_allAccounts_randomGUSD(minGUSD, maxGUSD, accounts, contracts, ETHAmount) {
    const gasCostList = []

    for (const account of accounts) {
      const randGUSDAmount = this.randAmountInWei(minGUSD, maxGUSD)
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, randGUSDAmount)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, ETHAmount, totalDebt)

      const tx = await contracts.borrowerOperations.openTrove(this._100pct, randGUSDAmount, upperHint, lowerHint, { from: account, value: ETHAmount })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async openTrove_allAccounts_decreasingGUSDAmounts(accounts, contracts, ETHAmount, maxGUSDAmount) {
    const gasCostList = []

    let i = 0
    for (const account of accounts) {
      const GUSDAmount = (maxGUSDAmount - i).toString()
      const GUSDAmountWei = web3.utils.toWei(GUSDAmount, 'give')
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, GUSDAmountWei)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, ETHAmount, totalDebt)

      const tx = await contracts.borrowerOperations.openTrove(this._100pct, GUSDAmountWei, upperHint, lowerHint, { from: account, value: ETHAmount })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
      i += 1
    }
  ...
  static async openTrove(contracts, {
    ...
    if (!extraGUSDAmount) extraGUSDAmount = this.toBN(0)
    else if (typeof extraGUSDAmount == 'string') extraGUSDAmount = this.toBN(extraGUSDAmount)
    ...
    const gusdAmount = MIN_DEBT.add(extraGUSDAmount)
    ...
    const tx = await contracts.borrowerOperations.openTrove(maxFeePercentage, gusdAmount, upperHint, lowerHint, extraParams)
  ...
  static async withdrawGUSD(contracts, {
    maxFeePercentage,
    gusdAmount,
    ICR,
    upperHint,
    ...
    assert(!(gusdAmount && ICR) && (gusdAmount || ICR), "Specify either gusd amount or target ICR, but not both")
    ...
    if (ICR) {
      ...
      gusdAmount = await this.getNetBorrowingAmount(contracts, increasedTotalDebt)
    } else {
      increasedTotalDebt = await this.getAmountWithBorrowingFee(contracts, gusdAmount)
    }

    await contracts.borrowerOperations.withdrawGUSD(maxFeePercentage, gusdAmount, upperHint, lowerHint, extraParams)
  ...
  static async adjustTrove_allAccounts(accounts, contracts, ETHAmount, GUSDAmount) {
    const gasCostList = []
    ...
    for (const account of accounts) {
      ...
      let GUSDChangeBN = this.toBN(GUSDAmount)

      const { newColl, newDebt } = await this.getCollAndDebtFromAdjustment(contracts, account, ETHChangeBN, GUSDChangeBN)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const zero = this.toBN('0')

      let isDebtIncrease = GUSDChangeBN.gt(zero)
      GUSDChangeBN = GUSDChangeBN.abs() 

      // Add GIVE to trove
      if (ETHChangeBN.gt(zero)) {
        tx = await contracts.borrowerOperations.adjustTrove(this._100pct, 0, GUSDChangeBN, isDebtIncrease, upperHint, lowerHint, { from: account, value: ETHChangeBN })
      // Withdraw GIVE from trove
      } else if (ETHChangeBN.lt(zero)) {
        ETHChangeBN = ETHChangeBN.neg()
        tx = await contracts.borrowerOperations.adjustTrove(this._100pct, ETHChangeBN, GUSDChangeBN, isDebtIncrease, upperHint, lowerHint, { from: account })
      }
  ...
  static async adjustTrove_allAccounts_randomAmount(accounts, contracts, ETHMin, ETHMax, GUSDMin, GUSDMax) {
    const gasCostList = []
    ...
    for (const account of accounts) {
      ...
      let GUSDChangeBN = this.toBN(this.randAmountInWei(GUSDMin, GUSDMax))
      ...
      // Withdraw GIVE from trove
        ...
        ETHChangeBN = ETHChangeBN.neg()
        tx = await contracts.borrowerOperations.adjustTrove(this._100pct, ETHChangeBN, GUSDChangeBN, isDebtIncrease, lowerHint,  upperHint,{ from: account })
      }

      const gas = this.gasUsed(tx)
      // console.log(`GIVE change: ${ETHChangeBN},  GUSDChange: ${GUSDChangeBN}, gas: ${gas} `)
  ...
  static async withdrawGUSD_allAccounts(accounts, contracts, amount) {
    const gasCostList = []

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromWithdrawGUSD(contracts, account, amount)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.withdrawGUSD(this._100pct, amount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async withdrawGUSD_allAccounts_randomAmount(min, max, accounts, contracts) {
    const gasCostList = []

    for (const account of accounts) {
      const randGUSDAmount = this.randAmountInWei(min, max)

      const { newColl, newDebt } = await this.getCollAndDebtFromWithdrawGUSD(contracts, account, randGUSDAmount)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.withdrawGUSD(this._100pct, randGUSDAmount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async repayGUSD_allAccounts(accounts, contracts, amount) {
    const gasCostList = []

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromRepayGUSD(contracts, account, amount)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.repayGUSD(amount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async repayGUSD_allAccounts_randomAmount(min, max, accounts, contracts) {
    const gasCostList = []

    for (const account of accounts) {
      const randGUSDAmount = this.randAmountInWei(min, max)

      const { newColl, newDebt } = await this.getCollAndDebtFromRepayGUSD(contracts, account, randGUSDAmount)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, newDebt)

      const tx = await contracts.borrowerOperations.repayGUSD(randGUSDAmount, upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async redeemCollateral(redeemer, contracts, GUSDAmount, maxFee = this._100pct) {
    const price = await contracts.priceFeedTestnet.getPrice()
    const tx = await this.performRedemptionTx(redeemer, price, contracts, GUSDAmount, maxFee)
    const gas = await this.gasUsed(tx)
    return gas
  }

  static async redeemCollateralAndGetTxObject(redeemer, contracts, GUSDAmount, maxFee = this._100pct) {
    const price = await contracts.priceFeedTestnet.getPrice()
    const tx = await this.performRedemptionTx(redeemer, price, contracts, GUSDAmount, maxFee)
    return tx
  }
  ...
  static async redeemCollateral_allAccounts_randomAmount(min, max, accounts, contracts) {
    ...
    for (const redeemer of accounts) {
      ...
      await this.performRedemptionTx(redeemer, price, contracts, randGUSDAmount)
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async performRedemptionTx(redeemer, price, contracts, GUSDAmount, maxFee = 0) {
    const redemptionhint = await contracts.hintHelpers.getRedemptionHints(GUSDAmount, price, 0)
    ...
    const tx = await contracts.troveManager.redeemCollateral(GUSDAmount,
      firstRedemptionHint,
  ...
  static async provideToSP_allAccounts_randomAmount(min, max, accounts, stabilityPool) {
    const gasCostList = []
    for (const account of accounts) {
      const randomGUSDAmount = this.randAmountInWei(min, max)
      const tx = await stabilityPool.provideToSP(randomGUSDAmount, this.ZERO_ADDRESS, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async withdrawFromSP_allAccounts_randomAmount(min, max, accounts, stabilityPool) {
    ...
    for (const account of accounts) {
      const randomGUSDAmount = this.randAmountInWei(min, max)
      const tx = await stabilityPool.withdrawFromSP(randomGUSDAmount, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  static async withdrawGIVEGainToTrove_allAccounts(accounts, contracts) {
    const gasCostList = []
    for (const account of accounts) {
      ...
      console.log(`entireColl: ${entireColl}`)
      console.log(`entireDebt: ${entireDebt}`)
      const GIVEGain = await contracts.stabilityPool.getDepositorGIVEGain(account)
      const newColl = entireColl.add(GIVEGain)
      const {upperHint, lowerHint} = await this.getBorrowerOpsListHint(contracts, newColl, entireDebt)

      const tx = await contracts.stabilityPool.withdrawGIVEGainToTrove(upperHint, lowerHint, { from: account })
      const gas = this.gasUsed(tx)
      gasCostList.push(gas)
    }
    return this.getGasMetrics(gasCostList)
  ...
  // --- GVTY & Lockup Contract functions ---
  ...
  static async getTimeFromSystemDeployment(gvtyToken, web3, timePassedSinceDeployment) {
    const deploymentTime = await gvtyToken.getDeploymentStartTime()
