...
class ModelParams:
    def __init__(self):
        ...
        self.lookback = 5 # Lookback parameter for GIVE price momentum
...
### Functions
def get_new_momentum(data, params, GIVE_price):
    lookback = params.lookback
    if lookback == 0:
        return 0

    GIVE_price_past = get_past_GIVE_price(data, params)

    new_momentum = (GIVE_price - GIVE_price_past) /  GIVE_price_past
    return new_momentum

def get_past_GIVE_price(data, params):
    length = len(data.GIVE_price)
    
    GIVE_price_past = None
    if (params.lookback > length):
        GIVE_price_past = data.GIVE_price[0]
    else:
        GIVE_price_past = data.GIVE_price[length - params.lookback - 1]

    if GIVE_price_past == 0:
        return 1
    return GIVE_price_past
...
# return the exogenous market demand for holding GVTY tokens. Assume constant. Could be a function of:
...
# Given Givety's hard price ceiling of 1.10, 
...
### Various GIVE price functions

def constant_GIVE_price(last_price):
    return last_price

# GIVE price generator is a random walk (normal dist.), with occasional large +ve and -ve jumps
def randomwalk_GIVE_price(last_price):
    big_event = 0
...
def linear_increasing_GIVE_price(last_price, gradient):
    return last_price + gradient

def oscillating_GIVE_price(min, magnitude, i):
    return min + magnitude + magnitude*np.sin(i)

def linear_decreasing_GIVE_price(start, gradient, i):
    val = (start - (gradient*i))
    if val <= 0:
        return 0
    return val

def one_over_i_GIVE_price(scale, i):
    return scale/i

def quadratic_GIVE_price(min, scale, i):
    return min + scale*(i**2)

def sublinear_GIVE_price(last_price, steepness, i):
...
for i in range(1, 250):
    last_GIVE_price =  data.GIVE_price[-1]

    # update exogenous GIVE price

    # GIVE_price = last_GIVE_price
    GIVE_price = randomwalk_GIVE_price(last_GIVE_price)
    # GIVE_price = oscillating_GIVE_price(500, 100, i)
    # GIVE_price = quadratic_GIVE_price(500, 10, i)
    # GIVE_price = linear_increasing_GIVE_price(last_GIVE_price, 3)
    # GIVE_price = linear_decreasing_GIVE_price(800, 1, i)
    # GIVE_price = one_over_i_GIVE_price(1000, i)
    # GIVE_price = sublinear_GIVE_price(last_GIVE_price, 10, i)
    
    momentum = get_new_momentum(data, params, GIVE_price)
    ...
    data.GIVE_price.append(GIVE_price)
...
ax3.set_title('GIVE Price')
plt.ylim(0, 1000)
plt.plot(data.GIVE_price)
