...
def liquidate_troves(troves, index, data):
  ...
  price_GUSD_previous = data.loc[index-1,'Price_GUSD']
  price_GVTY_previous = data.loc[index-1,'price_GVTY']
  ...
  liquidation_gain = ether_liquidated*price_ether_current - debt_liquidated*price_GUSD_previous
  airdrop_gain = price_GVTY_previous * quantity_GVTY_airdrop
  ...
  if index <= day:
   return_stability = initial_return*(1+shock_return)
    ...
    return_stability = min(0.5, 365*(data.loc[index-day:index, 'liquidation_gain'].sum()+data.loc[index-day:index, 'airdrop_gain'].sum())/(price_GUSD_previous*stability_pool_previous))
  else:
    return_stability = (365/30)*(data.loc[index-month:index, 'liquidation_gain'].sum()+data.loc[index-month:index, 'airdrop_gain'].sum())/(price_GUSD_previous*stability_pool_previous)
  
  return[troves, return_stability, debt_liquidated, ether_liquidated, liquidation_gain, airdrop_gain, n_liquidate]

"""Close Troves"""

def close_troves(troves, index2, price_GUSD_previous):
  ...
  elif price_GUSD_previous >=1:
    number_closetroves = max(0, n_steady * (1+shock_closetroves))
  else:
    number_closetroves = max(0, n_steady * (1+shock_closetroves)) + beta*(1-price_GUSD_previous)*n_troves
...
def adjust_troves(troves, index):
  ...
  #A part of the troves are adjusted by adjusting debt
    if p >= ratio:
      ...
      if check>2:
        ...
        issuance_GUSD_adjust = issuance_GUSD_adjust + rate_issuance * (supply_new - working_trove['Supply'])
  ...
  return[troves, issuance_GUSD_adjust]

"""Open Troves"""

def open_troves(troves, index1, price_GUSD_previous):
  ...
  elif price_GUSD_previous <=1 + rate_issuance:
    number_opentroves = max(0, n_steady * (1+shock_opentroves))
  else:
    number_opentroves = max(0, n_steady * (1+shock_opentroves)) + alpha*(price_GUSD_previous-rate_issuance-1)*n_troves
  ...
  return[troves, number_opentroves, issuance_GUSD_open]
...
#Calculating Price
  ...
  price_GUSD_previous = float(data['Price_GUSD'][index-1])
  price_GUSD_current= price_GUSD_previous*((supply-stability_pool)/(liquidity_pool_previous*(drift_liquidity+shock_liquidity)))**(1/delta)
...
#Stabilizer
  ...
  if price_GUSD_current > 1.1 + rate_issuance:
    ...
    supply_wanted=stability_pool+liquidity_pool_previous*(drift_liquidity+shock_liquidity)*((1.1+rate_issuance)/price_GUSD_previous)**delta
  ...
  if price_GUSD_current < 1 - rate_redemption:
    ...
    #supply_current = sum(troves['Supply'])
    supply_target=stability_pool+liquidity_pool_previous*(drift_liquidity+shock_liquidity)*((1-rate_redemption)/price_GUSD_previous)**delta
    ...
    if supply_diff < redemption_ratio * liquidity_pool:
      ...
      price_GUSD_current= price_GUSD_previous * (liquidity_pool/(liquidity_pool_previous*(drift_liquidity+shock_liquidity)))**(1/delta)
  ...
  troves = troves.reset_index(drop=True)
  return[price_GUSD_current, liquidity_pool, troves, issuance_GUSD_stabilizer, redemption_fee, n_redempt, redemption_pool, n_open]
...
def GVTY_market(index, data):
  quantity_GVTY = (100000000/3)*(1-0.5**(index/period))
  ...
  return[price_GVTY_current, annualized_earning, MC_GVTY_current]

"""# Simulation Program"""

#Defining Initials
initials = {"Price_GUSD":[1.00], "Price_Ether":[price_ether_initial], "n_open":[initial_open], "n_close":[0], "n_liquidate": [0], "n_redempt":[0], 
            "n_troves":[initial_open], "stability":[0], "liquidity":[0], "redemption_pool":[0],
            "supply_GUSD":[0],  "return_stability":[initial_return], "airdrop_gain":[0], "liquidation_gain":[0],  "issuance_fee":[0], "redemption_fee":[0],
            "price_GVTY":[price_GVTY_initial], "MC_GVTY":[0], "annualized_earning":[0]}
...
result_open = open_troves(troves, 0, data['Price_GUSD'][0])
troves = result_open[0]
issuance_GUSD_open = result_open[2]
data.loc[0,'issuance_fee'] = issuance_GUSD_open * initials["Price_GUSD"][0]
data.loc[0,'supply_GUSD'] = troves["Supply"].sum()
...
#close troves
  result_close = close_troves(troves, index, price_GUSD_previous)
...
#adjust troves
  ...
  issuance_GUSD_adjust = result_adjustment[1]

#open troves
  result_open = open_troves(troves, index, price_GUSD_previous)
  troves = result_open[0]
...
#Calculating Price, Liquidity Pool, and Redemption
  ...
  price_GUSD_current = result_price[0]
  liquidity_pool = result_price[1]
  troves = result_price[2]
  issuance_GUSD_stabilizer = result_price[3]
...
#Summary
  issuance_fee = price_GUSD_current * (issuance_GUSD_adjust + issuance_GUSD_open + issuance_GUSD_stabilizer)
  n_troves = troves.shape[0]
  supply_GUSD = troves['Supply'].sum()
  if index >= month:
    price_GVTY.append(price_GVTY_current)

  new_row = {"Price_GUSD":float(price_GUSD_current), "Price_Ether":float(price_ether_current), "n_open":float(n_open), "n_close":float(n_close), 
             "n_liquidate":float(n_liquidate), "n_redempt": float(n_redempt), "n_troves":float(n_troves),
              "stability":float(stability_pool), "liquidity":float(liquidity_pool), "redemption_pool":float(redemption_pool), "supply_GUSD":float(supply_GUSD),
             ...
             "annualized_earning":float(annualized_earning), "MC_GVTY":float(MC_GVTY_current), "price_GVTY":float(price_GVTY_current)
             }
  data = data.append(new_row, ignore_index=True)
  if price_GUSD_current < 0:
    break
...
fig.add_trace(
    go.Scatter(x=data.index/720, y=data['Price_GUSD'], name="GUSD Price"),
    secondary_y=False,
)
fig.add_trace(
    go.Scatter(x=data.index/720, y=data['Price_Ether'], name="Give Price"),
    secondary_y=True,
)
fig.update_layout(
    title_text="Price Dynamics of GUSD and Give"
)
fig.update_xaxes(tick0=0, dtick=1, title_text="Month")
fig.update_yaxes(title_text="GUSD Price", secondary_y=False)
fig.update_yaxes(title_text="Give Price", secondary_y=True)
fig.show()
...
fig.add_trace(
    go.Scatter(x=data.index/720, y=data['supply_GUSD'], name="GUSD Supply"),
    secondary_y=True,
)
fig.update_layout(
    title_text="Dynamics of Trove Numbers and GUSD Supply"
)
fig.update_xaxes(tick0=0, dtick=1, title_text="Month")
fig.update_yaxes(title_text="Number of Troves", secondary_y=False)
fig.update_yaxes(title_text="GUSD Supply", secondary_y=True)
fig.show()
...
fig = make_subplots(specs=[[{"secondary_y": True}]])
fig.add_trace(
    go.Scatter(x=data.index/720, y=data['price_GVTY'], name="GVTY Price"),
    secondary_y=False,
)
fig.add_trace(
    go.Scatter(x=data.index/720, y=data['MC_GVTY'], name="GVTY Market Cap"),
    secondary_y=True,
)
fig.update_layout(
    title_text="Dynamics of the Price and Market Cap of GVTY"
)
fig.update_xaxes(tick0=0, dtick=1, title_text="Month")
fig.update_yaxes(title_text="GVTY Price", secondary_y=False)
fig.update_yaxes(title_text="GVTY Market Cap", secondary_y=True)
fig.show()
...
initials = {"Price_GUSD":[1.00], "Price_Ether":[price_ether_initial], "n_open":[initial_open], "n_close":[0], "n_liquidate": [0], "n_redempt":[0], 
            "n_troves":[initial_open], "stability":[0], "liquidity":[0], "redemption_pool":[0],
            "supply_GUSD":[0],  "return_stability":[initial_return], "airdrop_gain":[0], "liquidation_gain":[0],  "issuance_fee":[0], "redemption_fee":[0],
            "price_GVTY":[price_GVTY_initial], "MC_GVTY":[0], "annualized_earning":[0], "base_rate":[base_rate_initial]}
data2 = pd.DataFrame(initials)
troves2= pd.DataFrame({"Ether_Price":[], "Ether_Quantity":[], "CR_initial":[], 
              "Supply":[], "Rational_inattention":[], "CR_current":[]})
result_open = open_troves(troves2, 0, data2['Price_GUSD'][0])
troves2 = result_open[0]
issuance_GUSD_open = result_open[2]
data2.loc[0,'issuance_fee'] = issuance_GUSD_open * initials["Price_GUSD"][0]
data2.loc[0,'supply_GUSD'] = troves2["Supply"].sum()
...
#exogenous give price input
  ...
  price_GUSD_previous = data2.loc[index-1,'Price_GUSD']
  price_GVTY_previous = data2.loc[index-1,'price_GVTY']
...
#close troves
  result_close = close_troves(troves2, index, price_GUSD_previous)
...
#open troves
  result_open = open_troves(troves2, index, price_GUSD_previous)
  troves2 = result_open[0]
  n_open = result_open[1]  
  issuance_GUSD_open = result_open[2]
...
#GVTY Market
  result_GVTY = GVTY_market(index, data2)
  price_GVTY_current = result_GVTY[0]
  annualized_earning = result_GVTY[1]
  MC_GVTY_current = result_GVTY[2]
...
#Summary
  ...
  supply_GUSD = troves2['Supply'].sum()
  if index >= month:
    price_GVTY.append(price_GVTY_current)
...
fig.add_trace(
    go.Scatter(x=data2.index/720, y=data2['Price_GUSD'], name="GUSD Price New", line = dict(dash='dot')),
    secondary_y=False,
)
fig.update_layout(
    title_text="Price Dynamics of GUSD and Give"
...
fig.add_trace(
    go.Scatter(x=data2.index/720, y=data2['supply_GUSD'], name="GUSD Supply New", line = dict(dash='dot')),
    secondary_y=True,
)
fig.update_layout(
    title_text="Dynamics of Trove Numbers and GUSD Supply"
...
fig.add_trace(
    go.Scatter(x=data2.index/720, y=data2['price_GVTY'], name="GVTY Price New", line = dict(dash='dot')),
    secondary_y=False,
)
fig.add_trace(
    go.Scatter(x=data2.index/720, y=data2['MC_GVTY'], name="GVTY Market Cap New", line = dict(dash='dot')),
    secondary_y=True,
)
fig.update_layout(
    title_text="Dynamics of the Price and Market Cap of GVTY"
