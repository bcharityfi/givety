...
contract('Deploying and funding One Year Lockup Contracts', async accounts => {
  const [givetyAG, A, B, C, D, E, F, G, H, I, J] = accounts;
  ...
  describe('Deploying LCs', async accounts => {
    it("GVTY Deployer can deploy LCs through the Factory", async () => {
      ...
      const LCDeploymentTx_C = await lockupContractFactory.deployLockupContract(C, oneYearFromSystemDeployment, { from: givetyAG })
    ...
    it("Anyone can deploy LCs through the Factory", async () => {
      ...
      const LCDeploymentTx_3 = await lockupContractFactory.deployLockupContract(givetyAG, oneYearFromSystemDeployment, { from: I })
    ...
    it("GVTY Deployer can deploy LCs directly", async () => {
      ...
      const LC_B = await LockupContract.new(gvtyToken.address, B, oneYearFromSystemDeployment, { from: givetyAG })
      const LC_B_txReceipt = await web3.eth.getTransactionReceipt(LC_B.transactionHash)

      const LC_C = await LockupContract.new(gvtyToken.address, C, oneYearFromSystemDeployment, { from: givetyAG })
    ...
    it("Anyone can deploy LCs directly", async () => {
      ...
      const LC_B = await LockupContract.new(gvtyToken.address, B, oneYearFromSystemDeployment, { from: E })
      const LC_B_txReceipt = await web3.eth.getTransactionReceipt(LC_B.transactionHash)

      const LC_C = await LockupContract.new(gvtyToken.address, C, oneYearFromSystemDeployment, { from: F })
      const LC_C_txReceipt = await web3.eth.getTransactionReceipt(LC_C.transactionHash)
    ...
    it("LC deployment stores the beneficiary's address in the LC", async () => {
      // Deploy 5 LCs
      const deployedLCtx_A = await lockupContractFactory.deployLockupContract(A, oneYearFromSystemDeployment, { from: givetyAG })
      const deployedLCtx_B = await lockupContractFactory.deployLockupContract(B, oneYearFromSystemDeployment, { from: givetyAG })
      const deployedLCtx_C = await lockupContractFactory.deployLockupContract(C, oneYearFromSystemDeployment, { from: givetyAG })
      const deployedLCtx_D = await lockupContractFactory.deployLockupContract(D, oneYearFromSystemDeployment, { from: givetyAG })
      const deployedLCtx_E = await lockupContractFactory.deployLockupContract(E, oneYearFromSystemDeployment, { from: givetyAG })
    ...
    it("LC deployment through the Factory records the LC contract address and deployer as a k-v pair in the Factory", async () => {
      ...
      assert.equal(givetyAG, await lockupContractFactory.lockupContractToDeployer(LCAddress_A))
      assert.equal(givetyAG, await lockupContractFactory.lockupContractToDeployer(LCAddress_B))
      assert.equal(givetyAG, await lockupContractFactory.lockupContractToDeployer(LCAddress_C))
      assert.equal(givetyAG, await lockupContractFactory.lockupContractToDeployer(LCAddress_D))
      assert.equal(givetyAG, await lockupContractFactory.lockupContractToDeployer(LCAddress_E))
    })
    ...
    it("Direct deployment of LC sets the unlockTime in the LC", async () => {
      ...
      const LC_A = await LockupContract.new(gvtyToken.address, A, oneYearFromSystemDeployment, { from: givetyAG })
      const LC_B = await LockupContract.new(gvtyToken.address, B, '230582305895235', { from: B })
      const LC_C = await LockupContract.new(gvtyToken.address, C, dec(20, 18), { from: E })
    ...
    it("LC deployment through the Factory reverts when the unlockTime is < 1 year from system deployment", async () => {
      ...
      // Deploy 3 LCs through factory
      const LCDeploymentPromise_A = lockupContractFactory.deployLockupContract(A, nearlyOneYear, { from: givetyAG })
    ...
    it("Direct deployment of LC reverts when the unlockTime is < 1 year from system deployment", async () => {
      ...
      const LCDeploymentPromise_A = LockupContract.new(gvtyToken.address, A, nearlyOneYear, { from: givetyAG })
      const LCDeploymentPromise_B = LockupContract.new(gvtyToken.address, B, '37', { from: B })
      const LCDeploymentPromise_C = LockupContract.new(gvtyToken.address, C, '43200', { from: E })
  ...
  describe('Funding LCs', async accounts => {
    it("GVTY transfer from GVTY deployer to their deployed LC increases the GVTY balance of the LC", async () => {
      // Deploy 5 LCs
      const deployedLCtx_A = await lockupContractFactory.deployLockupContract(A, oneYearFromSystemDeployment, { from: givetyAG })
      const deployedLCtx_B = await lockupContractFactory.deployLockupContract(B, oneYearFromSystemDeployment, { from: givetyAG })
      const deployedLCtx_C = await lockupContractFactory.deployLockupContract(C, oneYearFromSystemDeployment, { from: givetyAG })
      ...
      assert.equal(await gvtyToken.balanceOf(LCAddress_D), '0')
      assert.equal(await gvtyToken.balanceOf(LCAddress_E), '0')

      // Multisig transfers GVTY to each LC
      await gvtyToken.transfer(LCAddress_A, GVTYEntitlement_A, { from: multisig })
      await gvtyToken.transfer(LCAddress_B, GVTYEntitlement_B, { from: multisig })
      await gvtyToken.transfer(LCAddress_C, GVTYEntitlement_C, { from: multisig })
      await gvtyToken.transfer(LCAddress_D, GVTYEntitlement_D, { from: multisig })
      await gvtyToken.transfer(LCAddress_E, GVTYEntitlement_E, { from: multisig })

      assert.equal(await gvtyToken.balanceOf(LCAddress_A), GVTYEntitlement_A)
      assert.equal(await gvtyToken.balanceOf(LCAddress_B), GVTYEntitlement_B)
      assert.equal(await gvtyToken.balanceOf(LCAddress_C), GVTYEntitlement_C)
      assert.equal(await gvtyToken.balanceOf(LCAddress_D), GVTYEntitlement_D)
      assert.equal(await gvtyToken.balanceOf(LCAddress_E), GVTYEntitlement_E)
    })

    it("GVTY Multisig can transfer GVTY to LCs deployed through the factory by anyone", async () => {  
      ...
      // Multisig transfers GVTY to each LC
      await gvtyToken.transfer(LCAddress_A, dec(1, 18), { from: multisig })
      await gvtyToken.transfer(LCAddress_B, dec(2, 18), { from: multisig })
      await gvtyToken.transfer(LCAddress_C, dec(3, 18), { from: multisig })
      await gvtyToken.transfer(LCAddress_D, dec(4, 18), { from: multisig })
      await gvtyToken.transfer(LCAddress_E, dec(5, 18), { from: multisig })

      assert.equal(await gvtyToken.balanceOf(LCAddress_A), dec(1, 18))
      assert.equal(await gvtyToken.balanceOf(LCAddress_B), dec(2, 18))
      assert.equal(await gvtyToken.balanceOf(LCAddress_C), dec(3, 18))
      assert.equal(await gvtyToken.balanceOf(LCAddress_D), dec(4, 18))
      assert.equal(await gvtyToken.balanceOf(LCAddress_E), dec(5, 18))
    })

    // can't transfer GVTY to any LCs that were deployed directly
  })
  ...
  describe('Withdrawal attempts on funded, inactive LCs immediately after funding', async accounts => {
    it("Beneficiary can't withdraw from their funded LC", async () => {
      ...
      // Multisig transfers GVTY to each LC
      await gvtyToken.transfer(LC_A.address, GVTYEntitlement_A, { from: multisig })
      await gvtyToken.transfer(LC_B.address, GVTYEntitlement_B, { from: multisig })
      await gvtyToken.transfer(LC_C.address, GVTYEntitlement_C, { from: multisig })

      assert.equal(await gvtyToken.balanceOf(LC_A.address), GVTYEntitlement_A)
      assert.equal(await gvtyToken.balanceOf(LC_B.address), GVTYEntitlement_B)
      assert.equal(await gvtyToken.balanceOf(LC_C.address), GVTYEntitlement_C)
      ...
      for (LC of LCs) {
        try {
          ...
          const withdrawalAttemptTx = await LC.withdrawGVTY({ from: beneficiary })
    ...
    it("GVTY multisig can't withraw from a LC which it funded", async () => {
      // Deploy 3 LCs
      const deployedLCtx_A = await lockupContractFactory.deployLockupContract(A, oneYearFromSystemDeployment, { from: givetyAG })
      const deployedLCtx_B = await lockupContractFactory.deployLockupContract(B, oneYearFromSystemDeployment, { from: givetyAG })
      const deployedLCtx_C = await lockupContractFactory.deployLockupContract(C, oneYearFromSystemDeployment, { from: givetyAG })
      ...
      // GVTY multisig attempts to withdraw from LCs
      for (LC of LCs) {
        try {
          const withdrawalAttemptTx = await LC.withdrawGVTY({ from: multisig })
          assert.isFalse(withdrawalAttemptTx.receipt.status)
        } catch (error) {
          assert.include(error.message, "revert")
        }
    ...
    it("No one can withraw from a LC", async () => {
      // Deploy 3 LCs
      const deployedLCtx_A = await lockupContractFactory.deployLockupContract(A, GVTYEntitlement_A, { from: D })

      // Grab contract objects from deployment tx events
      const LC_A = await th.getLCFromDeploymentTx(deployedLCtx_A)

      // GivetyAG transfers GVTY to the LC
      await gvtyToken.transfer(LC_A.address, GVTYEntitlement_A, { from: multisig })
      ...
      try {
        const withdrawalAttemptTx = await LC_A.withdrawGVTY({ from: G })
        assert.isFalse(withdrawalAttemptTx.receipt.status)
      } catch (error) {
        assert.include(error.message, "revert")
      }
      ...
      try {
        const withdrawalAttemptTx = await LC_A.withdrawGVTY({ from: H })
        assert.isFalse(withdrawalAttemptTx.receipt.status)
      } catch (error) {
        assert.include(error.message, "revert")
      }
      ...
      try {
        const withdrawalAttemptTx = await LC_A.withdrawGVTY({ from: I })
        assert.isFalse(withdrawalAttemptTx.receipt.status)
      } catch (error) {
        assert.include(error.message, "revert")
      }
