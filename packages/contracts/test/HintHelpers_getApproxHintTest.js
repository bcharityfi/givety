...
contract('HintHelpers', async accounts => {
  ...
  /* Open a Trove for each account. GUSD debt is 200 GUSD each, with collateral beginning at
  1.5 give, and rising by 0.01 give per Trove.  Hence, the ICR of account (i + 1) is always 1% greater than the ICR of account i. 
 */

 // Open Troves in parallel, then withdraw GUSD in parallel
 const makeTrovesInParallel = async (accounts, n) => {
  ...
  const withdrawGUSDpromises = activeAccounts.map(account => withdrawGUSDfromTrove(account))
  await Promise.all(withdrawGUSDpromises)
 ...
 const withdrawGUSDfromTrove = async (account) => {
  await borrowerOperations.withdrawGUSD(th._100pct, '100000000000000000000', account, account, { from: account })
 }

 // Sequentially add coll and withdraw GUSD, 1 account at a time
  const makeTrovesInSequence = async (accounts, n) => {
    ...
    for (const account of activeAccounts) {
      ...
      await th.openTrove(contracts, { extraGUSDAmount: toBN(dec(10000, 18)), ICR: ICR_BN, extraParams: { from: account } })
  ...
  it("getApproxHint(): returns the address of a Trove within sqrt(length) positions of the correct insert position", async () => {
    ...
    const CRPercent_250 = Number(web3.utils.fromWei(CR_250, 'give')) * 100
    ...
    const ICRPercent_hintAddress_250 = Number(web3.utils.fromWei(ICR_hintAddress_250, 'give')) * 100
    ...
    const CRPercent_287 = Number(web3.utils.fromWei(CR_287, 'give')) * 100
    ...
    const ICRPercent_hintAddress_287 = Number(web3.utils.fromWei(ICR_hintAddress_287, 'give')) * 100
    ...
    const CRPercent_213 = Number(web3.utils.fromWei(CR_213, 'give')) * 100
    ...
    const ICRPercent_hintAddress_213 = Number(web3.utils.fromWei(ICR_hintAddress_213, 'give')) * 100
    ...
    assert.isBelow(ICR_Difference_213, sqrtLength)
     ...
     const CRPercent_201 = Number(web3.utils.fromWei(CR_201, 'give')) * 100
    ...
    //  const hintAddress_201 = await functionCaller.troveManager_getApproxHint(CR_201, sqrtLength * 10)
     ...
     const ICRPercent_hintAddress_201 = Number(web3.utils.fromWei(ICR_hintAddress_201, 'give')) * 100
  ...
  //     const ICRPercent_hintAddress = Number(web3.utils.fromWei(ICR_hintAddress, 'give')) * 100
  ...
  it("getApproxHint(): returns the head of the list if the CR is the max uint256 value", async () => {
    ...
    const ICRPercent_hintAddress_Max = Number(web3.utils.fromWei(ICR_hintAddress_Max, 'give')) * 100

     const firstTrove = await sortedTroves.getFirst()
     const ICR_FirstTrove = await troveManager.getNominalICR(firstTrove)
     const ICRPercent_FirstTrove = Number(web3.utils.fromWei(ICR_FirstTrove, 'give')) * 100
  ...
  it("getApproxHint(): returns the tail of the list if the CR is lower than ICR of any Trove", async () => {
    ...
    const ICRPercent_hintAddress_Min = Number(web3.utils.fromWei(ICR_hintAddress_Min, 'give')) * 100

     const lastTrove = await sortedTroves.getLast()
     const ICR_LastTrove = await troveManager.getNominalICR(lastTrove)
     const ICRPercent_LastTrove = Number(web3.utils.fromWei(ICR_LastTrove, 'give')) * 100
