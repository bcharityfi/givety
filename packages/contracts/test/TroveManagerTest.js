...
/* NOTE: Some tests involving GIVE redemption fees do not test for specific fee values.
 * Some only test that the fees are non-zero when they should occur.
 *
 * Specific GIVE gain values will depend on the final fee schedule used, and the final choices for
...
contract('TroveManager', async accounts => {
  ...
  const getOpenTroveTotalDebt = async (gusdAmount) => th.getOpenTroveTotalDebt(contracts, gusdAmount)
  ...
  const withdrawGUSD = async (params) => th.withdrawGUSD(contracts, params)
  ...
  it('liquidate(): closes a Trove that has ICR < MCR', async () => {
    ...
    // Alice increases debt to 180 GUSD, lowering her ICR to 1.11
    const A_GUSDWithdrawal = await getNetBorrowingAmount(dec(130, 18))

    const targetICR = toBN('1111111111111111111')
    await withdrawGUSD({ ICR: targetICR, extraParams: { from: alice } })
    ...
    // price drops to 1ETH:100GUSD, reducing Alice's ICR below MCR
  ...
  it("liquidate(): decreases ActivePool GIVE and GUSDDebt by correct amounts", async () => {
    // --- SETUP ---
    ...
    // check ActivePool GIVE and GUSD debt before
    const activePool_GIVE_Before = (await activePool.getGIVE()).toString()
    const activePool_RawEther_Before = (await web3.eth.getBalance(activePool.address)).toString()
    const activePool_GUSDDebt_Before = (await activePool.getGUSDDebt()).toString()

    assert.equal(activePool_GIVE_Before, A_collateral.add(B_collateral))
    assert.equal(activePool_RawEther_Before, A_collateral.add(B_collateral))
    th.assertIsApproximatelyEqual(activePool_GUSDDebt_Before, A_totalDebt.add(B_totalDebt))

    // price drops to 1ETH:100GUSD, reducing Bob's ICR below MCR
    await priceFeed.setPrice('100000000000000000000');

    // Confirm system is not in Recovery Mode
    assert.isFalse(await th.checkRecoveryMode(contracts));

    /* close Bob's Trove. Should liquidate his give and GUSD, 
    leaving Alice’s give and GUSD debt in the ActivePool. */
    await troveManager.liquidate(bob, { from: owner });

    // check ActivePool GIVE and GUSD debt 
    const activePool_GIVE_After = (await activePool.getGIVE()).toString()
    const activePool_RawEther_After = (await web3.eth.getBalance(activePool.address)).toString()
    const activePool_GUSDDebt_After = (await activePool.getGUSDDebt()).toString()

    assert.equal(activePool_GIVE_After, A_collateral)
    assert.equal(activePool_RawEther_After, A_collateral)
    th.assertIsApproximatelyEqual(activePool_GUSDDebt_After, A_totalDebt)
  })

  it("liquidate(): increases DefaultPool GIVE and GUSD debt by correct amounts", async () => {
    // --- SETUP ---
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: alice } })
    const { collateral: B_collateral, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(21, 17)), extraParams: { from: bob } })
    ...
    // check DefaultPool GIVE and GUSD debt before
    const defaultPool_GIVE_Before = (await defaultPool.getGIVE())
    const defaultPool_RawEther_Before = (await web3.eth.getBalance(defaultPool.address)).toString()
    const defaultPool_GUSDDebt_Before = (await defaultPool.getGUSDDebt()).toString()

    assert.equal(defaultPool_GIVE_Before, '0')
    assert.equal(defaultPool_RawEther_Before, '0')
    assert.equal(defaultPool_GUSDDebt_Before, '0')
    ...
    // check after
    const defaultPool_GIVE_After = (await defaultPool.getGIVE()).toString()
    const defaultPool_RawEther_After = (await web3.eth.getBalance(defaultPool.address)).toString()
    const defaultPool_GUSDDebt_After = (await defaultPool.getGUSDDebt()).toString()

    const defaultPool_GIVE = th.applyLiquidationFee(B_collateral)
    assert.equal(defaultPool_GIVE_After, defaultPool_GIVE)
    assert.equal(defaultPool_RawEther_After, defaultPool_GIVE)
    th.assertIsApproximatelyEqual(defaultPool_GUSDDebt_After, B_totalDebt)
  })
  ...
  it("liquidate(): updates the snapshots of total stakes and total collateral", async () => {
    ...
    // close Bob's Trove.  His give*0.995 and GUSD should be added to the DefaultPool.
    await troveManager.liquidate(bob, { from: owner });

    /* check snapshots after. Total stakes should be equal to the  remaining stake then the system: 
    10 give, Alice's stake.
     
    Total collateral should be equal to Alice's collateral plus her pending GIVE reward (Bob’s collaterale*0.995 give), earned
  ...
  it("liquidate(): updates the L_GIVE and L_GUSDDebt reward-per-unit-staked totals", async () => {
    // --- SETUP ---
    ...
    // price drops to 1ETH:100GUSD, reducing Carols's ICR below MCR
    await priceFeed.setPrice('100000000000000000000');
    ...
    // Carol's give*0.995 and GUSD should be added to the DefaultPool.
    const L_GIVE_AfterCarolLiquidated = await troveManager.L_GIVE()
    const L_GUSDDebt_AfterCarolLiquidated = await troveManager.L_GUSDDebt()

    const L_GIVE_expected_1 = th.applyLiquidationFee(C_collateral).mul(mv._1e18BN).div(A_collateral.add(B_collateral))
    const L_GUSDDebt_expected_1 = C_totalDebt.mul(mv._1e18BN).div(A_collateral.add(B_collateral))
    assert.isAtMost(th.getDifference(L_GIVE_AfterCarolLiquidated, L_GIVE_expected_1), 100)
    assert.isAtMost(th.getDifference(L_GUSDDebt_AfterCarolLiquidated, L_GUSDDebt_expected_1), 100)

    // Bob now withdraws GUSD, bringing his ICR to 1.11
    const { increasedTotalDebt: B_increasedTotalDebt } = await withdrawGUSD({ ICR: toBN(dec(111, 16)), extraParams: { from: bob } })

    // Confirm system is not in Recovery Mode
    assert.isFalse(await th.checkRecoveryMode(contracts));

    // price drops to 1ETH:50GUSD, reducing Bob's ICR below MCR
    ...
    /* Alice now has all the active stake. totalStakes in the system is now 10 give.
   ...
   L_GIVE = (0.995 / 20) + (10.4975*0.995  / 10) = 1.09425125 GIVE
   L_GUSDDebt = (180 / 20) + (890 / 10) = 98 GUSD */
    const L_GIVE_AfterBobLiquidated = await troveManager.L_GIVE()
    const L_GUSDDebt_AfterBobLiquidated = await troveManager.L_GUSDDebt()

    const L_GIVE_expected_2 = L_GIVE_expected_1.add(th.applyLiquidationFee(B_collateral.add(B_collateral.mul(L_GIVE_expected_1).div(mv._1e18BN))).mul(mv._1e18BN).div(A_collateral))
    const L_GUSDDebt_expected_2 = L_GUSDDebt_expected_1.add(B_totalDebt.add(B_increasedTotalDebt).add(B_collateral.mul(L_GUSDDebt_expected_1).div(mv._1e18BN)).mul(mv._1e18BN).div(A_collateral))
    assert.isAtMost(th.getDifference(L_GIVE_AfterBobLiquidated, L_GIVE_expected_2), 100)
    assert.isAtMost(th.getDifference(L_GUSDDebt_AfterBobLiquidated, L_GUSDDebt_expected_2), 100)
  })

  it("liquidate(): Liquidates undercollateralized trove if there are two troves in the system", async () => {
    await openTrove({ ICR: toBN(dec(200, 18)), extraParams: { from: bob, value: dec(100, 'give') } })

    // Alice creates a single trove with 0.7 GIVE and a debt of 70 GUSD, and provides 10 GUSD to SP
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })

    // Alice proves 10 GUSD to SP
    await stabilityPool.provideToSP(dec(10, 18), ZERO_ADDRESS, { from: alice })

    // Set GIVE:USD price to 105
  ...
  it("liquidate(): Given the same price and no other trove changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening troves", async () => {
    // Whale provides GUSD to SP
    const spDeposit = toBN(dec(100, 24))
    await openTrove({ ICR: toBN(dec(4, 18)), extraGUSDAmount: spDeposit, extraParams: { from: whale } })
  ...
  it("liquidate(): does not affect the SP deposit or GIVE gain when called on an SP depositor's address that has no trove", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const spDeposit = toBN(dec(1, 24))
    await openTrove({ ICR: toBN(dec(3, 18)), extraGUSDAmount: spDeposit, extraParams: { from: bob } })
    const { C_totalDebt, C_collateral } = await openTrove({ ICR: toBN(dec(218, 16)), extraGUSDAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    // Bob sends tokens to Dennis, who has no trove
    await gusdToken.transfer(dennis, spDeposit, { from: bob })

    //Dennis provides GUSD to SP
    ...
    assert.isFalse(await sortedTroves.contains(carol))
    // Check Dennis' SP deposit has absorbed Carol's debt, and he has received her liquidated GIVE
    const dennis_Deposit_Before = (await stabilityPool.getCompoundedGUSDDeposit(dennis)).toString()
    const dennis_GIVEGain_Before = (await stabilityPool.getDepositorGIVEGain(dennis)).toString()
    assert.isAtMost(th.getDifference(dennis_Deposit_Before, spDeposit.sub(liquidatedDebt)), 1000000)
    assert.isAtMost(th.getDifference(dennis_GIVEGain_Before, liquidatedColl), 1000)
    ...
    const dennis_Deposit_After = (await stabilityPool.getCompoundedGUSDDeposit(dennis)).toString()
    const dennis_GIVEGain_After = (await stabilityPool.getDepositorGIVEGain(dennis)).toString()
    assert.equal(dennis_Deposit_Before, dennis_Deposit_After)
    assert.equal(dennis_GIVEGain_Before, dennis_GIVEGain_After)
  })

  it("liquidate(): does not liquidate a SP depositor's trove with ICR > 110%, and does not affect their SP deposit or GIVE gain", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const spDeposit = toBN(dec(1, 24))
    await openTrove({ ICR: toBN(dec(3, 18)), extraGUSDAmount: spDeposit, extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(218, 16)), extraGUSDAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    //Bob provides GUSD to SP
    ...
    // Check Bob' SP deposit has absorbed Carol's debt, and he has received her liquidated GIVE
    ...
    assert.isAtMost(th.getDifference(bob_GIVEGain_Before, liquidatedColl), 1000)
  ...
  it("liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and GIVE gain", async () => {
    const A_spDeposit = toBN(dec(3, 24))
    const B_spDeposit = toBN(dec(1, 24))
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    await openTrove({ ICR: toBN(dec(8, 18)), extraGUSDAmount: A_spDeposit, extraParams: { from: alice } })
    const { collateral: B_collateral, totalDebt: B_debt } = await openTrove({ ICR: toBN(dec(218, 16)), extraGUSDAmount: B_spDeposit, extraParams: { from: bob } })
    const { collateral: C_collateral, totalDebt: C_debt } = await openTrove({ ICR: toBN(dec(210, 16)), extraGUSDAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
    ...
    assert.isAtMost(th.getDifference(bob_GIVEGain_Before, th.applyLiquidationFee(C_collateral)), 1000)

    // Alice provides GUSD to SP
    ...
    /* Alice's GUSD Loss = (300 / 400) * 200 = 150 GUSD
       Alice's GIVE gain = (300 / 400) * 2*0.995 = 1.4925 GIVE

       Bob's GUSDLoss = (100 / 400) * 200 = 50 GUSD
       Bob's GIVE gain = (100 / 400) * 2*0.995 = 0.4975 GIVE

     Check Bob' SP deposit has been reduced to 50 GUSD, and his GIVE gain has increased to 1.5 GIVE. */
    const alice_Deposit_After = (await stabilityPool.getCompoundedGUSDDeposit(alice)).toString()
    ...
    assert.isAtMost(th.getDifference(alice_GIVEGain_After, th.applyLiquidationFee(B_collateral).mul(A_spDeposit).div(totalDeposits)), 1000000)

    const bob_Deposit_After = await stabilityPool.getCompoundedGUSDDeposit(bob)
    const bob_GIVEGain_After = await stabilityPool.getDepositorGIVEGain(bob)
    ...
    assert.isAtMost(th.getDifference(bob_GIVEGain_After, bob_GIVEGain_Before.add(th.applyLiquidationFee(B_collateral).mul(bob_Deposit_Before).div(totalDeposits))), 1000000)
  })

  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const { gusdAmount: A_gusdAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraGUSDAmount: toBN(dec(300, 18)), extraParams: { from: alice } })
    const { gusdAmount: B_gusdAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraGUSDAmount: toBN(dec(200, 18)), extraParams: { from: bob } })
    const { gusdAmount: C_gusdAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraGUSDAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
    ...
    // Liquidate A, B and C
    const activeGUSDDebt_0 = await activePool.getGUSDDebt()
    const defaultGUSDDebt_0 = await defaultPool.getGUSDDebt()

    await troveManager.liquidate(alice)
    const activeGUSDDebt_A = await activePool.getGUSDDebt()
    const defaultGUSDDebt_A = await defaultPool.getGUSDDebt()

    await troveManager.liquidate(bob)
    const activeGUSDDebt_B = await activePool.getGUSDDebt()
    const defaultGUSDDebt_B = await defaultPool.getGUSDDebt()
    ...
    assert.equal((await gusdToken.balanceOf(alice)).toString(), A_gusdAmount)
    assert.equal((await gusdToken.balanceOf(bob)).toString(), B_gusdAmount)
    assert.equal((await gusdToken.balanceOf(carol)).toString(), C_gusdAmount)
  })

  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    await openTrove({ ICR: toBN(dec(8, 18)), extraGUSDAmount: toBN(dec(100, 18)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(221, 16)), extraGUSDAmount: toBN(dec(100, 18)), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraGUSDAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    // Defaulter opens with 60 GUSD, 0.6 GIVE
    ...
    /* Liquidate defaulter. 30 GUSD and 0.3 GIVE is distributed between A, B and C.

    A receives (30 * 2/4) = 15 GUSD, and (0.3*2/4) = 0.15 GIVE
    B receives (30 * 1/4) = 7.5 GUSD, and (0.3*1/4) = 0.075 GIVE
    C receives (30 * 1/4) = 7.5 GUSD, and (0.3*1/4) = 0.075 GIVE
    */
    await troveManager.liquidate(defaulter_1)
  ...
  it("liquidate(): when SP > 0, triggers GVTY reward event - increases the sum G", async () => {
    ...
    assert.equal(await stabilityPool.getTotalGUSDDeposits(), dec(100, 18))

    const G_Before = await stabilityPool.epochToScaleToG(0, 0)

    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)

    // Price drops to 1ETH:100GUSD, reducing defaulters to below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
    assert.isFalse(await th.checkRecoveryMode(contracts))
  ...
  it('liquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards', async () => {
    ...
    // A adds 10 GUSD to the SP, but less than C's debt
    ...
    await borrowerOperations.repayGUSD(dec(1, 18), D, D, {from: D})
    await borrowerOperations.repayGUSD(dec(1, 18), E, E, {from: E})
    ...
    const pendingGIVE_C = await troveManager.getPendingETHReward(C)
    const pendingGUSDDebt_C = await troveManager.getPendingGUSDDebtReward(C)
    const defaultPoolETH = await defaultPool.getGIVE()
    const defaultPoolGUSDDebt = await defaultPool.getGUSDDebt()
    assert.isTrue(pendingGIVE_C.lte(defaultPoolETH))
    assert.isTrue(pendingGUSDDebt_C.lte(defaultPoolGUSDDebt))
    //Check only difference is dust
    assert.isAtMost(th.getDifference(pendingGIVE_C, defaultPoolETH), 1000)
    assert.isAtMost(th.getDifference(pendingGUSDDebt_C, defaultPoolGUSDDebt), 1000)
  ...
  it('liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves', async () => {
    ...
    // Price drops to 1ETH:100GUSD, reducing Bob and Carol's ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
  ...
  it("liquidateTroves(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    // Liquidate defaulter. 30 GUSD and 0.3 GIVE is distributed uniformly between A, B and C. Each receive 10 GUSD, 0.1 GIVE
    await troveManager.liquidate(defaulter_1)
    ...
    // Whale enters system, pulling it into Normal Mode
    await openTrove({ ICR: toBN(dec(10, 18)), extraGUSDAmount: dec(1, 24), extraParams: { from: whale } })
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const D_balanceBefore = await gusdToken.balanceOf(dennis)
    const E_balanceBefore = await gusdToken.balanceOf(erin)
    const F_balanceBefore = await gusdToken.balanceOf(flyn)
    ...
    assert.equal((await gusdToken.balanceOf(dennis)).toString(), D_balanceBefore)
    assert.equal((await gusdToken.balanceOf(erin)).toString(), E_balanceBefore)
    assert.equal((await gusdToken.balanceOf(flyn)).toString(), F_balanceBefore)
  })

  it("liquidateTroves(): A liquidation sequence containing Pool offsets increases the TCR", async () => {
    // Whale provides 500 GUSD to SP
    await openTrove({ ICR: toBN(dec(100, 18)), extraGUSDAmount: toBN(dec(500, 18)), extraParams: { from: whale } })
    ...
    // Check pool has 500 GUSD
    assert.equal((await stabilityPool.getTotalGUSDDeposits()).toString(), dec(500, 18))
    ...
    assert.equal((await stabilityPool.getTotalGUSDDeposits()).toString(), '0')
  ...
  it("liquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and GIVE gain", async () => {
    // Whale provides 400 GUSD to the SP
    const whaleDeposit = toBN(dec(40000, 18))
    await openTrove({ ICR: toBN(dec(100, 18)), extraGUSDAmount: whaleDeposit, extraParams: { from: whale } })
    ...
    const { collateral: A_coll, totalDebt: A_debt } = await openTrove({ ICR: toBN(dec(2, 18)), extraGUSDAmount: A_deposit, extraParams: { from: alice } })
    const { collateral: B_coll, totalDebt: B_debt } = await openTrove({ ICR: toBN(dec(2, 18)), extraGUSDAmount: B_deposit, extraParams: { from: bob } })
    ...
    // Check 800 GUSD in Pool
    const totalDeposits = whaleDeposit.add(A_deposit).add(B_deposit)
    assert.equal((await stabilityPool.getTotalGUSDDeposits()).toString(), totalDeposits)
    ...
    Total GUSD in Pool: 800 GUSD

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 150 + 350 + 150 = 650 GUSD
    Total liquidated GIVE = 1.1 + 3.1 + 1.1 = 5.3 GIVE

    whale gusd loss: 650 * (400/800) = 325 gusd
    alice gusd loss:  650 *(100/800) = 81.25 gusd
    bob gusd loss: 650 * (300/800) = 243.75 gusd

    whale remaining deposit: (400 - 325) = 75 gusd
    alice remaining deposit: (100 - 81.25) = 18.75 gusd
    bob remaining deposit: (300 - 243.75) = 56.25 gusd
    ...
    Total remaining deposits: 150 GUSD
    Total GIVE gain: 4.975 GIVE */

    // Check remaining GUSD Deposits and GIVE gain, for whale and depositors whose troves were liquidated
    const whale_Deposit_After = await stabilityPool.getCompoundedGUSDDeposit(whale)
    const alice_Deposit_After = await stabilityPool.getCompoundedGUSDDeposit(alice)
    const bob_Deposit_After = await stabilityPool.getCompoundedGUSDDeposit(bob)

    const whale_GIVEGain = await stabilityPool.getDepositorGIVEGain(whale)
    const alice_GIVEGain = await stabilityPool.getDepositorGIVEGain(alice)
    const bob_GIVEGain = await stabilityPool.getDepositorGIVEGain(bob)
    ...
    assert.isAtMost(th.getDifference(whale_GIVEGain, th.applyLiquidationFee(liquidatedColl).mul(whaleDeposit).div(totalDeposits)), 100000)
    assert.isAtMost(th.getDifference(alice_GIVEGain, th.applyLiquidationFee(liquidatedColl).mul(A_deposit).div(totalDeposits)), 100000)
    assert.isAtMost(th.getDifference(bob_GIVEGain, th.applyLiquidationFee(liquidatedColl).mul(B_deposit).div(totalDeposits)), 100000)

    // Check total remaining deposits and GIVE gain in Stability Pool
    const total_GUSDinSP = (await stabilityPool.getTotalGUSDDeposits()).toString()
    const total_GIVEinSP = (await stabilityPool.getGIVE()).toString()

    assert.isAtMost(th.getDifference(total_GUSDinSP, totalDeposits.sub(liquidatedDebt)), 1000)
    assert.isAtMost(th.getDifference(total_GIVEinSP, th.applyLiquidationFee(liquidatedColl)), 1000)
  })

  it("liquidateTroves(): when SP > 0, triggers GVTY reward event - increases the sum G", async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } })

    // A, B, C open troves
    await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraGUSDAmount: toBN(dec(100, 18)), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraParams: { from: C } })
  ...
  it('batchLiquidateTroves(): closes every trove with ICR < MCR in the given array', async () => {
    ...
    // Price drops to 1ETH:100GUSD, reducing A, B, C ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
  ...
  it('batchLiquidateTroves(): does not liquidate troves that are not in the given array', async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: toBN(dec(500, 18)), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: toBN(dec(500, 18)), extraParams: { from: erin } })
  ...
  it("batchLiquidateTroves(): skips if trove is non-existent", async () => {
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraGUSDAmount: spDeposit, extraParams: { from: whale } })
    ...
    th.assertIsApproximatelyEqual((await stabilityPool.getTotalGUSDDeposits()).toString(), spDeposit.sub(A_debt).sub(B_debt))
  ...
  it("batchLiquidateTroves(): skips if a trove has been closed", async () => {
    ...
    await gusdToken.transfer(carol, dec(100, 18), { from: whale })
  ...
  it("batchLiquidateTroves: when SP > 0, triggers GVTY reward event - increases the sum G", async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } })
  ...
  it('getRedemptionHints(): gets the address of the first Trove and the final ICR of the last Trove involved in a redemption', async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(310, 16)), extraGUSDAmount: partialRedemptionAmount, extraParams: { from: alice } })
  ...
  it('getRedemptionHints(): returns 0 as partialRedemptionHintNICR when reaching _maxIterations', async () => {
    ...
    // Get hints for a redemption of 170 + 30 + some extra GUSD. At least 3 iterations are needed
  ...
  it('redeemCollateral(): cancels the provided GUSD with debt from Troves with the lowest ICRs and sends an equivalent amount of Give', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(310, 16)), extraGUSDAmount: dec(10, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(290, 16)), extraGUSDAmount: dec(8, 18), extraParams: { from: bob } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(250, 16)), extraGUSDAmount: dec(10, 18), extraParams: { from: carol } })
    const partialRedemptionAmount = toBN(2)
    const redemptionAmount = C_netDebt.add(B_netDebt).add(partialRedemptionAmount)
    // start Dennis with a high ICR
    await openTrove({ ICR: toBN(dec(100, 18)), extraGUSDAmount: redemptionAmount, extraParams: { from: dennis } })

    const dennis_GIVEBalance_Before = toBN(await web3.eth.getBalance(dennis))

    const dennis_GUSDBalance_Before = await gusdToken.balanceOf(dennis)
    ...
    // Find hints for redeeming 20 GUSD
    const {
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(redemptionAmount, price, 0)
    ...
    // Dennis redeems 20 GUSD
    // Don't pay for gas, as it makes it easier to calculate the received Give
    ...
    /* check that Dennis' redeemed 20 GUSD has been cancelled with debt from Bobs's Trove (8) and Carol's Trove (10).
    The remaining lot (2) is sent to Alice's Trove, who had the best ICR.
    It leaves her with (3) GUSD debt + 50 for gas compensation. */
    th.assertIsApproximatelyEqual(alice_debt_After, A_totalDebt.sub(partialRedemptionAmount))
    assert.equal(bob_debt_After, '0')
    assert.equal(carol_debt_After, '0')

    const dennis_GIVEBalance_After = toBN(await web3.eth.getBalance(dennis))
    const receivedETH = dennis_GIVEBalance_After.sub(dennis_GIVEBalance_Before)

    const expectedTotalETHDrawn = redemptionAmount.div(toBN(200)) // convert redemptionAmount GUSD to GIVE, at GIVE:USD price 200
    ...
    const dennis_GUSDBalance_After = (await gusdToken.balanceOf(dennis)).toString()
    assert.equal(dennis_GUSDBalance_After, dennis_GUSDBalance_Before.sub(redemptionAmount))
  })
  ...
  it('redeemCollateral(): ends the redemption sequence when the token redemption request has been filled', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraGUSDAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraGUSDAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraGUSDAmount: dec(20, 18), extraParams: { from: carol } })
    const redemptionAmount = A_debt.add(B_debt).add(C_debt)
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraGUSDAmount: dec(10, 18), extraParams: { from: dennis } })
    const { totalDebt: E_totalDebt, collateral: E_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraGUSDAmount: dec(10, 18), extraParams: { from: erin } })

    // --- TEST --- 

    // open trove from redeemer.  Redeemer has highest ICR (100ETH, 100 GUSD), 20000%
    const { gusdAmount: F_gusdAmount } = await openTrove({ ICR: toBN(dec(200, 18)), extraGUSDAmount: redemptionAmount.mul(toBN(2)), extraParams: { from: flyn } })
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-60) = 40 GUSD
    const flynBalance = await gusdToken.balanceOf(flyn)
    th.assertIsApproximatelyEqual(flynBalance, F_gusdAmount.sub(redemptionAmount))
  ...
  it('redeemCollateral(): ends the redemption sequence when max iterations have been reached', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraGUSDAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraGUSDAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(286, 16)), extraGUSDAmount: dec(20, 18), extraParams: { from: carol } })
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-40) = 60 GUSD
    const flynBalance = (await gusdToken.balanceOf(flyn)).toString()
    th.assertIsApproximatelyEqual(flynBalance, F_gusdAmount.sub(redemptionAmount))
  ...
  it("redeemCollateral(): performs partial redemption if resultant debt is > minimum net debt", async () => {
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveGUSDAmount(dec(10000, 18)), A, A, { from: A, value: dec(1000, 'give') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveGUSDAmount(dec(20000, 18)), B, B, { from: B, value: dec(1000, 'give') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveGUSDAmount(dec(30000, 18)), C, C, { from: C, value: dec(1000, 'give') })

    // A and C send all their tokens to B
    await gusdToken.transfer(B, await gusdToken.balanceOf(A), {from: A})
    await gusdToken.transfer(B, await gusdToken.balanceOf(C), {from: C})
    ...
    // GUSD redemption is 55000 US
    const GUSDRedemption = dec(55000, 18)
    const tx1 = await th.redeemCollateralAndGetTxObject(B, contracts, GUSDRedemption, th._100pct)
  ...
  it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveGUSDAmount(dec(6000, 18)), A, A, { from: A, value: dec(1000, 'give') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveGUSDAmount(dec(20000, 18)), B, B, { from: B, value: dec(1000, 'give') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveGUSDAmount(dec(30000, 18)), C, C, { from: C, value: dec(1000, 'give') })
  ...
  it('redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(363, 16)), extraGUSDAmount: dec(5, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(344, 16)), extraGUSDAmount: dec(8, 18), extraParams: { from: bob } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(333, 16)), extraGUSDAmount: dec(10, 18), extraParams: { from: carol } })
    ...
    // Oops, another transaction gets in the way
    {
      ...
      // Alice redeems 1 GUSD from Carol's Trove
    ...
    // Dennis tries to redeem 20 GUSD
    const redemptionTx = await troveManager.redeemCollateral(
      redemptionAmount,
      firstRedemptionHint,
      upperPartialRedemptionHint,
    ...
    // Since Alice already redeemed 1 GUSD from Carol's Trove, Dennis was  able to redeem:
    //  - 9 GUSD from Carol's
    //  - 8 GUSD from Bob's
    // for a total of 17 GUSD.

    // Dennis calculated his hint for redeeming 2 GUSD from Alice's Trove, but after Alice's transaction
    // got in the way, he would have needed to redeem 3 GUSD to fully complete his redemption of 20 GUSD.
    // This would have required a different hint, therefore he ended up with a partial redemption.

    const dennis_GIVEBalance_After = toBN(await web3.eth.getBalance(dennis))
    const receivedETH = dennis_GIVEBalance_After.sub(dennis_GIVEBalance_Before)

    // Expect only 17 worth of GIVE drawn
    const expectedTotalETHDrawn = fullfilledRedemptionAmount.sub(frontRunRedepmtion).div(toBN(200)) // redempted GUSD converted to GIVE, at GIVE:USD price 200
    const expectedReceivedETH = expectedTotalETHDrawn.sub(ETHFee)

    th.assertIsApproximatelyEqual(expectedReceivedETH, receivedETH)

    const dennis_GUSDBalance_After = (await gusdToken.balanceOf(dennis)).toString()
    th.assertIsApproximatelyEqual(dennis_GUSDBalance_After, dennis_GUSDBalance_Before.sub(fullfilledRedemptionAmount.sub(frontRunRedepmtion)))
  })
  ...
  it.skip("redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool", async () => {
    ...
    const amount = await getOpenTroveGUSDAmount(dec(110, 18))
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(133, 16)), extraGUSDAmount: amount, extraParams: { from: bob } })

    await gusdToken.transfer(carol, amount, { from: bob })
    ...
    const carol_GIVEBalance_Before = toBN(await web3.eth.getBalance(carol))
    ...
    const carol_GIVEBalance_After = toBN(await web3.eth.getBalance(carol))

    const expectedTotalETHDrawn = toBN(amount).div(toBN(100)) // convert 100 GUSD to GIVE at GIVE:USD price of 100
    const expectedReceivedETH = expectedTotalETHDrawn.sub(ETHFee)

    const receivedETH = carol_GIVEBalance_After.sub(carol_GIVEBalance_Before)
    assert.isTrue(expectedReceivedETH.eq(receivedETH))

    const carol_GUSDBalance_After = (await gusdToken.balanceOf(carol)).toString()
    assert.equal(carol_GUSDBalance_After, '0')
  })
  ...
  it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
    ...
    const { gusdAmount: B_gusdAmount, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(133, 16)), extraGUSDAmount: A_debt, extraParams: { from: bob } })

    await gusdToken.transfer(carol, B_gusdAmount, { from: bob })
  ...
  it("redeemCollateral(): finds the last Trove with ICR == 110% even if there is more than one", async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: amount1, extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: amount1, extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: amount1, extraParams: { from: carol } })
    const redemptionAmount = C_totalDebt.add(B_totalDebt).add(A_totalDebt)
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(195, 16)), extraGUSDAmount: redemptionAmount, extraParams: { from: dennis } })
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraGUSDAmount: dec(10, 18), extraParams: { from: whale } })
  ...
  it("redeemCollateral(): reverts when argument _amount is 0", async () => {
    ...
    // Alice opens trove and transfers 500GUSD to Erin, the would-be redeemer
    await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: dec(500, 18), extraParams: { from: alice } })
    await gusdToken.transfer(erin, dec(500, 18), { from: alice })
  ...
  it("redeemCollateral(): reverts if max fee > 100%", async () => {
    await openTrove({ ICR: toBN(dec(400, 16)), extraGUSDAmount: dec(10, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraGUSDAmount: dec(20, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraGUSDAmount: dec(30, 18), extraParams: { from: C } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraGUSDAmount: dec(40, 18), extraParams: { from: D } })
  ...
  it("redeemCollateral(): reverts if fee exceeds max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraGUSDAmount: dec(80, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraGUSDAmount: dec(90, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: C } })
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt)

    // Check total GUSD supply
    ...
    // GUSD redemption is 27 USD: a redemption that incurs a fee of 27/(270 * 2) = 5%
    const attemptedGUSDRedemption = expectedTotalSupply.div(toBN(10))

    // Max fee is <5%
    const lessThan5pct = '49999999999999999'
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedGUSDRedemption, lessThan5pct), "Fee exceeded provided maximum")
  
    await troveManager.setBaseRate(0)  // artificially zero the baseRate
    
    // Max fee is 1%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedGUSDRedemption, dec(1, 16)), "Fee exceeded provided maximum")
  
    await troveManager.setBaseRate(0)

     // Max fee is 3.754%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedGUSDRedemption, dec(3754, 13)), "Fee exceeded provided maximum")
  
    await troveManager.setBaseRate(0)

    // Max fee is 0.5%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedGUSDRedemption, dec(5, 15)), "Fee exceeded provided maximum")
  })

  it("redeemCollateral(): succeeds if fee is less than max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraGUSDAmount: dec(9500, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(395, 16)), extraGUSDAmount: dec(9000, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(390, 16)), extraGUSDAmount: dec(10000, 18), extraParams: { from: C } })
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt)
    ...
    // GUSD redemption fee with 10% of the supply will be 0.5% + 1/(10*2)
    const attemptedGUSDRedemption = expectedTotalSupply.div(toBN(10))

    // Attempt with maxFee > 5.5%
    const price = await priceFeed.getPrice()
    const ETHDrawn = attemptedGUSDRedemption.mul(mv._1e18BN).div(price)
    const slightlyMoreThanFee = (await troveManager.getRedemptionFeeWithDecay(ETHDrawn))
    const tx1 = await th.redeemCollateralAndGetTxObject(A, contracts, attemptedGUSDRedemption, slightlyMoreThanFee)
    ...
    const tx2 = await th.redeemCollateralAndGetTxObject(C, contracts, attemptedGUSDRedemption, exactSameFee)
    assert.isTrue(tx2.receipt.status)

    await troveManager.setBaseRate(0)

     // Max fee is 10%
    const tx3 = await th.redeemCollateralAndGetTxObject(B, contracts, attemptedGUSDRedemption, dec(1, 17))
    assert.isTrue(tx3.receipt.status)

    await troveManager.setBaseRate(0)

    // Max fee is 37.659%
    const tx4 = await th.redeemCollateralAndGetTxObject(A, contracts, attemptedGUSDRedemption, dec(37659, 13))
    assert.isTrue(tx4.receipt.status)

    await troveManager.setBaseRate(0)

    // Max fee is 100%
    const tx5 = await th.redeemCollateralAndGetTxObject(C, contracts, attemptedGUSDRedemption, dec(1, 18))
    assert.isTrue(tx5.receipt.status)
  })

  it("redeemCollateral(): doesn't affect the Stability Pool deposits or GIVE gain of redeemed-from troves", async () => {
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })

    // B, C, D, F open trove
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(195, 16)), extraGUSDAmount: dec(200, 18), extraParams: { from: carol } })
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraGUSDAmount: dec(400, 18), extraParams: { from: dennis } })
    const { totalDebt: F_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: flyn } })

    const redemptionAmount = B_totalDebt.add(C_totalDebt).add(D_totalDebt).add(F_totalDebt)
    // Alice opens trove and transfers GUSD to Erin, the would-be redeemer
    await openTrove({ ICR: toBN(dec(300, 16)), extraGUSDAmount: redemptionAmount, extraParams: { from: alice } })
    await gusdToken.transfer(erin, redemptionAmount, { from: alice })
    ...
    const bob_SPDeposit_before = (await stabilityPool.getCompoundedGUSDDeposit(bob)).toString()
    const carol_SPDeposit_before = (await stabilityPool.getCompoundedGUSDDeposit(carol)).toString()
    const dennis_SPDeposit_before = (await stabilityPool.getCompoundedGUSDDeposit(dennis)).toString()

    const bob_GIVEGain_before = (await stabilityPool.getDepositorGIVEGain(bob)).toString()
    const carol_GIVEGain_before = (await stabilityPool.getDepositorGIVEGain(carol)).toString()
    const dennis_GIVEGain_before = (await stabilityPool.getDepositorGIVEGain(dennis)).toString()

    // Check the remaining GUSD and GIVE in Stability Pool after liquidation is non-zero
    const GUSDinSP = await stabilityPool.getTotalGUSDDeposits()
    const ETHinSP = await stabilityPool.getGIVE()
    assert.isTrue(GUSDinSP.gte(mv._zeroBN))
    ...
    const bob_SPDeposit_after = (await stabilityPool.getCompoundedGUSDDeposit(bob)).toString()
    const carol_SPDeposit_after = (await stabilityPool.getCompoundedGUSDDeposit(carol)).toString()
    const dennis_SPDeposit_after = (await stabilityPool.getCompoundedGUSDDeposit(dennis)).toString()

    const bob_GIVEGain_after = (await stabilityPool.getDepositorGIVEGain(bob)).toString()
    const carol_GIVEGain_after = (await stabilityPool.getDepositorGIVEGain(carol)).toString()
    const dennis_GIVEGain_after = (await stabilityPool.getDepositorGIVEGain(dennis)).toString()

    // Check B, C, D Stability Pool deposits and GIVE gain have not been affected by redemptions from their troves
  ...
  it("redeemCollateral(): caller can redeem their entire GUSDToken balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })

    // Alice opens trove and transfers 400 GUSD to Erin, the would-be redeemer
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraGUSDAmount: dec(400, 18), extraParams: { from: alice } })
    await gusdToken.transfer(erin, dec(400, 18), { from: alice })

    // Check Erin's balance before
    const erin_balance_before = await gusdToken.balanceOf(erin)
    assert.equal(erin_balance_before, dec(400, 18))

    // B, C, D open trove
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraGUSDAmount: dec(590, 18), extraParams: { from: bob } })
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraGUSDAmount: dec(1990, 18), extraParams: { from: carol } })
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(500, 16)), extraGUSDAmount: dec(1990, 18), extraParams: { from: dennis } })
    ...
    const activePool_coll_before = await activePool.getGIVE()
    ...
    // Erin attempts to redeem 400 GUSD
    const {
      firstRedemptionHint,
      partialRedemptionHintNICR
    ...
    // Check activePool debt reduced by  400 GUSD
    const activePool_debt_after = await activePool.getGUSDDebt()
    assert.equal(activePool_debt_before.sub(activePool_debt_after), dec(400, 18))

    /* Check ActivePool coll reduced by $400 worth of Give: at GIVE:USD price of $200, this should be 2 GIVE.

    therefore remaining ActivePool GIVE should be 198 */
    const activePool_coll_after = await activePool.getGIVE()
    ...
    const erin_balance_after = (await gusdToken.balanceOf(erin)).toString()
    assert.equal(erin_balance_after, '0')
  })

  it("redeemCollateral(): reverts when requested redemption amount exceeds caller's GUSD token balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    ...
    // Get active debt and coll before redemption
    const activePool_debt_before = await activePool.getGUSDDebt()
    const activePool_coll_before = (await activePool.getGIVE()).toString()
    ...
    // Erin tries to redeem 1000 GUSD
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
      ...
      assert.include(error.message, "Requested redemption amount must be <= user's GUSD token balance")
    }

    // Erin tries to redeem 401 GUSD
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
    ...
    // Erin tries to redeem 239482309 GUSD
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
    ...
    // Erin tries to redeem 2^256 - 1 GUSD
  ...
  it("redeemCollateral(): value of issued GIVE == face value of redeemed GUSD (assuming 1 GUSD has value of $1)", async () => {
    const { collateral: W_coll } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })

    // Alice opens trove and transfers 1000 GUSD each to Erin, Flyn, Graham
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraGUSDAmount: dec(4990, 18), extraParams: { from: alice } })
    await gusdToken.transfer(erin, dec(1000, 18), { from: alice })
    await gusdToken.transfer(flyn, dec(1000, 18), { from: alice })
    await gusdToken.transfer(graham, dec(1000, 18), { from: alice })

    // B, C, D open trove
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraGUSDAmount: dec(1590, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(600, 16)), extraGUSDAmount: dec(1090, 18), extraParams: { from: carol } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(800, 16)), extraGUSDAmount: dec(1090, 18), extraParams: { from: dennis } })

    const totalColl = W_coll.add(A_coll).add(B_coll).add(C_coll).add(D_coll)

    const price = await priceFeed.getPrice()

    const _120_GUSD = '120000000000000000000'
    const _373_GUSD = '373000000000000000000'
    const _950_GUSD = '950000000000000000000'

    // Check Give in activePool
    const activeGIVE_0 = await activePool.getGIVE()
    assert.equal(activeGIVE_0, totalColl.toString());

    let firstRedemptionHint
    let partialRedemptionHintNICR


    // Erin redeems 120 GUSD
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_120_GUSD, price, 0))
    ...
    /* 120 GUSD redeemed.  Expect $120 worth of GIVE removed. At GIVE:USD price of $200, 
    GIVE removed = (120/200) = 0.6 GIVE
    Total active GIVE = 280 - 0.6 = 279.4 GIVE */

    const activeGIVE_1 = await activePool.getGIVE()
    assert.equal(activeGIVE_1.toString(), activeGIVE_0.sub(toBN(_120_GUSD).mul(mv._1e18BN).div(price)));

    // Flyn redeems 373 GUSD
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_373_GUSD, price, 0))
    ...
    /* 373 GUSD redeemed.  Expect $373 worth of GIVE removed. At GIVE:USD price of $200, 
    GIVE removed = (373/200) = 1.865 GIVE
    Total active GIVE = 279.4 - 1.865 = 277.535 GIVE */
    const activeGIVE_2 = await activePool.getGIVE()
    assert.equal(activeGIVE_2.toString(), activeGIVE_1.sub(toBN(_373_GUSD).mul(mv._1e18BN).div(price)));

    // Graham redeems 950 GUSD
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_950_GUSD, price, 0))
    ...
    /* 950 GUSD redeemed.  Expect $950 worth of GIVE removed. At GIVE:USD price of $200, 
    GIVE removed = (950/200) = 4.75 GIVE
    Total active GIVE = 277.535 - 4.75 = 272.785 GIVE */
    const activeGIVE_3 = (await activePool.getGIVE()).toString()
    assert.equal(activeGIVE_3.toString(), activeGIVE_2.sub(toBN(_950_GUSD).mul(mv._1e18BN).div(price)));
  })
  ...
  it("redeemCollateral(): reverts if there is zero outstanding system debt", async () => {
    // --- SETUP --- illegally mint GUSD to Bob
    await gusdToken.unprotectedMint(bob, dec(100, 18))

    assert.equal((await gusdToken.balanceOf(bob)), dec(100, 18))
    ...
    // Bob tries to redeem his illegally obtained GUSD
    try {
      const redemptionTx = await troveManager.redeemCollateral(
        dec(100, 18),
        firstRedemptionHint,
  ...
  it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
    // --- SETUP --- illegally mint GUSD to Bob
    await gusdToken.unprotectedMint(bob, '101000000000000000000')

    assert.equal((await gusdToken.balanceOf(bob)), '101000000000000000000')

    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(1000, 16)), extraGUSDAmount: dec(40, 18), extraParams: { from: carol } })
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(1000, 16)), extraGUSDAmount: dec(40, 18), extraParams: { from: dennis } })

    const totalDebt = C_totalDebt.add(D_totalDebt)
    th.assertIsApproximatelyEqual((await activePool.getGUSDDebt()).toString(), totalDebt)
    ...
    // Bob attempts to redeem his ill-gotten 101 GUSD, from a system that has 100 GUSD outstanding debt
    try {
      const redemptionTx = await troveManager.redeemCollateral(
  ...
  it("redeemCollateral(): a redemption made when base rate is zero increases the base rate", async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(190, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(180, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: C } })
    ...
    const A_balanceBefore = await gusdToken.balanceOf(A)

    await th.redeemCollateral(A, contracts, dec(10, 18))

    // Check A's balance has decreased by 10 GUSD
    assert.equal(await gusdToken.balanceOf(A), A_balanceBefore.sub(toBN(dec(10, 18))).toString())
  ...
  it("redeemCollateral(): a redemption made when base rate is non-zero increases the base rate, for negligible time passed", async () => {
    ...
    const A_balanceBefore = await gusdToken.balanceOf(A)
    const B_balanceBefore = await gusdToken.balanceOf(B)
    ...
    // Check B's balance has decreased by 10 GUSD
    assert.equal(await gusdToken.balanceOf(B), B_balanceBefore.sub(toBN(dec(10, 18))).toString())
  ...
  it("redeemCollateral(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation [ @skip-on-coverage ]", async () => {
    ...
    // Check A's balance has decreased by 10 GUSD
    assert.equal(A_balanceBefore.sub(await gusdToken.balanceOf(A)), dec(10, 18))
  ...
  it("redeemCollateral(): a redemption made at zero base rate send a non-zero ETHFee to GVTY staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 GVTY
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await gvtyToken.approve(gvtyStaking.address, dec(1, 18), { from: multisig })
    await gvtyStaking.stake(dec(1, 18), { from: multisig })
    ...
    // Check GVTY Staking contract balance before is zero
    const gvtyStakingBalance_Before = await web3.eth.getBalance(gvtyStaking.address)
    assert.equal(gvtyStakingBalance_Before, '0')
    ...
    // Check GVTY Staking contract balance after is non-zero
    const gvtyStakingBalance_After = toBN(await web3.eth.getBalance(gvtyStaking.address))
    assert.isTrue(gvtyStakingBalance_After.gt(toBN('0')))
  })

  it("redeemCollateral(): a redemption made at zero base increases the GIVE-fees-per-GVTY-staked in GVTY Staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 GVTY
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await gvtyToken.approve(gvtyStaking.address, dec(1, 18), { from: multisig })
    await gvtyStaking.stake(dec(1, 18), { from: multisig })
    ...
    // Check GVTY Staking GIVE-fees-per-GVTY-staked before is zero
    ...
    // Check GVTY Staking GIVE-fees-per-GVTY-staked after is non-zero
    const F_GIVE_After = await gvtyStaking.F_GIVE()
    assert.isTrue(F_GIVE_After.gt('0'))
  })

  it("redeemCollateral(): a redemption made at a non-zero base rate send a non-zero ETHFee to GVTY staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 GVTY
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await gvtyToken.approve(gvtyStaking.address, dec(1, 18), { from: multisig })
    await gvtyStaking.stake(dec(1, 18), { from: multisig })
    ...
    const gvtyStakingBalance_Before = toBN(await web3.eth.getBalance(gvtyStaking.address))
    ...
    // check GVTY Staking balance has increased
    assert.isTrue(gvtyStakingBalance_After.gt(gvtyStakingBalance_Before))
  })

  it("redeemCollateral(): a redemption made at a non-zero base rate increases GIVE-per-GVTY-staked in the staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 GVTY
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await gvtyToken.approve(gvtyStaking.address, dec(1, 18), { from: multisig })
    await gvtyStaking.stake(dec(1, 18), { from: multisig })
    ...
    // check GVTY Staking balance has increased
    assert.isTrue(F_GIVE_After.gt(F_GIVE_Before))
  })

  it("redeemCollateral(): a redemption sends the GIVE remainder (ETHDrawn - ETHFee) to the redeemer", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 GVTY
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await gvtyToken.approve(gvtyStaking.address, dec(1, 18), { from: multisig })
    await gvtyStaking.stake(dec(1, 18), { from: multisig })
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: C } })
    ...
    // Check total GUSD supply
    const activeGUSD = await activePool.getGUSDDebt()
    const defaultGUSD = await defaultPool.getGUSDDebt()

    const totalGUSDSupply = activeGUSD.add(defaultGUSD)
    th.assertIsApproximatelyEqual(totalGUSDSupply, totalDebt)
    ...
    /*
    At GIVE:USD price of 200:
    ETHDrawn = (9 / 200) = 0.045 GIVE
    ETHfee = (0.005 + (1/2) *( 9/260)) * ETHDrawn = 0.00100384615385 GIVE
    ETHRemainder = 0.045 - 0.001003... = 0.0439961538462
    */

    const A_balanceAfter = toBN(await web3.eth.getBalance(A))

    // check A's GIVE balance has increased by 0.045 GIVE 
    const price = await priceFeed.getPrice()
  ...
  it("redeemCollateral(): a full redemption (leaving trove with 0 debt), closes the trove", async () => {
    ...
    const { netDebt: W_netDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraGUSDAmount: dec(10000, 18), extraParams: { from: whale } })

    const { netDebt: A_netDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: A } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: C } })
    const { netDebt: D_netDebt } = await openTrove({ ICR: toBN(dec(280, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: D } })
    ...
    // whale redeems 360 GUSD.  Expect this to fully redeem A, B, C, and partially redeem D.
  ...
  const redeemCollateral3Full1Partial = async () => {
    ...
    const { netDebt: A_netDebt, collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: A } })
    const { netDebt: B_netDebt, collateral: B_coll } = await openTrove({ ICR: toBN(dec(190, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt, collateral: C_coll } = await openTrove({ ICR: toBN(dec(180, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: C } })
    const { netDebt: D_netDebt } = await openTrove({ ICR: toBN(dec(280, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: D } })
    const redemptionAmount = A_netDebt.add(B_netDebt).add(C_netDebt).add(toBN(dec(10, 18)))
    ...
    // whale redeems GUSD.  Expect this to fully redeem A, B, C, and partially redeem D.
    await th.redeemCollateral(whale, contracts, redemptionAmount)
    ...
    /*
    At GIVE:USD price of 200, with full redemptions from A, B, C:

    ETHDrawn from A = 100/200 = 0.5 GIVE --> Surplus = (1-0.5) = 0.5
    ETHDrawn from B = 120/200 = 0.6 GIVE --> Surplus = (1-0.6) = 0.4
    ETHDrawn from C = 130/200 = 0.65 GIVE --> Surplus = (2-0.65) = 1.35
    */
  ...
  it("redeemCollateral(): emits correct debt and coll values in each redeemed trove's TroveUpdated event", async () => {
    ...
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: C } })
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(280, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: D } })
    ...
    // whale redeems GUSD.  Expect this to fully redeem A, B, C, and partially redeem 15 GUSD from D.
    ...
    /* Expect D to have lost 15 debt and (at GIVE price of 200) 15/200 = 0.075 GIVE. 
    So, expect remaining debt = (85 - 15) = 70, and remaining GIVE = 1 - 15/200 = 0.925 remaining. */
    const price = await priceFeed.getPrice()
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's GIVE surplus (collateral - GIVE drawn) available for the trove owner to claim", async () => {
    const {
      A_netDebt, A_coll,
      B_netDebt, B_coll,
      C_netDebt, C_coll,
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's GIVE surplus (collateral - GIVE drawn) available for the trove owner after re-opening trove", async () => {
    const {
    ...
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: A } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(190, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: B } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(180, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: C } })
  ...
  it('redeemCollateral(): reverts if fee eats up all returned collateral', async () => {
    // --- SETUP ---
    const { gusdAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: dec(1, 24), extraParams: { from: alice } })
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const {
        firstRedemptionHint,
        partialRedemptionHintNICR
      } = await hintHelpers.getRedemptionHints(gusdAmount, price, 0)
      ...
      await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: bob } })
      await borrowerOperations.adjustTrove(th._100pct, 0, gusdAmount, true, alice, alice, { from: alice, value: gusdAmount.mul(mv._1e18BN).div(price) })
    }
  ...
  it("getPendingGUSDDebtReward(): Returns 0 if there is no pending GUSDDebt reward", async () => {
    // Make some troves
    const { totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraGUSDAmount: dec(100, 18), extraParams: { from: defaulter_1 } })

    await openTrove({ ICR: toBN(dec(3, 18)), extraGUSDAmount: dec(20, 18), extraParams: { from: carol } })

    await openTrove({ ICR: toBN(dec(20, 18)), extraGUSDAmount: totalDebt, extraParams: { from: whale } })
    ...
    const current_L_GUSDDebt = await troveManager.L_GUSDDebt()
    assert.equal(current_L_GUSDDebt, 0)

    const carolSnapshot_L_GUSDDebt = (await troveManager.rewardSnapshots(carol))[1]
    assert.equal(carolSnapshot_L_GUSDDebt, 0)

    const carol_PendingGUSDDebtReward = await troveManager.getPendingGUSDDebtReward(carol)
    assert.equal(carol_PendingGUSDDebtReward, 0)
  })

  it("getPendingETHReward(): Returns 0 if there is no pending GIVE reward", async () => {
    // make some troves
    const { totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraGUSDAmount: dec(100, 18), extraParams: { from: defaulter_1 } })
    ...
    // Confirm there are no pending rewards from liquidation
    const current_L_GIVE = await troveManager.L_GIVE()
    assert.equal(current_L_GIVE, 0)

    const carolSnapshot_L_GIVE = (await troveManager.rewardSnapshots(carol))[0]
    assert.equal(carolSnapshot_L_GIVE, 0)
  ...
  it("computeICR(): Returns 2^256-1 for GIVE:USD = 100, coll = 1 GIVE, debt = 100 GUSD", async () => {
    const price = dec(100, 18)
    const coll = dec(1, 'give')
    const debt = dec(100, 18)
  ...
  it("computeICR(): returns correct ICR for GIVE:USD = 100, coll = 200 GIVE, debt = 30 GUSD", async () => {
    const price = dec(100, 18)
    const coll = dec(200, 'give')
  ...
  it("computeICR(): returns correct ICR for GIVE:USD = 250, coll = 1350 GIVE, debt = 127 GUSD", async () => {
  ...
  it("computeICR(): returns correct ICR for GIVE:USD = 100, coll = 1 GIVE, debt = 54321 GUSD", async () => {
    const price = dec(100, 18)
    const coll = dec(1, 'give')
  ...
  it("getTroveStatus(): Returns status", async () => {
    ...
    await openTrove({ ICR: toBN(dec(150, 16)), extraGUSDAmount: B_totalDebt, extraParams: { from: A } })

    // to be able to repay:
    await gusdToken.transfer(B, B_totalDebt, { from: A })
