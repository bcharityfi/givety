...
contract('TroveManager - in Recovery Mode', async accounts => {
  const _1_Ether = web3.utils.toWei('1', 'give')
  const _2_Ether = web3.utils.toWei('2', 'give')
  const _3_Ether = web3.utils.toWei('3', 'give')
  const _3pt5_Ether = web3.utils.toWei('3.5', 'give')
  const _6_Ether = web3.utils.toWei('6', 'give')
  const _10_Ether = web3.utils.toWei('10', 'give')
  const _20_Ether = web3.utils.toWei('20', 'give')
  const _21_Ether = web3.utils.toWei('21', 'give')
  const _22_Ether = web3.utils.toWei('22', 'give')
  const _24_Ether = web3.utils.toWei('24', 'give')
  const _25_Ether = web3.utils.toWei('25', 'give')
  const _30_Ether = web3.utils.toWei('30', 'give')
  ...
  it("checkRecoveryMode(): Returns true if TCR falls below CCR", async () => {
    ...
    // price drops to 1ETH:150GUSD, reducing TCR below 150%.  setPrice() calls checkTCRAndSetRecoveryMode() internally.
  ...
  it("liquidate(), with ICR < 100%: removes stake and updates totalStakes", async () => {
    ...
    //  Alice and Bob withdraw such that the TCR is ~150%
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraGUSDAmount: dec(2000, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with ICR < 100%: updates system snapshots correctly", async () => {
    ...
    // --- TEST ---
    // price drops to 1ETH:100GUSD, reducing TCR below 150%, and all Troves below 100% ICR
    await priceFeed.setPrice('100000000000000000000')
  ...
  it("liquidate(), with ICR < 100%: closes the Trove and removes it from the Trove array", async () => {
    ...
    //  Alice and Bob withdraw such that the TCR is ~150%
    await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(150, 16)), extraGUSDAmount: dec(2000, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with ICR < 100%: only redistributes to active Troves - no offset to Stability Pool", async () => {
    ...
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraGUSDAmount: spDeposit, extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: dennis } })
  ...
  it("liquidate(), with 100 < ICR < 110%: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    //  Bob withdraws up to 2000 GUSD of debt, bringing his ICR to 210%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(210, 16)), extraGUSDAmount: dec(2000, 18), extraParams: { from: bob } })
    ...
    // --- TEST ---
    // price drops to 1ETH:100GUSD, reducing TCR to 117%
    await priceFeed.setPrice('100000000000000000000')
  ...
  it("liquidate(), with 100% < ICR < 110%: updates system snapshots correctly", async () => {
    ...
    //  Bob withdraws up to 20000 GUSD of debt, bringing his ICR to 210%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(210, 16)), extraGUSDAmount: dec(20000, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: dennis } })
    ...
    /*
    Prior to Dennis liquidation, total stakes and total collateral were each 27 give. 
  
    Check snapshots. Dennis' liquidated collateral is distributed and remains in the system. His 
    stake is removed, leaving 24+3*0.995 give total collateral, and 24 give total stakes. */
    ...
    /* After Bob's liquidation, Bob's stake (21 give) should be removed from total stakes, 
  ...
  it("liquidate(), with 100% < ICR < 110%: offsets as much debt as possible with the Stability Pool, then redistributes the remainder coll and debt", async () => {
    ...
    // Alice deposits 390GUSD to the Stability Pool
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: alice })
    ...
    // check pool GUSD before liquidation
    const stabilityPoolGUSD_Before = (await stabilityPool.getTotalGUSDDeposits()).toString()
    assert.equal(stabilityPoolGUSD_Before, '390000000000000000000')
    ...
    /* Now, liquidate Bob. Liquidated coll is 21 give, and liquidated debt is 2000 GUSD.
    
    With 390 GUSD in the StabilityPool, 390 GUSD should be offset with the pool, leaving 0 in the pool.
  
    Stability Pool rewards for alice should be:
    GUSDLoss: 390GUSD
    GIVEGain: (390 / 2000) * 21*0.995 = 4.074525 give

    After offsetting 390 GUSD and 4.074525 give, the remainders - 1610 GUSD and 16.820475 give - should be redistributed to all active Troves.
   */
    // Liquidate Bob
    await troveManager.liquidate(bob, { from: owner })

    const aliceDeposit = await stabilityPool.getCompoundedGUSDDeposit(alice)
    const aliceGIVEGain = await stabilityPool.getDepositorGIVEGain(alice)
    const aliceExpectedGIVEGain = spDeposit.mul(th.applyLiquidationFee(B_coll)).div(B_totalDebt)

    assert.equal(aliceDeposit.toString(), 0)
    assert.equal(aliceGIVEGain.toString(), aliceExpectedGIVEGain)

    /* Now, check redistribution to active Troves. Remainders of 1610 GUSD and 16.82 give are distributed.
    
    Now, only Alice and Dennis have a stake in the system - 3 give each, thus total stakes is 6 give.
  
    Rewards-per-unit-staked from the redistribution should be:
  
    L_GUSDDebt = 1610 / 6 = 268.333 GUSD
    L_GIVE = 16.820475 /6 =  2.8034125 give
    */
    const L_GUSDDebt = (await troveManager.L_GUSDDebt()).toString()
    const L_GIVE = (await troveManager.L_GIVE()).toString()

    assert.isAtMost(th.getDifference(L_GUSDDebt, B_totalDebt.sub(spDeposit).mul(mv._1e18BN).div(A_coll.add(D_coll))), 100)
    assert.isAtMost(th.getDifference(L_GIVE, th.applyLiquidationFee(B_coll.sub(B_coll.mul(spDeposit).div(B_totalDebt)).mul(mv._1e18BN).div(A_coll.add(D_coll)))), 100)
  })
  ...
  it("liquidate(), with ICR > 110%, trove has lowest ICR, and StabilityPool is empty: does nothing", async () => {
    ...
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraGUSDAmount: dec(2000, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraGUSDAmount: dec(2000, 18), extraParams: { from: dennis } })
    ...
    assert.equal(L_GUSDDebt, '0')
    assert.equal(L_GIVE, '0')
  ...
  // --- liquidate(), applied to trove with ICR > 110% that has the lowest ICR, and Stability Pool GUSD is GREATER THAN liquidated debt ---

  it("liquidate(), with 110% < ICR < TCR, and StabilityPool GUSD > debt to liquidate: offsets the trove entirely with the pool", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 GUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 GUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraGUSDAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraGUSDAmount: B_totalDebt, extraParams: { from: alice } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraGUSDAmount: dec(2000, 18), extraParams: { from: dennis } })

    // Alice deposits GUSD in the Stability Pool
    ...
    /* Check accrued Stability Pool rewards after. Total Pool deposits was 1490 GUSD, Alice sole depositor.
    As liquidated debt (250 GUSD) was completely offset

    Alice's expected compounded deposit: (1490 - 250) = 1240GUSD
    Alice's expected GIVE gain:  Bob's liquidated capped coll (minus gas comp), 2.75*0.995 give
  
    */
    const aliceExpectedDeposit = await stabilityPool.getCompoundedGUSDDeposit(alice)
    const aliceExpectedGIVEGain = await stabilityPool.getDepositorGIVEGain(alice)

    assert.isAtMost(th.getDifference(aliceExpectedDeposit.toString(), spDeposit.sub(B_totalDebt)), 2000)
    assert.isAtMost(th.getDifference(aliceExpectedGIVEGain, th.applyLiquidationFee(B_totalDebt.mul(th.toBN(dec(11, 17))).div(price))), 3000)
  ...
  it("liquidate(), with ICR% = 110 < TCR, and StabilityPool GUSD > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 GUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 GUSD of debt, resulting in ICR of 220%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(220, 16)), extraGUSDAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraGUSDAmount: B_totalDebt, extraParams: { from: alice } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraGUSDAmount: dec(2000, 18), extraParams: { from: dennis } })
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool GUSD > debt to liquidate: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 GUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 GUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraGUSDAmount: dec(250, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool GUSD > debt to liquidate: updates system snapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 GUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 GUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraGUSDAmount: dec(250, 18), extraParams: { from: bob } })
    ...
    // totalStakesSnapshot should have reduced to 22 give - the sum of Alice's coll( 20 give) and Dennis' coll (2 give )
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool GUSD > debt to liquidate: closes the Trove", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 GUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 GUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraGUSDAmount: dec(250, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool GUSD > debt to liquidate: can liquidate troves out of order", async () => {
    // taking out 1000 GUSD, CR of 200%
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: totalLiquidatedDebt, extraParams: { from: whale } })
  ...
  GUSD is LESS THAN the liquidated debt: a non fullfilled liquidation --- */

  it("liquidate(), with ICR > 110%, and StabilityPool GUSD < liquidated debt: Trove remains active", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 GUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 GUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraGUSDAmount: dec(1500, 18), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(240, 16)), extraGUSDAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraGUSDAmount: dec(2000, 18), extraParams: { from: dennis } })

    // Alice deposits 1490 GUSD in the Stability Pool
    ...
    /* Since the pool only contains 100 GUSD, and Bob's pre-liquidation debt was 250 GUSD,
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool GUSD < liquidated debt: Trove remains in TroveOwners array", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 GUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 GUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraGUSDAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    // Alice deposits 100 GUSD in the Stability Pool
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool GUSD < liquidated debt: nothing happens", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 GUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 GUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraGUSDAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    /*  Since Bob's debt (250 GUSD) is larger than all GUSD in the Stability Pool, Liquidation won’t happen

    After liquidation, totalStakes snapshot should equal Alice's stake (20 give) + Dennis stake (2 give) = 22 give.

    Since there has been no redistribution, the totalCollateral snapshot should equal the totalStakes snapshot: 22 give.

    Bob's new coll and stake should remain the same, and the updated totalStakes should still equal 25 give.
    */
    const bob_Trove = await troveManager.Troves(bob)
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool GUSD < liquidated debt: updates system shapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 GUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 GUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraGUSDAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    /* After liquidation, totalStakes snapshot should still equal the total stake: 25 give

    Since there has been no redistribution, the totalCollateral snapshot should equal the totalStakes snapshot: 25 give.*/
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool GUSD < liquidated debt: causes correct Pool offset and GIVE gain, and doesn't redistribute to active troves", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 GUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 GUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraGUSDAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    assert.equal(aliceExpectedGIVEGain.toString(), '0')
    ...
    const L_GUSDDebt_After = (await troveManager.L_GUSDDebt()).toString()
    const L_GIVE_After = (await troveManager.L_GIVE()).toString()

    assert.equal(L_GUSDDebt_After, '0')
    assert.equal(L_GIVE_After, '0')
  })

  it("liquidate(), with ICR > 110%, and StabilityPool GUSD < liquidated debt: ICR of non liquidated trove does not change", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 GUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 GUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    // Carol withdraws up to debt of 240 GUSD, -> ICR of 250%.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraGUSDAmount: dec(1500, 18), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(240, 16)), extraGUSDAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraGUSDAmount: dec(2000, 18), extraParams: { from: dennis } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(250, 16)), extraGUSDAmount: dec(240, 18), extraParams: { from: carol } })
    ...
    //Check SP GUSD has been completely emptied
    assert.equal((await stabilityPool.getTotalGUSDDeposits()).toString(), dec(100, 18))
    ...
    await gusdToken.transfer(bob, dec(100, 18), { from: alice })
    ...
    // Alice provides another 50 GUSD to pool
    ...
    //Check SP GUSD has been completely emptied
    assert.equal((await stabilityPool.getTotalGUSDDeposits()).toString(), dec(150, 18))
  ...
  it("liquidate() with ICR > 110%, and StabilityPool GUSD < liquidated debt: total liquidated coll and debt is correct", async () => {
    // Whale provides 50 GUSD to the SP
    await openTrove({ ICR: toBN(dec(300, 16)), extraGUSDAmount: dec(50, 18), extraParams: { from: whale } })
  ...
  it("liquidate(): Doesn't liquidate undercollateralized trove if it is the only trove in the system", async () => {
    // Alice creates a single trove with 0.62 GIVE and a debt of 62 GUSD, and provides 10 GUSD to SP
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    /*  Since there is 0 GUSD in the stability Pool, A, with ICR >110%, should stay active.
  ...
  it("liquidate(): does not affect the SP deposit or GIVE gain when called on an SP depositor's address that has no trove", async () => {
    ...
    await openTrove({ ICR: toBN(dec(220, 16)), extraGUSDAmount: spDeposit, extraParams: { from: bob } })

    // Bob sends tokens to Dennis, who has no trove
    await gusdToken.transfer(dennis, spDeposit, { from: bob })

    //Dennis provides 200 GUSD to SP
    ...
    assert.isAtMost(th.getDifference(dennis_GIVEGain_Before, th.applyLiquidationFee(C_coll)), 1000)
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(220, 16)), extraGUSDAmount: dec(1000, 18), extraParams: { from: whale } })

    const { gusdAmount: A_gusdAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: dec(300, 18), extraParams: { from: alice } })
    const { gusdAmount: B_gusdAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: dec(200, 18), extraParams: { from: bob } })
    const { gusdAmount: C_gusdAmount } = await openTrove({ ICR: toBN(dec(206, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: carol } })
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, re-open, be reedemed and claim again", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 GUSD of debt, resulting in ICRs of 266%.
    // Bob withdraws up to 480 GUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraGUSDAmount: dec(480, 18), extraParams: { from: bob } })
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraGUSDAmount: B_totalDebt, extraParams: { from: alice } })
    ...
    // Bob re-opens the trove, price 200, total debt 80 GUSD, ICR = 120% (lowest one)
    // Dennis redeems 30, so Bob has a surplus of (200 * 0.48 - 30) / 200 = 0.33 GIVE
    await priceFeed.setPrice('200000000000000000000')
    const { collateral: B_coll_2, netDebt: B_netDebt_2 } = await openTrove({ ICR: toBN(dec(150, 16)), extraGUSDAmount: dec(480, 18), extraParams: { from: bob, value: bob_remainingCollateral } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraGUSDAmount: B_netDebt_2, extraParams: { from: dennis } })
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, after another claim from a redemption", async () => {
    // --- SETUP ---
    // Bob withdraws up to 90 GUSD of debt, resulting in ICR of 222%
    const { collateral: B_coll, netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(222, 16)), extraGUSDAmount: dec(90, 18), extraParams: { from: bob } })
    // Dennis withdraws to 150 GUSD of debt, resulting in ICRs of 266%.
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraGUSDAmount: B_netDebt, extraParams: { from: dennis } })

    // --- TEST ---
    // skip bootstrapping phase
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_WEEK * 2, web3.currentProvider)

    // Dennis redeems 40, so Bob has a surplus of (200 * 1 - 40) / 200 = 0.8 GIVE
    ...
    // Bob re-opens the trove, price 200, total debt 250 GUSD, ICR = 240% (lowest one)
    ...
    // Alice deposits GUSD in the Stability Pool
    await openTrove({ ICR: toBN(dec(266, 16)), extraGUSDAmount: B_totalDebt_2, extraParams: { from: alice } })
  ...
  it("liquidateTroves(): With all ICRs > 110%, Liquidates Troves until system leaves recovery mode", async () => {
    ...
    // Everyone withdraws some GUSD from their Trove, resulting in different ICRs
    ...
    const { totalDebt: H_totalDebt } = await openTrove({ ICR: toBN(dec(222, 16)), extraGUSDAmount: dec(5000, 18), extraParams: { from: harry } })
    const liquidationAmount = E_totalDebt.add(F_totalDebt).add(G_totalDebt).add(H_totalDebt)
    await openTrove({ ICR: toBN(dec(400, 16)), extraGUSDAmount: liquidationAmount, extraParams: { from: alice } })

    // Alice deposits GUSD to Stability Pool
    await stabilityPool.provideToSP(liquidationAmount, ZERO_ADDRESS, { from: alice })

    // price drops
    // price drops to 1ETH:90GUSD, reducing TCR below 150%
  ...
  it("liquidateTroves(): Liquidates Troves until 1) system has left recovery mode AND 2) it reaches a Trove with ICR >= 110%", async () => {
    ...
    // price drops to 1ETH:85GUSD, reducing TCR below 150%
  ...
  it('liquidateTroves(): liquidates only up to the requested number of undercollateralized troves', async () => {
    await openTrove({ ICR: toBN(dec(300, 16)), extraParams: { from: whale, value: dec(300, 'give') } })
  ...
  it("liquidateTroves(): does nothing if n = 0", async () => {
    await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: dec(100, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: dec(200, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: dec(300, 18), extraParams: { from: carol } })
  ...
  it("liquidateTroves(): a liquidation sequence containing Pool offsets increases the TCR", async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: dec(500, 18), extraParams: { from: whale } })
    ...
    await openTrove({ ICR: toBN(dec(198, 16)), extraGUSDAmount: dec(101, 18), extraParams: { from: defaulter_1 } })
    await openTrove({ ICR: toBN(dec(184, 16)), extraGUSDAmount: dec(217, 18), extraParams: { from: defaulter_2 } })
    await openTrove({ ICR: toBN(dec(183, 16)), extraGUSDAmount: dec(328, 18), extraParams: { from: defaulter_3 } })
    await openTrove({ ICR: toBN(dec(186, 16)), extraGUSDAmount: dec(431, 18), extraParams: { from: defaulter_4 } })
    ...
    // Check Stability Pool has 500 GUSD
  ...
  it("liquidateTroves(): A liquidation sequence of pure redistributions decreases the TCR, due to gas compensation, but up to 0.5%", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(250, 16)), extraGUSDAmount: dec(500, 18), extraParams: { from: whale } })
    ...
    const { collateral: d1_coll, totalDebt: d1_totalDebt } = await openTrove({ ICR: toBN(dec(198, 16)), extraGUSDAmount: dec(101, 18), extraParams: { from: defaulter_1 } })
    const { collateral: d2_coll, totalDebt: d2_totalDebt } = await openTrove({ ICR: toBN(dec(184, 16)), extraGUSDAmount: dec(217, 18), extraParams: { from: defaulter_2 } })
    const { collateral: d3_coll, totalDebt: d3_totalDebt } = await openTrove({ ICR: toBN(dec(183, 16)), extraGUSDAmount: dec(328, 18), extraParams: { from: defaulter_3 } })
    const { collateral: d4_coll, totalDebt: d4_totalDebt } = await openTrove({ ICR: toBN(dec(166, 16)), extraGUSDAmount: dec(431, 18), extraParams: { from: defaulter_4 } })
  ...
  it('liquidateTroves(): does nothing if all troves have ICR > 110% and Stability Pool is empty', async () => {
    ...
    // Confirm 0 GUSD in Stability Pool
    assert.equal((await stabilityPool.getTotalGUSDDeposits()).toString(), '0')
  ...
  it('liquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves', async () => {
    ...
    // Whale adds GUSD to SP
    const spDeposit = F_totalDebt.add(G_totalDebt)
    await openTrove({ ICR: toBN(dec(285, 16)), extraGUSDAmount: spDeposit, extraParams: { from: whale } })
    ...
    // Confirm GUSD in Stability Pool
    assert.equal((await stabilityPool.getTotalGUSDDeposits()).toString(), spDeposit.toString())
  ...
  it('liquidateTroves():  emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(285, 16)), extraGUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    const entireDebt_A = (await troveManager.Troves(alice))[0].add(await troveManager.getPendingGUSDDebtReward(alice))
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const { gusdAmount: gusdAmountD } = await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: dennis } })
    const { gusdAmount: gusdAmountE } = await openTrove({ ICR: toBN(dec(133, 16)), extraParams: { from: erin } })
    const { gusdAmount: gusdAmountF } = await openTrove({ ICR: toBN(dec(111, 16)), extraParams: { from: freddy } })

    // Check list size is 4
    assert.equal((await sortedTroves.getSize()).toString(), '4')

    // Check token balances before
    assert.equal((await gusdToken.balanceOf(dennis)).toString(), gusdAmountD)
    assert.equal((await gusdToken.balanceOf(erin)).toString(), gusdAmountE)
    assert.equal((await gusdToken.balanceOf(freddy)).toString(), gusdAmountF)
  ...
  it("liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and GIVE gain", async () => {
    // Whale provides GUSD to the SP
    const { gusdAmount: W_gusdAmount } = await openTrove({ ICR: toBN(dec(300, 16)), extraGUSDAmount: dec(4000, 18), extraParams: { from: whale } })
    await stabilityPool.provideToSP(W_gusdAmount, ZERO_ADDRESS, { from: whale })

    const { gusdAmount: A_gusdAmount, totalDebt: A_totalDebt, collateral: A_coll } = await openTrove({ ICR: toBN(dec(191, 16)), extraGUSDAmount: dec(40, 18), extraParams: { from: alice } })
    const { gusdAmount: B_gusdAmount, totalDebt: B_totalDebt, collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: dec(240, 18), extraParams: { from: bob } })
    ...
    await stabilityPool.provideToSP(A_gusdAmount, ZERO_ADDRESS, { from: alice })
    await stabilityPool.provideToSP(B_gusdAmount, ZERO_ADDRESS, { from: bob })

    const totalDeposit = W_gusdAmount.add(A_gusdAmount).add(B_gusdAmount)
    ...
    // Check GUSD in Pool
    assert.equal((await stabilityPool.getTotalGUSDDeposits()).toString(), totalDeposit)
    ...
    Total GUSD in Pool: 680 GUSD

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 100 + 300 + 100 = 500 GUSD
    Total liquidated GIVE = 1 + 3 + 1 = 5 GIVE

    Whale GUSD Loss: 500 * (400/680) = 294.12 GUSD
    Alice GUSD Loss:  500 *(40/680) = 29.41 GUSD
    Bob GUSD Loss: 500 * (240/680) = 176.47 GUSD

    Whale remaining deposit: (400 - 294.12) = 105.88 GUSD
    Alice remaining deposit: (40 - 29.41) = 10.59 GUSD
    Bob remaining deposit: (240 - 176.47) = 63.53 GUSD

    Whale GIVE Gain: 5*0.995 * (400/680) = 2.93 GIVE
    Alice GIVE Gain: 5*0.995 *(40/680) = 0.293 GIVE
    Bob GIVE Gain: 5*0.995 * (240/680) = 1.76 GIVE

    Total remaining deposits: 180 GUSD
    Total GIVE gain: 5*0.995 GIVE */

    const GUSDinSP = (await stabilityPool.getTotalGUSDDeposits()).toString()
    const ETHinSP = (await stabilityPool.getGIVE()).toString()

    // Check remaining GUSD Deposits and GIVE gain, for whale and depositors whose troves were liquidated
    const whale_Deposit_After = (await stabilityPool.getCompoundedGUSDDeposit(whale)).toString()
    ...
    const whale_GIVEGain = (await stabilityPool.getDepositorGIVEGain(whale)).toString()
    const alice_GIVEGain = (await stabilityPool.getDepositorGIVEGain(alice)).toString()
    const bob_GIVEGain = (await stabilityPool.getDepositorGIVEGain(bob)).toString()

    const liquidatedDebt = A_totalDebt.add(B_totalDebt).add(C_totalDebt)
    const liquidatedColl = A_coll.add(B_coll).add(C_coll)
    assert.isAtMost(th.getDifference(whale_Deposit_After, W_gusdAmount.sub(liquidatedDebt.mul(W_gusdAmount).div(totalDeposit))), 100000)
    assert.isAtMost(th.getDifference(alice_Deposit_After, A_gusdAmount.sub(liquidatedDebt.mul(A_gusdAmount).div(totalDeposit))), 100000)
    assert.isAtMost(th.getDifference(bob_Deposit_After, B_gusdAmount.sub(liquidatedDebt.mul(B_gusdAmount).div(totalDeposit))), 100000)

    assert.isAtMost(th.getDifference(whale_GIVEGain, th.applyLiquidationFee(liquidatedColl).mul(W_gusdAmount).div(totalDeposit)), 2000)
    assert.isAtMost(th.getDifference(alice_GIVEGain, th.applyLiquidationFee(liquidatedColl).mul(A_gusdAmount).div(totalDeposit)), 2000)
    assert.isAtMost(th.getDifference(bob_GIVEGain, th.applyLiquidationFee(liquidatedColl).mul(B_gusdAmount).div(totalDeposit)), 2000)
    ...
    assert.isAtMost(th.getDifference(total_GUSDinSP, totalDeposit.sub(liquidatedDebt)), 1000)
    assert.isAtMost(th.getDifference(total_GIVEinSP, th.applyLiquidationFee(liquidatedColl)), 1000)
  })

  it("liquidateTroves(): Liquidating troves at ICR <=100% with SP deposits does not alter their deposit or GIVE gain", async () => {
    // Whale provides 400 GUSD to the SP
    await openTrove({ ICR: toBN(dec(300, 16)), extraGUSDAmount: dec(400, 18), extraParams: { from: whale } })
    await stabilityPool.provideToSP(dec(400, 18), ZERO_ADDRESS, { from: whale })

    await openTrove({ ICR: toBN(dec(182, 16)), extraGUSDAmount: dec(170, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(180, 16)), extraGUSDAmount: dec(300, 18), extraParams: { from: bob } })
    ...
    // Check GUSD and GIVE in Pool  before
    ...
    assert.equal(GUSDinSP_Before, dec(800, 18))
    ...
    // Check GUSD and GIVE in Pool after
    ...
    const whale_GIVEGain_After = (await stabilityPool.getDepositorGIVEGain(whale)).toString()
    ...
    assert.equal(whale_GIVEGain_After, '0')
    assert.equal(alice_GIVEGain_After, '0')
    assert.equal(bob_GIVEGain_After, '0')
  })
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains active", async () => {
    ...
    await openTrove({ ICR: toBN(dec(300, 16)), extraGUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    That leaves 50 GUSD in the Pool to absorb exactly half of Carol's debt (100) */
    await troveManager.liquidateTroves(10)
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains in TroveOwners Array", async () => {
    ...
    // Whale provides GUSD to the SP
    const spDeposit = A_totalDebt.add(B_totalDebt).add(C_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(220, 16)), extraGUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
  ...
  it("liquidateTroves() with a non fullfilled liquidation: still can liquidate further troves after the non-liquidated, emptied pool", async () => {
    ...
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraGUSDAmount: D_totalDebt, extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(208, 16)), extraParams: { from: erin } })
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
     ...
     That leaves 97 GUSD in the Pool that won’t be enough to absorb Carol,
  ...
  it("liquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 GUSD in the Pool that won’t be enough to absorb any other trove */
    const tx = await troveManager.liquidateTroves(10)

    // Expect system debt reduced by 203 GUSD and system coll 2.3 GIVE
    const entireSystemCollAfter = await troveManager.getEntireSystemColl()
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt()
  ...
  it("liquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values", async () => {
    ...
    // Whale provides GUSD to the SP
    const spDeposit = A_totalDebt.add(B_totalDebt).add(C_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(240, 16)), extraGUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 GUSD in the Pool which won’t be enough for any other liquidation */
    const liquidationTx = await troveManager.liquidateTroves(10)

    const [liquidatedDebt, liquidatedColl, collGasComp, gusdGasComp] = th.getEmittedLiquidationValues(liquidationTx)
    ...
    assert.equal(gusdGasComp.toString(), dec(400, 18))
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
    ...
    await openTrove({ ICR: toBN(dec(426, 16)), extraGUSDAmount: spDeposit, extraParams: { from: alice } })
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
    ...
    const spDeposit = B_totalDebt.add(C_totalDebt).add(D_totalDebt).add(E_totalDebt).add(F_totalDebt)
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(426, 16)), extraGUSDAmount: spDeposit, extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(426, 16)), extraGUSDAmount: A_totalDebt, extraParams: { from: whale } })

    // Alice deposits GUSD to Stability Pool
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: alice })

    // to compensate borrowing fee
    await gusdToken.transfer(alice, A_totalDebt, { from: whale })
    // Alice closes trove
  ...
  it("batchLiquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    // Expect system debt reduced by 203 GUSD and system coll by 2 GIVE
    const entireSystemCollAfter = await troveManager.getEntireSystemColl()
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt()
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool GUSD > debt to liquidate: can liquidate troves out of order", async () => {
    ...
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(206, 16)), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(280, 16)), extraGUSDAmount: dec(500, 18), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(282, 16)), extraGUSDAmount: dec(500, 18), extraParams: { from: freddy } })

    // Whale provides 1000 GUSD to the SP
    const spDeposit = A_totalDebt.add(C_totalDebt).add(D_totalDebt)
    await openTrove({ ICR: toBN(dec(219, 16)), extraGUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool empty: doesn't liquidate any troves", async () => {
    ...
    const dennisDebt_After = (await troveManager.Troves(dennis))[0].add(await troveManager.getPendingGUSDDebtReward(dennis))
    const bobDebt_After = (await troveManager.Troves(bob))[0].add(await troveManager.getPendingGUSDDebtReward(bob))
    const carolDebt_After = (await troveManager.Troves(carol))[0].add(await troveManager.getPendingGUSDDebtReward(carol))
  ...
  it('batchLiquidateTroves(): skips liquidation of troves with ICR > TCR, regardless of Stability Pool size', async () => {
    ...
    await openTrove({ ICR: toBN(dec(245, 16)), extraGUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    // Whale withdraws entire deposit, and re-deposits 132 GUSD
    ...
    // Expected Stability Pool to fully absorb B (92 GUSD + 10 virtual debt), 
    // but not E as there are not enough funds in Stability Pool
    
    const stabilityBefore = await stabilityPool.getTotalGUSDDeposits()
    ...
    const stabilityAfter = await stabilityPool.getTotalGUSDDeposits()
  ...
  it('batchLiquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    // Whale opens trove and adds 220 GUSD to SP
    const spDeposit = F_totalDebt.add(G_totalDebt).add(A_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(285, 16)), extraGUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
