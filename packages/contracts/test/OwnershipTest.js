...
contract('All Givety functions with onlyOwner modifier', async accounts => {
  ...
  describe('CommunityIssuance', async accounts => {
    it("setAddresses(): reverts when called by non-owner, with wrong addresses, or twice", async () => {
      const params = [gvtyToken.address, stabilityPool.address]
  ...
  describe('GVTYStaking', async accounts => {
    it("setAddresses(): reverts when called by non-owner, with wrong addresses, or twice", async () => {
      await testSetAddresses(gvtyStaking, 5)
    })
  })

  describe('LockupContractFactory', async accounts => {
    it("setGVTYAddress(): reverts when called by non-owner, with wrong address, or twice", async () => {
      await th.assertRevert(lockupContractFactory.setGVTYTokenAddress(gvtyToken.address, { from: alice }))

      const params = [gvtyToken.address]

      // Attempt to use zero address
      await testZeroAddress(lockupContractFactory, params, 'setGVTYTokenAddress')
      // Attempt to use non contract
      await testNonContractAddress(lockupContractFactory, params, 'setGVTYTokenAddress')

      // Owner can successfully set any address
      const txOwner = await lockupContractFactory.setGVTYTokenAddress(gvtyToken.address, { from: owner })

      assert.isTrue(txOwner.receipt.status)
      // fails if called twice
      await th.assertRevert(lockupContractFactory.setGVTYTokenAddress(gvtyToken.address, { from: owner }))
    })
  })
})
