...
GUSD_GAS_COMPENSATION = 200.0
MIN_NET_DEBT = 1800.0
MAX_FEE = Wei(1e18)

"""# Give price (exogenous)

Give is the collateral for GUSD. The give price $P_t^e$ follows 
> $P_t^e = P_{t-1}^e (1+\zeta_t^e)(1+\sigma_t^e)$, 

where $\zeta_t^e \sim N(0, $ sd_ether$)$ represents give price shock and $\sigma_t^e$ the drift of give price. At the end of the year, the expected give price is:
...
"""# GVTY price
In the first month, the price of GVTY follows

> $P_t^q = P_{t-1}^q (1+\zeta_t^q)(1+\sigma_t^q)$. 

Note that $\zeta_t^q \sim N(0,$ sd_GVTY) represents GVTY price shock and $\sigma_t^q$ the drift. Here, $\sigma_t^q =$ drift_GVTY, so that the expected GVTY price increases from price_GVTY_initial to the following at the end of the first month:
> $E(P_{720}^q) = $price_GVTY_initial$ \cdot (1+$ drift_GVTY$)^{720}$

The GVTY price from the second month on is endogenously determined.
"""

#GVTY price & airdrop
price_GVTY_initial = 0.4
price_GVTY = [price_GVTY_initial]
sd_GVTY=0.005
drift_GVTY = 0.0035
supply_GVTY=[0]
GVTY_total_supply=100000000

"""**GVTY Endogenous Price**

The staked GVTY pool earning consists of the issuance fee revenue and redemption fee revenue
> $R_t^q = R_t^i + R_t^r.$
...
> $$r_t=\frac{GVTY Market Cap}{Annualized Earning}=\frac{MC_t}{E_t}$$

> $MC_t=P_t^q \cdot$ GVTY_total_supply

Therefore, the GVTY price dynamics is determined
> $$P_t^q=discount \cdot \frac{r^{PE}}{GVTY\_total\_supply}E_t$$

Interpretation: The denominator implies that with more GVTY tokens issued, GVTY price decreases. However, the depreciation effect can be counteracted by the growth of the earning.
...
$R_{t-1}^s$ is the return in the stability pool, which consists of liquidation gain and airdrop GVTY gain.
...
The natural rate compensates the opportunity cost and risk undertaken by the stability pool providers. It resembles the risk-free government bond return in the macroeconomics model. Stability pool depositors compare the return of the stability pool with the outside investment opportunities. A positive shock $\zeta_t^n$ implies investment on other platforms, e.g. Compound, GiveSwap, Aave, yield higher returns, thus making the stability pool less appealing.
...
> (collateral in give, debt in GUSD, collateral ratio target, rational inattention, collateral ratio)
...
$P_{t}^{l}$ is the price of GUSD.

$N_t^o$ is rounded to an integer.

---

The amount of GUSD tokens generated by a new trove is
> $$Q_t^d(i) = \frac{P_t^e Q_t^e(i)}{CR^*(i)}.$$

---


The distribution of give $Q_t^e(i)$ follows
...
The collateral ratio of the existing troves vary with the give price $P_t^e$
...
#sensitivity to GUSD price & issuance fee
alpha = 0.3
...
#close troves
sd_closetroves=0.5
#sensitivity to GUSD price
beta = 0.2

"""**Trove Liquidation**

At the beginning of each period, 
right after the feed of give price, 
...
- $R_t^a=P_{t}^q\hat{Q}_t^q$ is the airdrop gain, $\hat{Q}_t^q=1000$ denotes the amount of GVTY token airdropped to the stability pool providers
- $D_{t}^{s}$ is the total amount of GUSD deposited in the Stability Pool (see below)
...
print(" - GIVE period 1 -")
print(f"Min GIVE price: {min(price_ether[1:period1])}")
print(f"Max GIVE price: {max(price_ether[1:period1])}")
for i in range(period1, period2):
    random.seed(2019375+10000*i)
    shock_ether = random.normalvariate(0, sd_ether)
    price_ether.append(price_ether[i-1] * (1 + shock_ether) * (1 + drift_ether2))
print(" - GIVE period 2 -")
print(f"Min GIVE price: {min(price_ether[period1:period2])}")
print(f"Max GIVE price: {max(price_ether[period1:period2])}")
for i in range(period2, period3):
    random.seed(2019375+10000*i)
    shock_ether = random.normalvariate(0, sd_ether)
    price_ether.append(price_ether[i-1] * (1 + shock_ether) * (1 + drift_ether3))
print(" - GIVE period 3 -")
print(f"Min GIVE price: {min(price_ether[period2:period3])}")
print(f"Max GIVE price: {max(price_ether[period2:period3])}")
for i in range(period3, period4):
    random.seed(2019375+10000*i)
    shock_ether = random.normalvariate(0, sd_ether)
    price_ether.append(price_ether[i-1] * (1 + shock_ether) * (1 + drift_ether4))
print(" - GIVE period 4 -")
print(f"Min GIVE price: {min(price_ether[period3:period4])}")
print(f"Max GIVE price: {max(price_ether[period3:period4])}")
...
"""GVTY Price - First Month"""

#GVTY price
for i in range(1, month):
    random.seed(2+13*i)
    shock_GVTY = random.normalvariate(0,sd_GVTY)  
    price_GVTY.append(price_GVTY[i-1]*(1+shock_GVTY)*(1+drift_GVTY))
...
def pending_liquidations(contracts, price_ether_current):
    ...
    stability_pool_balance = contracts.stabilityPool.getTotalGUSDDeposits()
    trove = last_trove
...
# F = 0.5 ** (1/8760)
# F = 0.99992087674
def quantity_GVTY_airdrop(index):
    F = 0.99992087674
    if index <= 0:
        return 0
    return 32e6 * (F ** (index-1) - F ** index)

def liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_GUSD, price_GVTY_current, data, index):
    if len(active_accounts) == 0:
        return [0, 0]

    stability_pool_previous = contracts.stabilityPool.getTotalGUSDDeposits() / 1e18
    stability_pool_eth_previous = contracts.stabilityPool.getGIVE() / 1e18
    ...
    while pending_liquidations(contracts, price_ether_current):
        try:
            ...
            for i in range(NUM_LIQUIDATIONS):
                ...
                ICR = contracts.troveManager.getCurrentICR(trove, Wei(price_ether_current * 1e18))
                print(f"ICR: {ICR}")
    stability_pool_current = contracts.stabilityPool.getTotalGUSDDeposits() / 1e18
    stability_pool_eth_current = contracts.stabilityPool.getGIVE() / 1e18
    ...
    liquidation_gain = ether_liquidated * price_ether_current - debt_liquidated * price_GUSD
    airdrop_gain = price_GVTY_current * quantity_GVTY_airdrop(index)

    data['liquidation_gain'][index] = liquidation_gain
    data['airdrop_gain'][index] = airdrop_gain

    return_stability = calculate_stability_return(contracts, price_GUSD, data, index)

    return [ether_liquidated, return_stability]

def calculate_stability_return(contracts, price_GUSD, data, index):
    stability_pool_previous = contracts.stabilityPool.getTotalGUSDDeposits() / 1e18
    if index == 0:
        ...
        return_stability = (year/index) * \
            (sum(data['liquidation_gain'][0:index]) +
             ...
             ) / (price_GUSD * stability_pool_previous)
    else:
...
def close_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_GUSD, index):
    if len(active_accounts) == 0:
        return [0]
    ...
    if index <= 240:
        ...
        number_closetroves = max(0, n_steady * (1+shock_closetroves)) + beta*(1-price_GUSD)*n_troves
    ...
    for i in range(0, len(drops)):
        ...
        coll = amounts['coll']
        debt = amounts['debt']
        pending = get_gusd_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt)
        if pending == 0:
...
def transfer_from_to(contracts, from_account, to_account, amount):
    balance = contracts.gusdToken.balanceOf(from_account)
    transfer_amount = min(balance, amount)
    if transfer_amount == 0:
        return amount
    if from_account == to_account:
        return amount
    contracts.gusdToken.transfer(to_account, transfer_amount, { 'from': from_account })
    pending = amount - transfer_amount

    return pending

def get_gusd_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt):
    gusdBalance = contracts.gusdToken.balanceOf(account)
    if debt > gusdBalance:
        pending = debt - gusdBalance
        ...
        # try with active accounts, which are more likely to hold GUSD
        ...
        if pending > 0:
            print(f"\n ***Error: not enough GUSD to repay! {debt / 1e18} GUSD for {account}")
...
def adjust_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, index):
    ...
    for i, working_trove in enumerate(active_accounts):
        ...
        if p >= ratio:
            ...
            if check < -1:
                ...
                pending = get_gusd_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, repay_amount)
                if pending == 0:
                    contracts.borrowerOperations.repayGUSD(repay_amount, hints[0], hints[1], { 'from': account })
                ...
                if isNewTCRAboveCCR(contracts, 0, False, withdraw_amount_wei, True, floatToWei(price_ether_current)):
                    contracts.borrowerOperations.withdrawGUSD(MAX_FEE, withdraw_amount_wei, hints[0], hints[1], { 'from': account })
                    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
                    issuance_GUSD_adjust = issuance_GUSD_adjust + rate_issuance * withdraw_amount
    ...
    return [coll_added_float, issuance_GUSD_adjust]
...
def open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention, price_ether_current):
    ...
    debtChange = floatToWei(supply_trove) + GUSD_GAS_COMPENSATION
    gusd = get_gusd_amount_from_net_debt(contracts, floatToWei(supply_trove))
    if isNewTCRAboveCCR(contracts, coll, True, debtChange, True, floatToWei(price_ether_current)):
        contracts.borrowerOperations.openTrove(MAX_FEE, gusd, hints[0], hints[1],
...
def open_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_GUSD, index):
    random.seed(2019*index)
    shock_opentroves = random.normalvariate(0,sd_opentroves)
    n_troves = len(active_accounts)
    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
    coll_added = 0
    issuance_GUSD_open = 0

    if index <= 0:
        number_opentroves = initial_open
    elif price_GUSD <= 1 + rate_issuance:
        number_opentroves = max(0, n_steady * (1+shock_opentroves))
    else:
        number_opentroves = max(0, n_steady * (1+shock_opentroves)) + \
                        alpha * (price_GUSD - rate_issuance - 1) * n_troves
    ...
    for i in range(0, number_opentroves):
        ...
        issuance_GUSD_open = issuance_GUSD_open + rate_issuance * supply_trove
    ...
    return [coll_added, issuance_GUSD_open]
...
def stability_update(accounts, contracts, active_accounts, return_stability, index):
    supply = contracts.gusdToken.totalSupply() / 1e18
    stability_pool_previous = contracts.stabilityPool.getTotalGUSDDeposits() / 1e18
    ...
    if stability_pool > stability_pool_previous:
        ...
        while remaining > 0 and i < len(active_accounts):
          ...
          balance = contracts.gusdToken.balanceOf(account) / 1e18
        ...
        current_deposit = contracts.stabilityPool.getCompoundedGUSDDeposit(accounts[0])
...
"""GUSD Price, liquidity pool, and redemption

**Price Determination**

---
With the supply and demand of GUSD tokens defined above, the price of GUSD at the current period is given by the following equilibrium condition:
> $$S_t = D_t^s + D_t^l = D_t^s + D_{t-1}^l (1+\zeta_t^l)(1+\sigma_t^l) (\frac{P_t^l}{P_{t-1}^l})^\delta$$

where $S$ is the total supply of GUSD.

Solving this equation gives that:
> $$P_t^l = P_{t-1}^l (\frac{S_t-D_t^s}{D_{t-1}^l(1+\zeta_t^l)(1+\sigma_t^l)})^{1/\delta}$$
"""

def calculate_price(price_GUSD, liquidity_pool, liquidity_pool_next):
  # liquidity_pool = supply - stability_pool
  # liquidity_pool_next = liquidity_pool_previous * drift_liquidity * (1+shock_liquidity)
  price_GUSD_current= price_GUSD * (liquidity_pool / liquidity_pool_next) ** (1/delta)

  return price_GUSD_current

""" **Stabilizers**

There are two stabilizers to attenuate GUSD price deviation from its target range.
...
The amount of $Q_t^d(c)$ is expected to bring the GUSD price back to $1.1+f_t^i$. This means that
...
If $P_t^l < 1-f_t^r$, a fraction $\chi_t$ of GUSD in the liquidity pool is used for redemption
> $$D_t^r = \chi_t D_t^l,$$
...
Note that unlike stabilizer 1, stabilizer 2 has impact of GUSD price in
...
def redeem_trove(accounts, contracts, i, price_ether_current):
    gusd_balance = contracts.gusdToken.balanceOf(accounts[i])
    [firstRedemptionHint, partialRedemptionHintNICR, truncatedGUSDamount] = contracts.hintHelpers.getRedemptionHints(gusd_balance, price_ether_current, 70)
    if truncatedGUSDamount == Wei(0):
        return None
    ...
    try:
        ...
        print(f"GUSD Token:    {contracts.gusdToken.address}")
        print(f"i: {i}")
        print(f"account: {accounts[i]}")
        print(f"GUSD bal: {gusd_balance / 1e18}")
        print(f"truncated: {truncatedGUSDamount / 1e18}")
        ...
        print(f"amount: {truncatedGUSDamount}")
...
def price_stabilizer(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_GUSD, index):

    stability_pool = contracts.stabilityPool.getTotalGUSDDeposits() / 1e18
    redemption_pool = 0
    redemption_fee = 0
    issuance_GUSD_stabilizer = 0
    ...
    #Calculating Price
    price_GUSD_current = calculate_price(price_GUSD, liquidity_pool, liquidity_pool_next)
    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
    rate_redemption = contracts.troveManager.getRedemptionRateWithDecay() / 1e18

    #Stabilizer
    #Ceiling Arbitrageurs
    if price_GUSD_current > 1.1 + rate_issuance:
        supply_wanted = stability_pool + \
                        liquidity_pool_next * \
                        ((1.1+rate_issuance) / price_GUSD)**delta
        ...
        issuance_GUSD_stabilizer = rate_issuance * supply_trove
        if open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention):
            price_GUSD_current = 1.1 + rate_issuance
            liquidity_pool = supply_wanted - stability_pool


    #Floor Arbitrageurs
    if price_GUSD_current < 1 - rate_redemption:
        ...
        supply_target = stability_pool + \
                        liquidity_pool_next * \
                        ((1-rate_redemption) / price_GUSD)**delta
        ...
        if supply_diff < redemption_ratio * liquidity_pool:
            ...
            price_GUSD_current = 1 - rate_redemption
        else:
    ...
    return [price_GUSD_current, redemption_pool, redemption_fee, issuance_GUSD_stabilizer]

"""# GVTY Market"""

def GVTY_market(index, data):
    #quantity_GVTY = (GVTY_total_supply/3)*(1-0.5**(index/period))
    np.random.seed(2+3*index)
    if index <= month:
        price_GVTY_current = price_GVTY[index-1]
        ...
        price_GVTY_current = discount * PE_ratio * annualized_earning / GVTY_total_supply

    #MC_GVTY_current = price_GVTY_current * quantity_GVTY

    return [price_GVTY_current, annualized_earning]
