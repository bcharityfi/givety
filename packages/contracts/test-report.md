...
# Test output
The following is the output of a complete test run, made on commit [`a1824dd8`](https://github.com/givety/dev/tree/a1824dd88f4928b424cae372d59c4455d5c9a2c2), from April 16th, 2021.
...
$ hardhat test
  ...
  Contract: Access Control: Givety functions with the caller restricted to Givety contract(s)
    BorrowerOperations
      ✓ moveGIVEGainToTrove(): reverts when called by an account that is not StabilityPool
    TroveManager
    ...
    ActivePool
      ✓ sendGIVE(): reverts when called by an account that is not BO nor TroveM nor SP
      ✓ increaseGUSDDebt(): reverts when called by an account that is not BO nor TroveM
      ✓ decreaseGUSDDebt(): reverts when called by an account that is not BO nor TroveM nor SP
      ✓ fallback(): reverts when called by an account that is not Borrower Operations nor Default Pool
    DefaultPool
      ✓ sendGIVEToActivePool(): reverts when called by an account that is not TroveManager
      ✓ increaseGUSDDebt(): reverts when called by an account that is not TroveManager
      ✓ decreaseGUSD(): reverts when called by an account that is not TroveManager
    ...
    LockupContract
      ✓ withdrawGVTY(): reverts when caller is not beneficiary (68ms)
    GVTYStaking
      ✓ increaseF_GUSD(): reverts when caller is not TroveManager
    GVTYToken
      ✓ sendToGVTYStaking(): reverts when caller is not the GVTYSstaking (49ms)
    CommunityIssuance
      ✓ sendGVTY(): reverts when caller is not the StabilityPool
      ✓ issueGVTY(): reverts when caller is not the StabilityPool

  Contract: BorrowerOperations
    Without proxy
      ✓ addColl(): reverts when top-up would leave trove with ICR < MCR (278ms)
      ✓ addColl(): Increases the activePool GIVE and raw give balance by correct amount (203ms)
      ...
      ✓ addColl(), active Trove: applies pending rewards and updates user's L_GIVE, L_GUSDDebt snapshots (736ms)
      ...
      ✓ withdrawColl(): reverts when requested GIVE withdrawal is > the trove's collateral (508ms)
      ...
      ✓ withdrawColl(): reduces ActivePool GIVE and raw give by correct amount (291ms)
      ✓ withdrawColl(): updates the stake and updates the total stakes (342ms)
      ✓ withdrawColl(): sends the correct amount of GIVE to the user (289ms)
      ✓ withdrawColl(): applies pending rewards and updates user's L_GIVE, L_GUSDDebt snapshots (1205ms)
      ✓ withdrawGUSD(): reverts when withdrawal would leave trove with ICR < MCR (516ms)
      ✓ withdrawGUSD(): decays a non-zero base rate (1192ms)
      ✓ withdrawGUSD(): reverts if max fee > 100% (668ms)
      ✓ withdrawGUSD(): reverts if max fee < 0.5% in Normal mode (639ms)
      ✓ withdrawGUSD(): reverts if fee exceeds max fee percentage (938ms)
      ✓ withdrawGUSD(): succeeds when fee is less than max fee percentage (1272ms)
      ✓ withdrawGUSD(): doesn't change base rate if it is already zero (1044ms)
      ✓ withdrawGUSD(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation (899ms)
      ✓ withdrawGUSD(): borrower can't grief the baseRate and stop it decaying by issuing debt at higher frequency than the decay granularity (852ms)
      ✓ withdrawGUSD(): borrowing at non-zero base rate sends GUSD fee to GVTY staking contract (1004ms)
      ✓ withdrawGUSD(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct (923ms)
      ✓ withdrawGUSD(): Borrowing at non-zero base rate increases the GVTY staking contract GUSD fees-per-unit-staked (986ms)
      ✓ withdrawGUSD(): Borrowing at non-zero base rate sends requested amount to the user (1562ms)
      ✓ withdrawGUSD(): Borrowing at zero base rate changes GUSD fees-per-unit-staked (839ms)
      ✓ withdrawGUSD(): Borrowing at zero base rate sends debt request to user (790ms)
      ✓ withdrawGUSD(): reverts when calling address does not have active trove (370ms)
      ✓ withdrawGUSD(): reverts when requested withdrawal amount is zero GUSD (459ms)
      ✓ withdrawGUSD(): reverts when system is in Recovery Mode (649ms)
      ✓ withdrawGUSD(): reverts when withdrawal would bring the trove's ICR < MCR (347ms)
      ✓ withdrawGUSD(): reverts when a withdrawal would cause the TCR of the system to fall below the CCR (439ms)
      ✓ withdrawGUSD(): reverts if system is in Recovery Mode (312ms)
      ✓ withdrawGUSD(): increases the Trove's GUSD debt by the correct amount (208ms)
      ✓ withdrawGUSD(): increases GUSD debt in ActivePool by correct amount (230ms)
      ✓ withdrawGUSD(): increases user GUSDToken balance by correct amount (243ms)
      ✓ repayGUSD(): reverts when repayment would leave trove with ICR < MCR (392ms)
      ✓ repayGUSD(): Succeeds when it would leave trove with net debt >= minimum net debt (435ms)
      ✓ repayGUSD(): reverts when it would leave trove with net debt < minimum net debt (256ms)
      ✓ repayGUSD(): reverts when calling address does not have active trove (431ms)
      ✓ repayGUSD(): reverts when attempted repayment is > the debt of the trove (418ms)
      ✓ repayGUSD(): reduces the Trove's GUSD debt by the correct amount (403ms)
      ✓ repayGUSD(): decreases GUSD debt in ActivePool by correct amount (406ms)
      ✓ repayGUSD(): decreases user GUSDToken balance by correct amount (404ms)
      ✓ repayGUSD(): can repay debt in Recovery Mode (557ms)
      ✓ repayGUSD(): Reverts if borrower has insufficient GUSD balance to cover his debt repayment (1122ms)
      ...
      ✓ adjustTrove(): borrowing at non-zero base rate sends GUSD fee to GVTY staking contract (889ms)
      ✓ adjustTrove(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct (1013ms)
      ✓ adjustTrove(): Borrowing at non-zero base rate increases the GVTY staking contract GUSD fees-per-unit-staked (837ms)
      ✓ adjustTrove(): Borrowing at non-zero base rate sends requested amount to the user (895ms)
      ✓ adjustTrove(): Borrowing at zero base rate changes GUSD balance of GVTY staking contract (783ms)
      ✓ adjustTrove(): Borrowing at zero base rate changes GVTY staking contract GUSD fees-per-unit-staked (881ms)
      ✓ adjustTrove(): Borrowing at zero base rate sends total requested GUSD to the user (783ms)
      ...
      ✓ adjustTrove(): reverts when GUSD repaid is > debt of the trove (451ms)
      ✓ adjustTrove(): reverts when attempted GIVE withdrawal is >= the trove's collateral (683ms)
      ...
      ✓ adjustTrove(): changes GUSDToken balance by the requested decrease (529ms)
      ✓ adjustTrove(): changes GUSDToken balance by the requested increase (577ms)
      ✓ adjustTrove(): Changes the activePool GIVE and raw give balance by the requested decrease (1371ms)
      ✓ adjustTrove(): Changes the activePool GIVE and raw give balance by the amount of GIVE sent (548ms)
      ✓ adjustTrove(): Changes the GUSD debt in ActivePool by requested decrease (541ms)
      ✓ adjustTrove(): Changes the GUSD debt in ActivePool by requested increase (397ms)
      ...
      ✓ adjustTrove(): Reverts if requested coll withdrawal and give is sent (288ms)
      ...
      ✓ adjustTrove(): Reverts if borrower has insufficient GUSD balance to cover his debt repayment (496ms)
      ...
      ✓ closeTrove(): reduces ActivePool GIVE and raw give by correct amount (466ms)
      ✓ closeTrove(): reduces ActivePool debt by correct amount (522ms)
      ✓ closeTrove(): updates the the total stakes (790ms)
      ✓ closeTrove(): sends the correct amount of GIVE to the user (390ms)
      ✓ closeTrove(): subtracts the debt of the closed Trove from the Borrower's GUSDToken balance (405ms)
      ✓ closeTrove(): applies pending rewards (1267ms)
      ✓ closeTrove(): reverts if borrower has insufficient GUSD balance to repay his entire debt (347ms)
      ...
      ✓ openTrove(): borrowing at non-zero base rate sends GUSD fee to GVTY staking contract (923ms)
      ✓ openTrove(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Trove struct (865ms)
      ✓ openTrove(): Borrowing at non-zero base rate increases the GVTY staking contract GUSD fees-per-unit-staked (794ms)
      ✓ openTrove(): Borrowing at non-zero base rate sends requested amount to the user (832ms)
      ✓ openTrove(): Borrowing at zero base rate changes the GVTY staking contract GUSD fees-per-unit-staked (602ms)
      ...
      ✓ openTrove(): Increases the activePool GIVE and raw give balance by correct amount (294ms)
      ✓ openTrove(): records up-to-date initial snapshots of L_GIVE and L_GUSDDebt (639ms)
      ✓ openTrove(): allows a user to open a Trove, then close it, then re-open it (687ms)
      ✓ openTrove(): increases the Trove's GUSD debt by the correct amount (129ms)
      ✓ openTrove(): increases GUSD debt in ActivePool by the debt of the trove (162ms)
      ✓ openTrove(): increases user GUSDToken balance by correct amount (85ms)
      ✓ getCompositeDebt(): returns debt + gas comp
      ✓ closeTrove(): fails if owner cannot receive GIVE (440ms)
  ...
  Contract: CollSurplusPool
    ✓ CollSurplusPool::getGIVE(): Returns the GIVE balance of the CollSurplusPool after redemption (2203ms)
    ...
    ✓ CollSurplusPool: claimColl(): Reverts if owner cannot receive GIVE surplus (723ms)
    ✓ CollSurplusPool: reverts trying to send GIVE to it
  ...
  Contract: Deployment script - Sets correct contract addresses dependencies after deployment
    ...
    ✓ Sets the correct GUSDToken address in TroveManager
    ...
    ✓ Sets the correct GVTYStaking address in TroveManager
    ...
    ✓ Sets the correct GUSDToken address in StabilityPool
    ...
    ✓ Sets the correct GVTYStaking address in BorrowerOperations
    ✓ Sets the correct GVTYToken address in GVTYStaking
    ✓ Sets the correct ActivePool address in GVTYStaking
    ✓ Sets the correct ActivePool address in GVTYStaking
    ✓ Sets the correct ActivePool address in GVTYStaking
    ✓ Sets the correct BorrowerOperations address in GVTYStaking
    ✓ Sets the correct CommunityIssuance address in GVTYToken
    ✓ Sets the correct GVTYStaking address in GVTYToken
    ✓ Sets the correct LockupContractFactory address in GVTYToken
    ✓ Sets the correct GVTYToken address in LockupContractFactory
    ✓ Sets the correct GVTYToken address in CommunityIssuance
    ✓ Sets the correct StabilityPool address in CommunityIssuance

  Contract: DefaultPool
    ✓ sendGIVEToActivePool(): fails if receiver cannot receive GIVE
...
TCR: 11.892415157517211309
  ...
  Contract: GVTY Token
    ...
    ✓ sendToGVTYStaking(): changes balances of GVTYStaking and calling account by the correct amounts (48ms)
  ...
  Contract: Deploying and funding One Year Lockup Contracts
    Deploying LCs
      ✓ GVTY Deployer can deploy LCs through the Factory (121ms)
      ✓ Anyone can deploy LCs through the Factory (71ms)
      ✓ GVTY Deployer can deploy LCs directly (43ms)
    ...
    Funding LCs
      ✓ GVTY transfer from GVTY deployer to their deployed LC increases the GVTY balance of the LC (397ms)
      ✓ GVTY Multisig can transfer GVTY to LCs deployed through the factory by anyone (168ms)
    Withdrawal attempts on funded, inactive LCs immediately after funding
      ...
      ✓ GVTY multisig can't withraw from a LC which it funded (271ms)
      ✓ No one can withraw from a LC (78ms)

  Contract: Deploying the GVTY contracts: LCF, CI, GVTYStaking, and GVTYToken 
    CommunityIssuance deployment
      ✓ Stores the deployer's address
    GVTYStaking deployment
      ✓ Stores the deployer's address
    GVTYToken deployment
      ✓ Stores the multisig's address
      ✓ Stores the CommunityIssuance address
      ✓ Stores the LockupContractFactory address
      ✓ Mints the correct GVTY amount to the multisig's address: (9 million)
      ✓ Mints the correct GVTY amount to the CommunityIssuance contract address: 9 million
      ✓ Mints the correct GVTY amount to the bountyAddress EOA: 1 million
      ✓ Mints the correct GVTY amount to the lpRewardsAddress EOA: 2 million
    Community Issuance deployment
      ✓ Stores the deployer's address
      ✓ Has a supply cap of 9 million
      ✓ Givety  can set addresses if CI's GVTY balance is equal or greater than 9 million  (393ms)
      ✓ Givety  can't set addresses if CI's GVTY balance is < 9 million  (367ms)
    Connecting GVTYToken to LCF, CI and GVTYStaking
      ✓ sets the correct GVTYToken address in GVTYStaking (1866ms)
      ✓ sets the correct GVTYToken address in LockupContractFactory
      ✓ sets the correct GVTYToken address in CommunityIssuance (203ms)

  Contract: During the initial lockup period
    GVTY transfer during first year after GVTY deployment
      ✓ Givety multisig can not transfer GVTY to a LC that was deployed directly (105ms)
      ✓ Givety multisig can not transfer to an EOA or Givety system contracts (302ms)
      ✓ Givety multisig can not approve any EOA or Givety system contract to spend their GVTY (546ms)
      ✓ Givety multisig can not increaseAllowance for any EOA or Givety contract (285ms)
      ✓ Givety multisig can not decreaseAllowance for any EOA or Givety contract (338ms)
      ✓ Givety multisig can not be the sender in a transferFrom() call
      ✓ Givety multisig can not stake their GVTY in the staking contract
      ✓ Anyone (other than Givety multisig) can transfer GVTY to LCs deployed by anyone through the Factory (359ms)
      ✓ Anyone (other than Givety multisig) can transfer GVTY to LCs deployed by anyone directly (141ms)
      ✓ Anyone (other than givety multisig) can transfer to an EOA (106ms)
      ✓ Anyone (other than givety multisig) can approve any EOA or to spend their GVTY
      ✓ Anyone (other than givety multisig) can increaseAllowance for any EOA or Givety contract (244ms)
      ✓ Anyone (other than givety multisig) can decreaseAllowance for any EOA or Givety contract (693ms)
      ✓ Anyone (other than givety multisig) can be the sender in a transferFrom() call (62ms)
      ✓ Anyone (other than givety ) can stake their GVTY in the staking contract
    Lockup Contract Factory negative tests
      ✓ deployLockupContract(): reverts when GVTY token address is not set (197ms)
    Transferring GVTY to LCs
      ✓ Givety multisig can transfer GVTY (vesting) to lockup contracts they deployed (198ms)
      ✓ Givety multisig can transfer GVTY to lockup contracts deployed by anyone (1798ms)
    Deploying new LCs
      ✓ GVTY Deployer can deploy LCs through the Factory (46ms)
      ✓ Givety multisig can deploy LCs through the Factory
      ✓ Anyone can deploy LCs through the Factory (80ms)
      ✓ GVTY Deployer can deploy LCs directly (44ms)
      ✓ Givety multisig can deploy LCs directly (45ms)
      ...
      Withdrawal Attempts on LCs before unlockTime has passed 
        ✓ Givety multisig can't withdraw from a funded LC they deployed for another beneficiary through the Factory before the unlockTime
        ✓ Givety multisig can't withdraw from a funded LC that someone else deployed before the unlockTime (52ms)
  ...
  Contract: After the initial lockup period has passed
    Deploying new LCs
      ✓ GVTY Deployer can deploy new LCs
    ...
    Beneficiary withdrawal from initial LC
      ...
      ✓ Beneficiaries can withraw full GVTY balance of LC if it has increased since lockup period ended (234ms)
    Withdrawal attempts from LCs by non-beneficiaries
      ✓ GVTY Multisig can't withdraw from a LC they deployed through the Factory
      ✓ GVTY Multisig can't withdraw from a LC that someone else deployed
      ✓ Non-beneficiaries cannot withdraw from a LC (189ms)
    Transferring GVTY
      ✓ GVTY multisig can transfer GVTY to LCs they deployed (176ms)
      ✓ GVTY multisig can transfer tokens to LCs deployed by anyone (104ms)
      ✓ GVTY multisig can transfer GVTY directly to any externally owned account (141ms)
      ✓ Anyone can transfer GVTY to LCs deployed by anyone (179ms)
      ✓ Anyone can transfer to an EOA (421ms)
      ✓ Anyone can approve any EOA to spend their GVTY
      ✓ Anyone can increaseAllowance for any EOA or Givety contract (2441ms)
      ✓ Anyone can decreaseAllowance for any EOA or Givety contract (1034ms)
      ✓ Anyone can be the sender in a transferFrom() call (101ms)
      ✓ Anyone can stake their GVTY in the staking contract (55ms)
    Withdrawal Attempts on new LCs before unlockTime has passed
      ✓ GVTY Deployer can't withdraw from a funded LC they deployed for another beneficiary through the Factory, before the unlockTime (44ms)
      ✓ GVTY Deployer can't withdraw from a funded LC that someone else deployed, before the unlockTime (448ms)
    ...
    Withdrawals from new LCs after unlockTime has passed
      ✓ GVTY Deployer can't withdraw from a funded LC they deployed for another beneficiary through the Factory, after the unlockTime (100ms)
      ✓ GVTY multisig can't withdraw from a funded LC when they are not the beneficiary, after the unlockTime (529ms)
  ...
  Contract: GVTY community issuance arithmetic tests
...
issuance fraction after: 949066037374286
    ...
    ✓ Total GVTY tokens issued is 42.20 after a minute (38ms)
    ✓ Total GVTY tokens issued is 2,531.94 after an hour (41ms)
    ✓ Total GVTY tokens issued is 60,711.40 after a day (136ms)
    ✓ Total GVTY tokens issued is 422,568.60 after a week (331ms)
    ✓ Total GVTY tokens issued is 1,772,113.21 after a month
    ✓ Total GVTY tokens issued is 5,027,363.22 after 3 months (40ms)
    ✓ Total GVTY tokens issued is 9,264,902.04 after 6 months (47ms)
    ✓ Total GVTY tokens issued is 16,000,000 after a year (47ms)
    ✓ Total GVTY tokens issued is 24,000,000 after 2 years (47ms)
    ✓ Total GVTY tokens issued is 28,000,000 after 3 years (75ms)
    ✓ Total GVTY tokens issued is 30,000,000 after 4 years (40ms)
    ✓ Total GVTY tokens issued is 31,968,750 after 10 years (69ms)
    ✓ Total GVTY tokens issued is 31,999,969.48 after 20 years (68ms)
    ✓ Total GVTY tokens issued is 31,999,999.97 after 30 years (87ms)
  ...
  Contract: GVTYStaking revenue share tests
    ✓ stake(): reverts if amount is zero (74ms)
    ✓ GIVE fee per GVTY staked increases when a redemption fee is triggered and totalStakes > 0 (1562ms)
    ✓ GIVE fee per GVTY staked doesn't change when a redemption fee is triggered and totalStakes == 0 (3540ms)
    ✓ GUSD fee per GVTY staked increases when a redemption fee is triggered and totalStakes > 0 (1757ms)
    ✓ GUSD fee per GVTY staked doesn't change when a redemption fee is triggered and totalStakes == 0 (1705ms)
    ✓ GVTY Staking: A single staker earns all GIVE and GVTY fees that occur (2143ms)
    ✓ stake(): Top-up sends out all accumulated GIVE and GUSD gains to the staker (2138ms)
    ✓ getPendingGIVEGain(): Returns the staker's correct pending GIVE gain (1856ms)
    ✓ getPendingGUSDGain(): Returns the staker's correct pending GUSD gain (1965ms)
    ✓ GVTY Staking: Multiple stakers earn the correct share of all GIVE and GVTY fees, based on their stake size (3133ms)
    ✓ unstake(): reverts if caller has GIVE gains and can't receive GIVE (1149ms)
    ✓ receive(): reverts when it receives GIVE from an address that is not the Active Pool
  ...
  Contract: All Givety functions with onlyOwner modifier
    TroveManager
    ...
    LockupContractFactory
      ✓ setGVTYAddress(): reverts when called by non-owner, with wrong address, or twice (216ms)

  Contract: StabilityPool
    ✓ getGIVE(): gets the recorded GIVE balance
    ✓ getTotalGUSDDeposits(): gets the recorded GUSD balance

  Contract: ActivePool
    ✓ getGIVE(): gets the recorded GIVE balance
    ✓ getGUSDDebt(): gets the recorded GUSD balance
    ✓ increaseGUSD(): increases the recorded GUSD balance by the correct amount
    ✓ decreaseGUSD(): decreases the recorded GUSD balance by the correct amount
    ✓ sendGIVE(): decreases the recorded GIVE balance by the correct amount

  Contract: DefaultPool
    ✓ getGIVE(): gets the recorded GUSD balance
    ✓ getGUSDDebt(): gets the recorded GUSD balance
    ✓ increaseGUSD(): increases the recorded GUSD balance by the correct amount
    ✓ decreaseGUSD(): decreases the recorded GUSD balance by the correct amount (57ms)
    ✓ sendGIVEToActivePool(): decreases the recorded GIVE balance by the correct amount (50ms)
  ...
  Contract: BorrowerWrappers
    ✓ proxy owner can recover GIVE (85ms)
    ✓ non proxy owner cannot recover GIVE
    ...
    ✓ claimStakingGainsAndRecycle(): with only GIVE gain (1595ms)
    ✓ claimStakingGainsAndRecycle(): with only GUSD gain (821ms)
    ✓ claimStakingGainsAndRecycle(): with both GIVE and GUSD gains (1593ms)
  ...
  Contract: StabilityPool - GVTY Rewards
    GVTY Rewards
totalGVTYIssued_1: 30370113195977152000000
totalGVTYIssued_2: 30370113195977152000000
      ✓ liquidation < 1 minute after a deposit does not change totalGVTYIssued (743ms)
      ✓ withdrawFromSP(): reward term G does not update when no GVTY is issued (634ms)
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct GVTY gain. No liquidations. No front end. (1479ms)
      ✓ withdrawFromSP(): Depositors with varying initial deposit withdraw correct GVTY gain. No liquidations. No front end. (1345ms)
      ✓ withdrawFromSP(): Depositors with varying initial deposit withdraw correct GVTY gain. No liquidations. No front end. (1925ms)
      ✓ withdrawFromSP(): Depositor withdraws correct GVTY gain after serial pool-emptying liquidations. No front-ends. (5752ms)
      ✓ GVTY issuance for a given period is not obtainable if the SP was empty during the period (551ms)
      ✓ withdrawFromSP(): Several deposits of 100 GUSD span one scale factor change. Depositors withdraw correct GVTY gains (6075ms)
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct GVTY gain. No liquidations. Front ends and kickback rates. (1660ms)
      ✓ withdrawFromSP(): Depositors with varying initial deposit withdraw correct GVTY gain. Front ends and kickback rates (2813ms)
      ✓ withdrawFromSP(): Several deposits of 10k GUSD span one scale factor change. Depositors withdraw correct GVTY gains (4788ms)

  Contract: Pool Manager: Sum-Product rounding errors
    - Rounding errors: 100 deposits of 100GUSD into SP, then 200 liquidations of 49GUSD

  Contract: StabilityPool - Withdrawal of stability deposit - Reward calculations
    Stability Pool Withdrawal
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and GIVE Gain after one liquidation (757ms)
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and GIVE Gain after two identical liquidations (868ms)
      ✓ withdrawFromSP():  Depositors with equal initial deposit withdraw correct compounded deposit and GIVE Gain after three identical liquidations (1051ms)
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and GIVE Gain after two liquidations of increasing GUSD (965ms)
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and GIVE Gain after three liquidations of increasing GUSD (1152ms)
      ✓ withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and GIVE Gain after two identical liquidations (934ms)
      ✓ withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and GIVE Gain after three identical liquidations (1130ms)
      ✓ withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and GIVE Gain after three varying liquidations (1193ms)

      ✓ withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 GUSD.  A, B, C, D withdraw correct GUSD deposit and GIVE Gain (1512ms)
      ✓ withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 GUSD.  A, B, C, D withdraw correct GUSD deposit and GIVE Gain (1637ms)
      ✓ withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct GUSD deposit and GIVE Gain (1645ms)
      ✓ withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 GUSD.  A, B, C, D withdraw correct GUSD deposit and GIVE Gain (1704ms)
      ✓ withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct GUSD deposit and GIVE Gain (1753ms)
      ✓ withdrawFromSP(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 GUSD. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct GUSD deposit and GIVE Gain (1537ms)
      ...
      ✓ withdrawFromSP(): single deposit fully offset. After subsequent liquidations, depositor withdraws 0 deposit and *only* the GIVE Gain from one liquidation (1049ms)
      ✓ withdrawFromSP(): Depositor withdraws correct compounded deposit after liquidation empties the pool (4540ms)
      ✓ withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and GIVE Gain after one liquidation (918ms)
      ✓ withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraw correct compounded deposit and GIVE Gain after one liquidation (1187ms)
      ✓ withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and GIVE Gain after one liquidation (778ms)
      ✓ withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraws correct compounded deposit and GIVE Gain after one liquidation (3494ms)
alice deposit: 0
      ✓ withdrawFromSP(): Deposit that decreases to less than 1e-9 of it's original value is reduced to 0 (365ms)
      ✓ withdrawFromSP(): Several deposits of 10000 GUSD span one scale factor change. Depositors withdraws correct compounded deposit and GIVE Gain after one liquidation (1676ms)
      ✓ withdrawFromSP(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation (1848ms)
      ✓ withdrawFromSP(): Depositor's GIVE gain stops increasing after two scale changes (4134ms)
      ✓ withdrawFromSP(): Large liquidated coll/debt, deposits and GIVE price (932ms)
      ✓ withdrawFromSP(): Small liquidated coll/debt, large deposits and GIVE price (832ms)

  Contract: StabilityPool - Withdrawal of stability deposit - Reward calculations
    Stability Pool Withdrawal
      ✓ withdrawGIVEGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and GIVE Gain after one liquidation (1227ms)
      ✓ withdrawGIVEGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and GIVE Gain after two identical liquidations (1400ms)
      ✓ withdrawGIVEGainToTrove():  Depositors with equal initial deposit withdraw correct compounded deposit and GIVE Gain after three identical liquidations (1827ms)
      ✓ withdrawGIVEGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and GIVE Gain after two liquidations of increasing GUSD (1597ms)
      ✓ withdrawGIVEGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and GIVE Gain after three liquidations of increasing GUSD (1711ms)
      ✓ withdrawGIVEGainToTrove(): Depositors with varying deposits withdraw correct compounded deposit and GIVE Gain after two identical liquidations (1782ms)
      ✓ withdrawGIVEGainToTrove(): Depositors with varying deposits withdraw correct compounded deposit and GIVE Gain after three identical liquidations (1798ms)
      ✓ withdrawGIVEGainToTrove(): Depositors with varying deposits withdraw correct compounded deposit and GIVE Gain after three varying liquidations (4488ms)

      ✓ withdrawGIVEGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 GUSD.  A, B, C, D withdraw correct GUSD deposit and GIVE Gain (2168ms)
      ✓ withdrawGIVEGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 GUSD.  A, B, C, D withdraw correct GUSD deposit and GIVE Gain (5085ms)
      ✓ withdrawGIVEGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct GUSD deposit and GIVE Gain (2304ms)
      ✓ withdrawGIVEGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 GUSD.  A, B, C, D withdraw correct GUSD deposit and GIVE Gain (2335ms)
      ✓ withdrawGIVEGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct GUSD deposit and GIVE Gain (2145ms)
      ✓ withdrawGIVEGainToTrove(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 GUSD. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct GUSD deposit and GIVE Gain (2543ms)
      ✓ withdrawGIVEGainToTrove(): Depositor withdraws correct compounded deposit after liquidation empties the pool (1889ms)
      ✓ withdrawGIVEGainToTrove(): Pool-emptying liquidation increases epoch by one, resets scaleFactor to 0, and resets P to 1e18 (1891ms)
      ✓ withdrawGIVEGainToTrove(): Depositors withdraw correct compounded deposit after liquidation empties the pool (2324ms)
      ✓ withdrawGIVEGainToTrove(): single deposit fully offset. After subsequent liquidations, depositor withdraws 0 deposit and *only* the GIVE Gain from one liquidation (1719ms)
      ✓ withdrawGIVEGainToTrove(): Depositor withdraws correct compounded deposit after liquidation empties the pool (3958ms)
      ✓ withdrawGIVEGainToTrove(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and GIVE Gain after one liquidation (1276ms)
      ✓ withdrawGIVEGainToTrove(): Several deposits of varying amounts span one scale factor change. Depositors withdraw correct compounded deposit and GIVE Gain after one liquidation (4690ms)
      ✓ withdrawGIVEGainToTrove(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and GIVE Gain after one liquidation (1460ms)
      ✓ withdrawGIVEGainToTrove(): Several deposits of varying amounts span one scale factor change. Depositors withdraws correct compounded deposit and GIVE Gain after one liquidation (1986ms)
alice deposit: 0
      ✓ withdrawGIVEGainToTrove(): Deposit that decreases to less than 1e-9 of it's original value is reduced to 0 (3725ms)
      ✓ withdrawGIVEGainToTrove(): Several deposits of 10000 GUSD span one scale factor change. Depositors withdraws correct compounded deposit and GIVE Gain after one liquidation (2361ms)
      ✓ withdrawGIVEGainToTrove(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation (3814ms)
      ✓ withdrawGIVEGainToTrove(): Large liquidated coll/debt, deposits and GIVE price (959ms)
      ✓ withdrawGIVEGainToTrove(): Small liquidated coll/debt, large deposits and GIVE price (784ms)

  Contract: StabilityPool
    Stability Pool Mechanisms
      ✓ provideToSP(): increases the Stability Pool GUSD balance (354ms)
      ✓ provideToSP(): updates the user's deposit record in StabilityPool (355ms)
      ✓ provideToSP(): reduces the user's GUSD balance by the correct amount (321ms)
      ✓ provideToSP(): increases totalGUSDDeposits by correct amount (283ms)
      ...
      ✓ provideToSP(): reverts if user tries to provide more than their GUSD balance (1030ms)
      ✓ provideToSP(): reverts if user tries to provide 2^256-1 GUSD, which exceeds their balance (508ms)
      ✓ provideToSP(): reverts if cannot receive GIVE Gain (1073ms)
      ✓ provideToSP(): doesn't impact other users' deposits or GIVE gains (1906ms)
      ...
      ✓ provideToSP(): providing 0 GUSD reverts (1091ms)
      ✓ provideToSP(), new deposit: when SP > 0, triggers GVTY reward event - increases the sum G (1072ms)
      ...
      ✓ provideToSP(), new deposit: depositor does not receive any GVTY rewards (671ms)
      ✓ provideToSP(), new deposit after past full withdrawal: depositor does not receive any GVTY rewards (1679ms)
      ✓ provideToSP(), new eligible deposit: tagged front end receives GVTY rewards (1605ms)
      ...
      ✓ provideToSP(), new deposit: depositor does not receive GIVE gains (957ms)
      ✓ provideToSP(), new deposit after past full withdrawal: depositor does not receive GIVE gains (1928ms)
      ✓ provideToSP(), topup: triggers GVTY reward event - increases the sum G (1007ms)
      ✓ provideToSP(), topup from different front end: doesn't change the front end tag (1511ms)
      ✓ provideToSP(), topup: depositor receives GVTY rewards (1487ms)
      ✓ provideToSP(), topup: tagged front end receives GVTY rewards (1036ms)
      ...
      ✓ withdrawFromSP(): partial retrieval - retrieves correct GUSD amount and the entire GIVE Gain, and updates deposit (1102ms)
      ✓ withdrawFromSP(): partial retrieval - leaves the correct amount of GUSD in the Stability Pool (1128ms)
      ✓ withdrawFromSP(): full retrieval - leaves the correct amount of GUSD in the Stability Pool (1091ms)
      ✓ withdrawFromSP(): Subsequent deposit and withdrawal attempt from same account, with no intermediate liquidations, withdraws zero GIVE (1494ms)
      ✓ withdrawFromSP(): it correctly updates the user's GUSD and GIVE snapshots of entitled reward per unit staked (1049ms)
      ✓ withdrawFromSP(): decreases StabilityPool GIVE (3952ms)
      ✓ withdrawFromSP(): All depositors are able to withdraw from the SP to their account (2393ms)
      ✓ withdrawFromSP(): increases depositor's GUSD token balance by the expected amount (2098ms)
      ✓ withdrawFromSP(): doesn't impact other users Stability deposits or GIVE gains (4954ms)
      ...
      ✓ withdrawFromSP(): withdrawing 0 GUSD doesn't alter the caller's deposit or the total GUSD in the Stability Pool (993ms)
      ✓ withdrawFromSP(): withdrawing 0 GIVE Gain does not alter the caller's GIVE balance, their trove collateral, or the GIVE  in the Stability Pool (1139ms)
      ✓ withdrawFromSP(): Request to withdraw > caller's deposit only withdraws the caller's compounded deposit (1260ms)
      ✓ withdrawFromSP(): Request to withdraw 2^256-1 GUSD only withdraws the caller's compounded deposit (1229ms)
      ✓ withdrawFromSP(): caller can withdraw full deposit and GIVE gain during Recovery Mode (4371ms)
      ✓ getDepositorGIVEGain(): depositor does not earn further GIVE gains from liquidations while their compounded deposit == 0:  (1831ms)
      ✓ withdrawFromSP(): triggers GVTY reward event - increases the sum G (982ms)
      ✓ withdrawFromSP(), partial withdrawal: doesn't change the front end tag (1421ms)
      ✓ withdrawFromSP(), partial withdrawal: depositor receives GVTY rewards (1155ms)
      ✓ withdrawFromSP(), partial withdrawal: tagged front end receives GVTY rewards (1245ms)
      ...
      ✓ withdrawGIVEGainToTrove(): reverts when user has no active deposit (1043ms)
      ✓ withdrawGIVEGainToTrove(): Applies GUSDLoss to user's deposit, and redirects GIVE reward to user's Trove (932ms)
      ✓ withdrawGIVEGainToTrove(): reverts if it would leave trove with ICR < MCR (3920ms)
      ✓ withdrawGIVEGainToTrove(): Subsequent deposit and withdrawal attempt from same account, with no intermediate liquidations, withdraws zero GIVE (976ms)
      ✓ withdrawGIVEGainToTrove(): decreases StabilityPool GIVE and increases activePool GIVE (976ms)
      ✓ withdrawGIVEGainToTrove(): All depositors are able to withdraw their GIVE gain from the SP to their Trove (5974ms)
      ✓ withdrawGIVEGainToTrove(): All depositors withdraw, each withdraw their correct GIVE gain (2591ms)
      ✓ withdrawGIVEGainToTrove(): caller can withdraw full deposit and GIVE gain to their trove during Recovery Mode (1629ms)
      ✓ withdrawGIVEGainToTrove(): reverts if user has no trove (967ms)
      ✓ withdrawGIVEGainToTrove(): triggers GVTY reward event - increases the sum G (1298ms)
      ✓ withdrawGIVEGainToTrove(), partial withdrawal: doesn't change the front end tag (4778ms)
      ✓ withdrawGIVEGainToTrove(), eligible deposit: depositor receives GVTY rewards (1607ms)
      ✓ withdrawGIVEGainToTrove(), eligible deposit: tagged front end receives GVTY rewards (1701ms)
      ✓ withdrawGIVEGainToTrove(), eligible deposit: tagged front end's stake decreases (5345ms)
      ✓ withdrawGIVEGainToTrove(), eligible deposit: tagged front end's snapshots update (1984ms)
      ✓ withdrawGIVEGainToTrove(): reverts when depositor has no GIVE gain (1172ms)
...
B pending GIVE reward after L1: 39799999602000003960000000000
...
B stake after A11: 39999999999999999998008332745
  ...
  Contract: TroveManager - Redistribution reward calculations
    ...
    ✓ redistribution: Sequence of alternate opening/liquidation: final surviving trove has GIVE from all previously liquidated troves (1667ms)
  ...
  Contract: TroveManager - in Recovery Mode
    ...
    ✓ liquidate(), with 110% < ICR < TCR, and StabilityPool GUSD > debt to liquidate: offsets the trove entirely with the pool (1265ms)
    ✓ liquidate(), with ICR% = 110 < TCR, and StabilityPool GUSD > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus (1382ms)
    ✓ liquidate(), with  110% < ICR < TCR, and StabilityPool GUSD > debt to liquidate: removes stake and updates totalStakes (1361ms)
    ✓ liquidate(), with  110% < ICR < TCR, and StabilityPool GUSD > debt to liquidate: updates system snapshots (1287ms)
    ✓ liquidate(), with 110% < ICR < TCR, and StabilityPool GUSD > debt to liquidate: closes the Trove (6532ms)
    ✓ liquidate(), with 110% < ICR < TCR, and StabilityPool GUSD > debt to liquidate: can liquidate troves out of order (2667ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool GUSD < liquidated debt: Trove remains active (985ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool GUSD < liquidated debt: Trove remains in TroveOwners array (1134ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool GUSD < liquidated debt: nothing happens (1146ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool GUSD < liquidated debt: updates system shapshots (1284ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool GUSD < liquidated debt: causes correct Pool offset and GIVE gain, and doesn't redistribute to active troves (1266ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool GUSD < liquidated debt: ICR of non liquidated trove does not change (2161ms)
    ✓ liquidate() with ICR > 110%, and StabilityPool GUSD < liquidated debt: total liquidated coll and debt is correct (1912ms)
    ...
    ✓ liquidate(): does not affect the SP deposit or GIVE gain when called on an SP depositor's address that has no trove (708ms)
    ...
    ✓ liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and GIVE gain (1556ms)
    ✓ liquidateTroves(): Liquidating troves at ICR <=100% with SP deposits does not alter their deposit or GIVE gain (1391ms)
...
gasUsed:  636956
    ...
    ✓ batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool GUSD > debt to liquidate: can liquidate troves out of order (1357ms)
  ...
  Contract: TroveManager
    ...
    ✓ liquidate(): decreases ActivePool GIVE and GUSDDebt by correct amounts (567ms)
    ✓ liquidate(): increases DefaultPool GIVE and GUSD debt by correct amounts (553ms)
    ...
    ✓ liquidate(): updates the L_GIVE and L_GUSDDebt reward-per-unit-staked totals (984ms)
    ...
    ✓ liquidate(): does not affect the SP deposit or GIVE gain when called on an SP depositor's address that has no trove (964ms)
    ✓ liquidate(): does not liquidate a SP depositor's trove with ICR > 110%, and does not affect their SP deposit or GIVE gain (892ms)
    ✓ liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and GIVE gain (1141ms)
    ...
    ✓ liquidate(): when SP > 0, triggers GVTY reward event - increases the sum G (1081ms)
    ...
    ✓ liquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and GIVE gain (1214ms)
    ✓ liquidateTroves(): when SP > 0, triggers GVTY reward event - increases the sum G (5057ms)
    ...
    ✓ batchLiquidateTroves: when SP > 0, triggers GVTY reward event - increases the sum G (1428ms)
    ...
    ✓ redeemCollateral(): cancels the provided GUSD with debt from Troves with the lowest ICRs and sends an equivalent amount of Give (838ms)
    ...
    ✓ redeemCollateral(): doesn't affect the Stability Pool deposits or GIVE gain of redeemed-from troves (2311ms)
    ✓ redeemCollateral(): caller can redeem their entire GUSDToken balance (1086ms)
    ✓ redeemCollateral(): reverts when requested redemption amount exceeds caller's GUSD token balance (1320ms)
    ✓ redeemCollateral(): value of issued GIVE == face value of redeemed GUSD (assuming 1 GUSD has value of $1) (1353ms)
    ...
    ✓ redeemCollateral(): a redemption made at zero base rate send a non-zero ETHFee to GVTY staking contract (5136ms)
    ✓ redeemCollateral(): a redemption made at zero base increases the GIVE-fees-per-GVTY-staked in GVTY Staking contract (1207ms)
    ✓ redeemCollateral(): a redemption made at a non-zero base rate send a non-zero ETHFee to GVTY staking contract (1730ms)
    ✓ redeemCollateral(): a redemption made at a non-zero base rate increases GIVE-per-GVTY-staked in the staking contract (5971ms)
    ✓ redeemCollateral(): a redemption sends the GIVE remainder (ETHDrawn - ETHFee) to the redeemer (1113ms)
    ...
    ✓ redeemCollateral(): a redemption that closes a trove leaves the trove's GIVE surplus (collateral - GIVE drawn) available for the trove owner to claim (5373ms)
    ✓ redeemCollateral(): a redemption that closes a trove leaves the trove's GIVE surplus (collateral - GIVE drawn) available for the trove owner after re-opening trove (2154ms)
    ✓ redeemCollateral(): reverts if fee eats up all returned collateral (1412ms)
    ✓ getPendingGUSDDebtReward(): Returns 0 if there is no pending GUSDDebt reward (618ms)
    ✓ getPendingETHReward(): Returns 0 if there is no pending GIVE reward (620ms)
    ✓ computeICR(): Returns 0 if trove's coll is worth 0
    ✓ computeICR(): Returns 2^256-1 for GIVE:USD = 100, coll = 1 GIVE, debt = 100 GUSD
    ✓ computeICR(): returns correct ICR for GIVE:USD = 100, coll = 200 GIVE, debt = 30 GUSD
    ✓ computeICR(): returns correct ICR for GIVE:USD = 250, coll = 1350 GIVE, debt = 127 GUSD
    ✓ computeICR(): returns correct ICR for GIVE:USD = 100, coll = 1 GIVE, debt = 54321 GUSD
  ...
  Contract: Givpool
    ...
    Givpool, before calling setAddresses
      ✓ Stake fails
      ✓ Withdraw falis
      ✓ Claim fails
      ✓ Exit fails
