...
contract('Echidna debugger', async accounts => {
  ...
  before(async () => {
    ...
    gusdToken = await GUSDToken.at(await echidnaTester.gusdToken())
  ...
  it.only('GUSD balance', async () => {
    await echidnaTester.openTroveExt('0', '0', '4210965169908805439447313562489173090')

    const totalSupply = await gusdToken.totalSupply();
    const gasPoolBalance = await gusdToken.balanceOf(GAS_POOL_ADDRESS);
    const activePoolBalance = await activePool.getGUSDDebt();
    const defaultPoolBalance = await defaultPool.getGUSDDebt();
    const stabilityPoolBalance = await stabilityPool.getTotalGUSDDeposits();
    const currentTrove = await echidnaTester.echidnaProxies(0);
    const troveBalance = gusdToken.balanceOf(currentTrove);
