...
contract("PoolManager - random liquidations/deposits, then check all depositors can withdraw", async accounts => {
  ...
  const performLiquidation = async (remainingDefaulters, liquidatedAccountsDict) => {
    ...
    const liquidatedGUSD = (await troveManager.Troves(randomDefaulter))[0]
    ...
    const GUSDinPoolBefore = await stabilityPool.getTotalGUSDDeposits()
    const liquidatedTx = await troveManager.liquidate(randomDefaulter, { from: accounts[0] })
    const GUSDinPoolAfter = await stabilityPool.getTotalGUSDDeposits()
    ...
    console.log(`Liquidation. addr: ${th.squeezeAddr(randomDefaulter)} ICR: ${ICRPercent}% coll: ${liquidatedETH} debt: ${liquidatedGUSD} SP GUSD before: ${GUSDinPoolBefore} SP GUSD after: ${GUSDinPoolAfter} tx success: ${liquidatedTx.receipt.status}`)
  }
  ...
  const performSPDeposit = async (depositorAccounts, currentDepositors, currentDepositorsDict) => {
    ...
    const userBalance = (await gusdToken.balanceOf(randomDepositor))
    const maxGUSDDeposit = userBalance.div(toBN(dec(1, 18)))

    const randomGUSDAmount = th.randAmountInWei(1, maxGUSDDeposit)

    const depositTx = await stabilityPool.provideToSP(randomGUSDAmount, ZERO_ADDRESS, { from: randomDepositor })
    ...
    console.log(`SP deposit. addr: ${th.squeezeAddr(randomDepositor)} amount: ${randomGUSDAmount} tx success: ${depositTx.receipt.status} `)
  }
  ...
  const clearAllUndercollateralizedTroves = async (price) => {
    ...
    */
    while(await systemContainsTroveUnder100(price) && await troveManager.checkRecoveryMode()) {
      ...
      await gusdToken.transfer(lowestTrove, lowestTroveDebt, {from: whale})
  ...
  const attemptWithdrawAllDeposits = async (currentDepositors) => {
    ...
    for (depositor of currentDepositors) {
      ...
      const finalDeposit = await stabilityPool.getCompoundedGUSDDeposit(depositor)
      const GIVEGain = await stabilityPool.getDepositorGIVEGain(depositor)
      ...
      const ETHinSPAfter = (await stabilityPool.getGIVE()).toString()
      const GUSDinSPAfter = (await stabilityPool.getTotalGUSDDeposits()).toString()
      const GUSDBalanceSPAfter = (await gusdToken.balanceOf(stabilityPool.address))
      const depositAfter = await stabilityPool.getCompoundedGUSDDeposit(depositor)
      ...
      console.log(`--Before withdrawal--
                    ...
                    --After withdrawal--
                     ...
                     GIVE remaining in SP: ${ETHinSPAfter}
                     SP GUSD deposits tracker after: ${GUSDinSPAfter}
                     SP GUSD balance after: ${GUSDBalanceSPAfter}
                     `)
  ...
  describe("Stability Pool Withdrawals", async () => {
    ...
    it("Defaulters' Collateral in range [1, 1e8]. SP Deposits in range [100, 1e10]. GIVE:USD = 100", async () => {
      ...
      const defaulterGUSDProportionMin = 91
      const defaulterGUSDProportionMax = 180

      const depositorCollMin = 1
      const depositorCollMax = 100000000
      const depositorGUSDProportionMin = 100
      const depositorGUSDProportionMax = 100
      ...
      // setup:
      // account set L all add coll and withdraw GUSD
      await th.openTrove_allAccounts_randomGIVE_randomGUSD(defaulterCollMin,
        defaulterCollMax,
        defaulterAccounts,
        contracts,
        defaulterGUSDProportionMin,
        defaulterGUSDProportionMax,
        true)

      // account set S all add coll and withdraw GUSD
      await th.openTrove_allAccounts_randomGIVE_randomGUSD(depositorCollMin,
        depositorCollMax,
        depositorAccounts,
        contracts,
        depositorGUSDProportionMin,
        depositorGUSDProportionMax,
        true)
      ...
      const totalGUSDDepositsBeforeWithdrawals = await stabilityPool.getTotalGUSDDeposits()
      const totalETHRewardsBeforeWithdrawals = await stabilityPool.getGIVE()

      await attemptWithdrawAllDeposits(currentDepositors)

      const totalGUSDDepositsAfterWithdrawals = await stabilityPool.getTotalGUSDDeposits()
      const totalETHRewardsAfterWithdrawals = await stabilityPool.getGIVE()

      console.log(`Total GUSD deposits before any withdrawals: ${totalGUSDDepositsBeforeWithdrawals}`)
      console.log(`Total GIVE rewards before any withdrawals: ${totalETHRewardsBeforeWithdrawals}`)

      console.log(`Remaining GUSD deposits after withdrawals: ${totalGUSDDepositsAfterWithdrawals}`)
      console.log(`Remaining GIVE rewards after withdrawals: ${totalETHRewardsAfterWithdrawals}`)
    ...
    it("Defaulters' Collateral in range [1, 10]. SP Deposits in range [1e8, 1e10]. GIVE:USD = 100", async () => {
      // whale adds coll that holds TCR > 150%
      await borrowerOperations.openTrove(0, 0, whale, whale, { from: whale, value: dec(5, 29) })
    ...
    it("Defaulters' Collateral in range [1e6, 1e8]. SP Deposits in range [100, 1000]. Every liquidation empties the Pool. GIVE:USD = 100", async () => {
      // whale adds coll that holds TCR > 150%
      await borrowerOperations.openTrove(0, 0, whale, whale, { from: whale, value: dec(5, 29) })
    ...
    it("Defaulters' Collateral in range [1e6, 1e8]. SP Deposits in range [1e8 1e10]. GIVE:USD = 100", async () => {
      // whale adds coll that holds TCR > 150%
      await borrowerOperations.openTrove(0, 0, whale, whale, { from: whale, value: dec(5, 29) })
