...
import "./Interfaces/IGivpool.sol";
import "../Dependencies/console.sol";


// Adapted from: https://github.com/BCharity-Net/givety/blob/master/contracts/Givpool.sol
// Some more useful references:
// Synthetix proposal: https://sips.synthetix.io/sips/sip-31
// Original audit: https://github.com/BCharity-Net/givety/blob/master/synthetix/Givpool/review.pdf
// Incremental changes (commit by commit) from the original to this version: https://github.com/BCharity-Net/givety/pull/271
...
contract LPTokenWrapper is ILPTokenWrapper {
    ...
    function stake(uint256 amount) public virtual override {
        ...
        givToken.safeTransferFrom(msg.sender, address(this), amount);
    }

    function withdraw(uint256 amount) public virtual override {
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        givToken.safeTransfer(msg.sender, amount);
    }
}

/*
 * On deployment a new GiveSwap pool will be created for the pair GUSD/GIVE and its token will be set here.

 * Essentially the way it works is:

 * - Liquidity providers add funds to the GiveSwap pool, and get Giv LP tokens in exchange
 * - Liquidity providers stake those Giv LP tokens into Givpool rewards contract
 ...
 * - Liquidity providers can unstake Giv LP tokens to exit the program (i.e., stop earning rewards) when they want

 * Funds for rewards will only be added once, on deployment of GVTY token,
 ...
 * in order to avoid getting GVTY tokens locked.
 * That also means that the start time for the program will be the event that occurs first:
 * either GVTY token contract is deployed, and therefore GVTY tokens are minted to Givpool contract,
 * or first liquidity provider stakes Giv LP tokens into it.
 */
contract Givpool is LPTokenWrapper, Ownable, CheckContract, IGivpool {
    string constant public NAME = "Givpool";
    ...
    event GivTokenAddressChanged(address _givTokenAddress);
        ...
        onlyOwner
    {
        ...
        checkContract(_givTokenAddress);

        givToken = IERC20(_givTokenAddress);
        gvtyToken = IGVTYToken(_gvtyTokenAddress);
        duration = _duration;

        _notifyRewardAmount(gvtyToken.getLpRewardsEntitlement(), _duration);

        emit GVTYTokenAddressChanged(_gvtyTokenAddress);
        emit GivTokenAddressChanged(_givTokenAddress);
    ...
    function lastTimeRewardApplicable() public view override returns (uint256) {
        return GivetyMath._min(block.timestamp, periodFinish);
    }
    ...
    function stake(uint256 amount) public override {
        ...
        require(address(givToken) != address(0), "Liquidity Pool Token has not been set yet");
    ...
    function claimReward() public override {
        ...
        gvtyToken.transfer(msg.sender, reward);
    ...
    function _notifyRewardAmount(uint256 _reward, uint256 _duration) internal {
        assert(_reward > 0);
        assert(_reward == gvtyToken.balanceOf(address(this)));
