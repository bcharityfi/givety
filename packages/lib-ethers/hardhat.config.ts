...
import { _connectToContracts, _GivetyDeploymentJSON, _priceFeedIsTestnet } from "./src/contracts";
...
const hasGIVE = (network: string): network is keyof typeof giveAddresses => network in giveAddresses;
...
extendEnvironment(env => {
  env.deployGivety = async (
    deployer,
    useRealPriceFeed = false,
    giveAddress = undefined,
    overrides?: Overrides
  ) => {
...
task("deploy", "Deploys the contracts to the network")
  ...
  .addOptionalParam(
    "createGiveSwapPair",
    "Create a real GiveSwap v2 GIVE-GUSD pair instead of a mock ERC20 token",
    undefined,
    types.boolean
  )
  .setAction(
    async ({ channel, gasPrice, useRealPriceFeed, createGiveSwapPair }: DeployParams, env) => {
      ...
      let giveAddress: string | undefined = undefined;
      if (createGiveSwapPair) {
        if (!hasGIVE(env.network.name)) {
          throw new Error(`GIVE not deployed on ${env.network.name}`);
        }
        giveAddress = giveAddresses[env.network.name];
      }

      setSilent(false);

      const deployment = await env.deployGivety(deployer, useRealPriceFeed, giveAddress, overrides);
