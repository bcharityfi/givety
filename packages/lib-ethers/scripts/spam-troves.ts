...
import { Decimal, GUSD_MINIMUM_DEBT, Trove } from "givety-lib-base";
import { EthersGivety, EthersGivetyWithStore, BlockPolledGivetyStore } from "@givety/lib-ethers";
...
let funder: Wallet;
let givety: EthersGivetyWithStore<BlockPolledGivetyStore>;
...
const createTrove = async (nominalCollateralRatio: Decimal) => {
  ...
  const debt = GUSD_MINIMUM_DEBT.mul(2);
  ...
  await givety.populate
    .openTrove(
      Trove.recreate(new Trove(collateral, debt), givety.store.state.borrowingRate),
      {},
...
const runLoop = async () => {
  for (let i = 0; i < numberOfTrovesToCreate; ++i) {
    ...
    const nominalCollateralRatio = collateralRatio.div(givety.store.state.price);
...
const main = async () => {
  ...
  givety = await EthersGivety.connect(provider, { useStore: "blockPolled" });

  let stopStore: () => void;

  return new Promise<void>(resolve => {
    givety.store.onLoaded = resolve;
    stopStore = givety.store.start();
  })
    .then(runLoop)
    .then(() => {
      stopStore();
