...
export class SubgraphGivety implements ReadableGivety, ObservableGivety {
  private client: ApolloClient<NormalizedCacheObject>;

  constructor(uri = "http://localhost:8000/subgraphs/name/givety/subgraph", pollInterval = 4000) {
  ...
  getGUSDInStabilityPool() {
    return tokensInStabilityPool.get(this.client, undefined);
  }

  watchGUSDInStabilityPool(onGUSDInStabilityPoolChanged: (gusdInStabilityPool: Decimal) => void) {
    return tokensInStabilityPool.watch(this.client, onGUSDInStabilityPoolChanged, undefined);
  }
