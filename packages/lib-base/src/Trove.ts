...
/** @internal */ export type _GUSDBorrowing<T> = { borrowGUSD: T };
/** @internal */ export type _GUSDRepayment<T> = { repayGUSD: T };
...
/** @internal */ export type _NoGUSDBorrowing = Partial<_GUSDBorrowing<undefined>>;
/** @internal */ export type _NoGUSDRepayment = Partial<_GUSDRepayment<undefined>>;
...
export type _DebtChange<T> =
  | (_GUSDBorrowing<T> & _NoGUSDRepayment)
  | (_GUSDRepayment<T> & _NoGUSDBorrowing);

/** @internal */
export type _NoDebtChange = _NoGUSDBorrowing & _NoGUSDRepayment;

/**
 * Parameters of an {@link TransactableGivety.openTrove | openTrove()} transaction.
 *
 * @remarks
 ...
 *     <td> borrowGUSD </td>
 *     <td> T </td>
 *     <td> The amount of GUSD that's borrowed. </td>
 *   </tr>
 *
 * </table>
 *
...
/**
 * Parameters of a {@link TransactableGivety.closeTrove | closeTrove()} transaction.
 *
 * @remarks
 ...
 *     <td> repayGUSD? </td>
 *     <td> T </td>
 *     <td> <i>(Optional)</i> The amount of GUSD that's repaid. </td>
 *   </tr>
 *
 * </table>
 *
...
export type TroveClosureParams<T> = _CollateralWithdrawal<T> &
  ...
  Partial<_GUSDRepayment<T>> &
  _NoGUSDBorrowing;

/**
 * Parameters of an {@link TransactableGivety.adjustTrove | adjustTrove()} transaction.
 *
 * @remarks
 ...
 * least one.
 *
 * Defining both `depositCollateral` and `withdrawCollateral`, or both `borrowGUSD` and `repayGUSD`
 ...
 *     <td> borrowGUSD? </td>
 *     <td> T </td>
 *     <td> <i>(Optional)</i> The amount of GUSD that's borrowed. </td>
 *   </tr>
 *
 *   <tr>
 *     <td> repayGUSD? </td>
...
const debtChangeFrom = <T>({
  ...
  if (borrowGUSD !== undefined && repayGUSD !== undefined) {
    throw new Error(
      "TroveAdjustmentParams: 'borrowGUSD' and 'repayGUSD' can't be present at the same time"
    );
  }

  if (borrowGUSD !== undefined) {
    return { borrowGUSD };
  }

  if (repayGUSD !== undefined) {
    return { repayGUSD };
  }
};
...
export class Trove {
  ...
  /**
   * Amount of GUSD that must be repaid to close this Trove.
   *
   * @remarks
   * This doesn't include the liquidation reserve, which is refunded in case of normal closure.
   */
  get netDebt(): Decimal {
    if (this.debt.lt(GUSD_LIQUIDATION_RESERVE)) {
      throw new Error(`netDebt should not be used when debt < ${GUSD_LIQUIDATION_RESERVE}`);
    }

    return this.debt.sub(GUSD_LIQUIDATION_RESERVE);
  }
   ...
   *
   * @example
   * Can be used to check whether the Givety protocol is in recovery mode by using it on the return
   * value of {@link ReadableGivety.getTotal | getTotal()}. For example:
   *
   * ```typescript
  ...
  private _debtChange({ debt }: Trove, borrowingRate: Decimalish): _DebtChange<Decimal> {
    return debt.gt(this.debt)
      ? { borrowGUSD: unapplyFee(borrowingRate, debt.sub(this.debt)) }
      : { repayGUSD: this.debt.sub(debt) };
  }
    ...
    if (that.isEmpty) {
      return troveClosure(
        this.netDebt.nonZero
          ? { withdrawCollateral: this.collateral, repayGUSD: this.netDebt }
  ...
  apply(
    ...
    switch (change.type) {
      ...
      case "creation": {
        ...
        const { depositCollateral, borrowGUSD } = change.params;

        return new Trove(
          depositCollateral,
          GUSD_LIQUIDATION_RESERVE.add(applyFee(borrowingRate, borrowGUSD))
        );
      }
      ...
      case "adjustment": {
        const {
          setToZero,
          params: { depositCollateral, withdrawCollateral, borrowGUSD, repayGUSD }
        } = change;

        const collateralDecrease = withdrawCollateral ?? Decimal.ZERO;
        const collateralIncrease = depositCollateral ?? Decimal.ZERO;
        const debtDecrease = repayGUSD ?? Decimal.ZERO;
        const debtIncrease = borrowGUSD ? applyFee(borrowingRate, borrowGUSD) : Decimal.ZERO;
  ...
  /**
   * Calculate the result of an {@link TransactableGivety.openTrove | openTrove()} transaction.
   *
  ...
  /**
   * Calculate the parameters of an {@link TransactableGivety.openTrove | openTrove()} transaction
  ...
  /**
   * Calculate the result of an {@link TransactableGivety.adjustTrove | adjustTrove()} transaction
   * on this Trove.
   *
   * @param params - Parameters of the transaction.
  ...
  /**
   * Calculate the parameters of an {@link TransactableGivety.adjustTrove | adjustTrove()}
 ...
 * example to represent the {@link ReadableGivety.getTotal | total collateral and debt} locked up
 * in the protocol.
 *
