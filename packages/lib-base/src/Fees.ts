 ...
 *
 * @remarks
 * Returned by the {@link ReadableGivety.getFees | getFees()} function.
 *
 * @public
 */
export class Fees {
   ...
   * the `when` parameter.
   *
   * To calculate the borrowing fee in GUSD, multiply the borrowed GUSD amount by the borrowing rate.
   *
   * @example
   * ```typescript
   * const fees = await givety.getFees();
   *
   * const borrowedGUSDAmount = 100;
   * const borrowingRate = fees.borrowingRate();
   * const borrowingFeeGUSD = borrowingRate.mul(borrowedGUSDAmount);
   * ```
   */
   ...
   *
   * @param redeemedFractionOfSupply - The amount of GUSD being redeemed divided by the total supply.
   ...
   * precise, it depends on the fraction of the redeemed amount compared to the total GUSD supply,
   * which must be passed as a parameter.
   *
   * To calculate the redemption fee in GUSD, multiply the redeemed GUSD amount with the redemption
   * rate.
   *
   * @example
   * ```typescript
   ...
   *
   * const redeemedGUSDAmount = Decimal.from(100);
   * const redeemedFractionOfSupply = redeemedGUSDAmount.div(total.debt);
   * const redemptionRate = fees.redemptionRate(redeemedFractionOfSupply);
   * const redemptionFeeGUSD = redemptionRate.mul(redeemedGUSDAmount);
   * ```
   */
