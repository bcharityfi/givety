...
import { EthersGivety, ReadableEthersGivety } from "@givety/lib-ethers";
...
export const createRandomTrove = (price: Decimal) => {
  ...
  if (Math.random() < 0.5) {
    ...
    const debt = GUSD_LIQUIDATION_RESERVE.add(truncateLastDigits(maxDebt - benford(maxDebt)));

    return new Trove(collateral, debt);
  } else {
    const debt = GUSD_LIQUIDATION_RESERVE.add(100 * randomValue);
...
export const randomDebtChange = ({ debt }: Trove) =>
  Math.random() < 0.5
    ? { repayGUSD: debt.mul(1.1 * Math.random()) }
    : { borrowGUSD: debt.mul(0.5 * Math.random()) };

export const getListOfTroves = async (givety: ReadableGivety) =>
  givety.getTroves({
    first: await givety.getNumberOfTroves(),
    sortedBy: "descendingCollateralRatio",
    beforeRedistribution: false
  });

export const getListOfTrovesBeforeRedistribution = async (givety: ReadableGivety) =>
  givety.getTroves({
    first: await givety.getNumberOfTroves(),
    sortedBy: "descendingCollateralRatio",
...
export const getListOfTroveOwners = async (givety: ReadableGivety) =>
  getListOfTrovesBeforeRedistribution(givety).then(troves => troves.map(([owner]) => owner));
...
export const checkPoolBalances = async (
  givety: ReadableEthersGivety,
  listOfTroves: [string, TroveWithPendingRedistribution][],
  totalRedistributed: Trove
) => {
  const activePool = await givety._getActivePool();
  const defaultPool = await givety._getDefaultPool();
...
class EqualityCheck<T> {
  private name: string;
  private get: (l: ReadableGivety) => Promise<T>;
  private equals: (a: T, b: T) => boolean;

  constructor(
    name: string,
    get: (l: ReadableGivety) => Promise<T>,
  ...
  async allEqual(liquities: ReadableGivety[]) {
...
const checks = [
  ...
  new EqualityCheck("tokensInStabilityPool", l => l.getGUSDInStabilityPool(), decimalsEqual)
];

export const checkSubgraph = async (subgraph: SubgraphGivety, l1Givety: ReadableGivety) => {
  await Promise.all(checks.map(check => check.allEqual([subgraph, l1Givety])));

  const l1ListOfTroves = await getListOfTrovesBeforeRedistribution(l1Givety);
...
export const connectUsers = (users: Signer[]) =>
  Promise.all(users.map(user => EthersGivety.connect(user)));
