...
import { EthersTransactionFailedError, SentEthersGivetyTransaction } from "@givety/lib-ethers";
import { MinedReceipt } from "givety-lib-base";
...
const retryUpTo = async (
  times: number,
  sendTx: () => Promise<SentEthersGivetyTransaction>
...
export class GasHistogram<T> {
  ...
  async expectSuccess(sendTx: () => Promise<SentEthersGivetyTransaction<T>>): Promise<void> {
