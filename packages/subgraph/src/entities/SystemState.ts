...
export function getCurrentSystemState(): SystemState {
  ...
  if (currentSystemStateOrNull == null) {
    ...
    newSystemState.totalGVTYTokensStaked = DECIMAL_ZERO;
    newSystemState.save();
...
export function updateSystemStateByGvtyStakeChange(stakeChange: GvtyStakeChange): void {
  let systemState = getCurrentSystemState();

  systemState.totalGVTYTokensStaked = systemState.totalGVTYTokensStaked.plus(
