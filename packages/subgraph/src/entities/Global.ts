...
export function getGlobal(): Global {
  ...
  if (globalOrNull != null) {
    ...
    newGlobal.totalNumberOfGVTYStakes = 0;
    newGlobal.numberOfActiveGVTYStakes = 0;
...
export function updateTotalRedistributed(L_GIVE: BigInt, L_GUSDDebt: BigInt): void {
  let global = getGlobal();

  global.rawTotalRedistributedCollateral = L_GIVE;
  global.rawTotalRedistributedDebt = L_GUSDDebt;
  global.save();
}
...
export function increaseTotalNumberOfGVTYStakes(): void {
  let global = getGlobal();

  global.totalNumberOfGVTYStakes++;
  global.numberOfActiveGVTYStakes++;
  global.save();
}

export function increaseNumberOfActiveGVTYStakes(): void {
  let global = getGlobal();
...
export function decreaseNumberOfActiveGVTYStakes(): void {
  let global = getGlobal();
...
export function increaseTotalBorrowingFeesPaid(_GUSDFee: BigInt): void {
  let global = getGlobal();
  global.totalBorrowingFeesPaid = global.totalBorrowingFeesPaid.plus(decimalize(_GUSDFee));
  global.save();
}
