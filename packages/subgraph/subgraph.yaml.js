...
const { addresses, startBlock } = require(`givety-lib-ethers/deployments/${network}.json`);
...
const manifest = yaml`
specVersion: 0.0.2
description: Givety is a decentralized borrowing protocol offering interest-free liquidity against collateral in Give.
repository: https://github.com/BCharity-Net/givety/main/packages/subgraph
schema:
  file: ./schema.graphql
dataSources:
  - name: TroveManager
  ...
  - name: BorrowerOperations
    ...
    mapping:
      ...
      eventHandlers:
        - event: TroveUpdated(indexed address,uint256,uint256,uint256,uint8)
          handler: handleTroveUpdated
        - event: GUSDBorrowingFeePaid(indexed address,uint256)
          handler: handleGUSDBorrowingFeePaid
  - name: PriceFeed
    kind: ethereum/contract
    network: mainnet
    source:
  ...
  - name: StabilityPool
    ...
    mapping:
      ...
      eventHandlers:
        ...
        - event: GIVEGainWithdrawn(indexed address,uint256,uint256)
          handler: handleGIVEGainWithdrawn
  ...
  - name: GVTYStaking
    ...
    source:
      abi: GVTYStaking
      address: "${addresses.gvtyStaking}"
      startBlock: ${startBlock}
    mapping:
      file: ./src/mappings/GvtyStake.ts
      language: wasm/assemblyscript
      kind: ethereum/events
      apiVersion: 0.0.4
      entities:
      ...
      abis:
        - name: GVTYStaking
          file: ../lib-ethers/abi/GVTYStaking.json
      eventHandlers:
...
${[
  ["GUSDToken", addresses.gusdToken],
  ["GVTYToken", addresses.gvtyToken]
].map(
  ([name, address]) => yaml`
  - name: ${name}
    kind: ethereum/contract
